...
TL;DR: Toolathlon是一个包含32个应用程序和604个工具的语言代理基准，旨在评估代理在真实世界、长周期任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有语言代理基准在狭窄的领域或简化的任务上，缺乏真实性和长周期复杂性来评估真实世界性能。

Method: Toolathlon提供了多样化的应用程序和工具，真实的环境设置和基于执行的评估。它跨越32个应用程序和604个工具，包括日常和专业平台。大多数工具基于高质量的MCP服务器。我们提供了来自真实软件的真实初始环境状态，例如包含大量学生或真实金融电子表格的Canvas课程。该基准包含108个手动收集或制作的任务，平均需要大约20次交互才能完成，并通过专门的评估脚本进行严格验证。

Result: 在对SOTA模型的全面评估中，表现最好的模型Claude-4.5-Sonnet的成功率为38.6%，平均工具调用次数为20.2次，而顶尖的开源模型DeepSeek-V3.2-Exp的成功率为20.1%。

Conclusion: 我们期望Toolathlon能够推动开发更强大的语言代理，以应对真实世界中的长周期任务执行。
...
TL;DR: 该研究提出了一个用于Excel公式修复的数据集和基线方法，以解决新手用户在处理Excel公式时遇到的常见运行时错误。


<details>
  <summary>Details</summary>
Motivation: Excel新手用户常因逻辑错误或函数误解导致运行时错误，现有LLM虽能解释错误但无法自动修复，且缺乏高质量的训练和评估数据集。

Method: 提出数据生成流程，利用在线论坛的种子样本，结合LLM的少样本提示学习和LLM-as-a-Judge验证框架，以及基于执行的检查，生成包含618个样本的数据集。并提出一个利用LLM结合错误公式和电子表格上下文的基线修复技术。

Result: 生成了一个包含618个高质量样本的数据集，覆盖了常见的运行时错误。在评估中，使用执行指标测试了GPT-4o、GPT-4.1、Phi-3、Mistral等LLM在新建基准上的表现，并进行了人工标注和错误/函数分布分析。

Conclusion: 该研究填补了Excel公式修复领域高质量数据集的空白，提出的数据生成方法具有良好的可扩展性，可用于其他低资源编程语言的代码修复任务。
