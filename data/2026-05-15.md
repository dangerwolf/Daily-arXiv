...
TL;DR: LLMs 擅长生成和操作结构化数据，本研究提出 SpreadsheetArena 平台来评估 LLM 在生成电子表格方面的能力。


<details>
  <summary>Details</summary>
Motivation: 评估 LLM 在根据用户需求生成电子表格方面的能力，并找出该领域面临的挑战。

Method: 提出 SpreadsheetArena 平台，通过用户对 LLM 生成的电子表格进行盲法成对比较来评估模型性能。

Result: 研究发现，用户偏好的电子表格在风格、结构和功能上因用例而异；即使是排名靠前的模型，在金融领域生成的电子表格也未必符合专业最佳实践。

Conclusion: 电子表格生成是一个对 LLM 具有挑战性且有趣的复杂开放式任务类别，需要进一步研究。
...
TL;DR: LLMs在处理大型企业电子表格时存在困难，本文提出了FRTR-Bench基准和FRTR框架，显著提高了模型在多模态电子表格推理上的准确性，并降低了成本。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型在处理包含大量数据、链接工作表和视觉元素的复杂企业电子表格时存在可扩展性和多模态理解的挑战。

Method: 提出了FRTR-Bench，一个包含30个企业级Excel工作簿的大型多模态电子表格推理基准。并提出了FRTR框架，通过分解电子表格为行、列和块嵌入，采用混合词汇-密集检索（RRF）和多模态嵌入来处理数值和视觉信息。

Result: 在FRTR-Bench上，Claude Sonnet 4.5达到了74%的准确率，远超先前方法的24%。在使用GPT-5时，FRTR在SpreadsheetLLM基准上达到了87%的准确率，并将令牌使用量减少了约50%。

Conclusion: FRTR框架在多模态电子表格推理方面取得了显著的性能提升，为解决LLMs在处理复杂企业数据方面的挑战提供了有效方案。
...
TL;DR: SQuARE是一个混合检索框架，通过考虑表头的深度和合并单元格的密度来识别最适合回答用户问题的检索策略，从而解决电子表格问答的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理包含多行表头、合并单元格和单位注释的真实电子表格，并且缺乏一致模式的SQL视图也无法应对这些挑战。

Method: SQuARE采用分层检索方法，首先计算一个基于表头深度和合并密度的连续分数来确定路由策略。然后，它可以通过两种方式之一进行查询：一是通过保留结构的块检索，二是针对自动构建的关系表示执行SQL查询。当置信度低时，一个轻量级代理会监督来自两个路径的检索、精炼或结果的组合。

Result: 在包含多表头公司资产负债表、高度合并的联合国机构工作簿和各种公共数据集的数据集上，SQuARE在检索精度和端到端答案准确性方面均优于单一策略基线和ChatGPT-4o，同时保持了可预测的延迟。

Conclusion: SQuARE通过解耦检索和模型选择，提高了电子表格问答的准确性和鲁棒性，并能兼容未来的表格基础模型。
...
TL;DR: TableLLM是一个80亿参数的大型语言模型，专门用于处理文档和电子表格中的表格数据操作任务。


<details>
  <summary>Details</summary>
Motivation: 为了满足真实办公场景中对表格数据处理的需求，需要一个能够熟练处理文档和电子表格中表格数据的模型。

Method: 提出了一个远程监督训练方法，包括一个推理过程扩展策略和一个交叉验证策略，以提高模型理解推理模式和保证自动生成数据质量的能力。同时，构建了针对文档和电子表格格式的基准测试和评估流程。

Result: 通过在精心设计的基准测试上的评估，TableLLM在处理表格数据方面表现优于现有的通用和专用大型语言模型。

Conclusion: TableLLM在处理表格数据方面具有显著优势，并已公开模型检查点、源代码、基准测试和Web应用程序。
...
### [11] [FLAME: A small language model for spreadsheet formulas](https://arxiv.org/abs/2301.13779)
...
TL;DR: Trace2Skill框架通过分析大量执行经验来自动化生成LLM的领域技能，克服了手动编写和现有自动生成方法的局限性，显著提高了模型在电子表格、VisionQA和数学推理等任务上的表现，并且生成的技能具有良好的迁移性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 手动编写LLM的领域技能存在可扩展性瓶颈，而现有的自动化方法生成的技能往往脆弱或碎片化。

Method: Trace2Skill框架派遣并行子代理分析多样化的执行数据，提取特定轨迹的经验教训，并通过归纳推理将它们整合到一个统一的、无冲突的技能目录中。

Result: Trace2Skill在电子表格、VisionQA和数学推理等领域显著优于现有基线，并且生成的技能能够跨LLM规模迁移并泛化到未见过的设置。例如，在WikiTableQuestions上，由Qwen3.5-35B生成的技能将Qwen3.5-122B代理的性能提高了高达57.65个百分点。

Conclusion: 复杂代理的经验可以被打包成高度可迁移的、声明式的技能，而无需进行参数更新或使用外部检索模块，并且可以使用最小35B参数的开源模型来实现。
...
TL;DR: AIDABench 是一个全面的基准测试，用于评估人工智能系统在复杂数据分析任务中的端到端能力，现有模型在该基准测试上的表现仍有提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有的基准测试和评估往往侧重于孤立的能力或简化的场景，未能捕捉到实际应用中所需的端到端任务有效性。为了解决这一差距，我们引入了 AIDABench。

Method: AIDABench 包含 600 多个跨越问答、数据可视化和文件生成三个核心能力维度的文档分析任务。这些任务以真实场景为基础，涉及异构数据类型（如电子表格、数据库、财务报告和操作记录），并反映了不同行业和工作职能的分析需求。我们评估了 11 个最先进的模型，涵盖了专有模型（例如 Claude Sonnet 4.5、Gemini 3 Pro Preview）和开源模型（例如 Qwen3-Max-2026-01-23-Thinking）。

Result: 在 AIDABench 上，即使是表现最好的模型也只能达到 59.43% 的 pass-at-1 准确率，这表明复杂、真实的數據分析任务对当前的人工智能系统仍然是一个重大挑战。我们提供了跨每个能力维度的失败模式的详细分析，并指出了未来研究的关键挑战。

Conclusion: AIDABench 提供了一个原则性的参考，可用于企业采购、工具选择和模型优化。
...
TL;DR: AutoSAM是一个自动化框架，利用大型语言模型和检索增强生成技术，根据非结构化的工程文档自动生成先进反应堆模拟的输入文件。


<details>
  <summary>Details</summary>
Motivation: 手动创建先进反应堆系统模拟的输入文件（例如，使用SAM代码）是一项耗时且容易出错的任务，需要从各种工程文档中提取和转换数据。

Method: AutoSAM结合了大型语言模型代理、检索增强生成（基于用户指南和理论手册）以及PDF、图像、电子表格和文本文件的分析工具。它能够从包括系统图、设计报告和数据表在内的非结构化文档中提取参数，将其转换为中间表示，并生成经过验证的、兼容求解器的输入文件。其多模态检索流程集成了文本提取、基于视觉的图形解释、语义嵌入和查询应答。

Result: 在四个不同复杂程度的案例研究中（单管稳态模型、带温度反应性反馈的固体燃料通道、先进燃烧器测试反应堆堆芯、熔盐反应堆实验主回路），AutoSAM成功生成了可运行的SAM模型，这些模型与预期的热工水力行为一致。该框架实现了100%结构化输入的利用率，约88%的PDF文本提取率，以及100%的视觉几何提取完整性。

Conclusion: AutoSAM为通过提示驱动的反应堆建模提供了一条实际可行的途径，能够将分析人员提供的系统描述和支持文档转换为透明且可执行的SAM模拟。
...
TL;DR: LLMs在表格数据数值推理方面能力不足，本文提出Formula-R1模型，通过Formula Tuning（Fortune）框架，利用执行成功率和答案正确率作为奖励信号，训练LLMs生成可执行的电子表格公式，以提高表格推理能力，尤其在复杂表格和多步计算任务上表现优越。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs在处理复杂表格数据和进行精确数值推理方面存在不足，而电子表格公式提供了强大的符号操作能力，能够实现丰富的推理模式，这部分能力在现有LLMs中尚未得到充分探索。

Method: 提出Formula-R1模型，并采用Formula Tuning（Fortune）框架。该框架是一个面向表格推理的、由公式驱动的强化学习（RL）框架。Formula Tuning通过将生成可执行的电子表格公式用于通用表格数据问答，并使用执行成功率和答案正确率作为奖励信号来训练LLMs，从而减少对监督式公式标注的依赖。

Result: Formula Tuning在七个表格推理基准测试中显著提升了LLM在表格推理方面的性能，尤其是在涉及复杂表格和多步数值计算的任务上。Formula-R1在受控比较设置下持续优于现有方法。

Conclusion: Formula Tuning框架能够有效提升LLM在表格推理任务上的性能，尤其是在处理复杂表格和进行多步数值计算时。强化学习在公式驱动的表格推理中扮演着重要角色，并且Formula Tuning框架在表格推理方面展现了其广泛的应用潜力。
...
TL;DR: LLMs在处理表格数据方面取得了显著进展，但缺乏全面的基准测试来评估其在专业用户场景下的能力。本研究提出了MMTU，一个包含25个真实世界表格任务的大型基准，旨在全面评估模型理解、推理和操作表格的专家级能力。MMTU包含超过28000个问题，涵盖了表格理解、推理和编码等多种技能，这对当前前沿模型来说仍然是一个挑战。例如，GPT-5和DeepSeek R1的得分分别为69%和57%。该基准测试旨在推动在结构化数据处理和分析方面的基础模型的发展。


<details>
  <summary>Details</summary>
Motivation: 现有的自然语言处理（NLP）基准测试在不断扩展，但针对表格相关任务的评估却很少，并且只关注于少数几个任务，如自然语言到SQL（NL-to-SQL）和表格问答（Table-QA）。这忽略了专业用户面临的更广泛的真实世界任务，限制了我们对模型在该领域能力的理解和进步。

Method: 提出MMTU，一个包含25个真实世界表格任务的大型基准，拥有超过28000个问题，旨在全面评估模型在专家级别上理解、推理和操作真实表格的能力。这些任务源于计算机科学领域关于表格数据的数十年研究，并侧重于专业用户面临的复杂表格任务。

Result: MMTU基准测试显示，当前的前沿模型（如OpenAI GPT-5和DeepSeek R1）在MMTU上的得分分别为69%和57%，表明在表格理解、推理和编码等综合技能方面仍有很大的提升空间。

Conclusion: MMTU基准测试表明，处理真实世界表格任务需要模型具备综合运用表格理解、推理和编码等多种能力。当前的模型仍有很大的改进空间，MMTU的推出将有助于推动结构化数据处理和分析领域基础模型的发展。
...
TL;DR: The paper argues that AI should focus on modeling real-world entities and their relations, rather than just perceptions like pixels and words. It highlights the prevalence and value of relational data in companies and explains why relational learning hasn't become mainstream, suggesting steps for its future prominence.


<details>
  <summary>Details</summary>
Motivation: The current AI focus on modeling perceptions (pixels, words, phonemes) is misguided; the world should be modeled as entities with properties and relations. Valuable data in companies is often relational, not in the form studied by introductory ML.

Method: The paper analyzes why relational learning is not dominant, except in restricted cases, and outlines what is needed for its wider adoption.

Result: Relational learning, despite its potential and the prevalence of relational data, is not widely adopted.

Conclusion: Relational learning needs specific advancements to achieve widespread use and modeling of real-world entities and their relationships effectively.
...
### [19] [SheetAgent: Towards A Generalist Agent for Spreadsheet Reasoning and Manipulation via Large Language Models](https://arxiv.org/abs/2403.03636)
...
### [20] [Large Language Model for Table Processing: A Survey](https://arxiv.org/abs/2402.05121)
*Weizheng Lu,Jing Zhang,Ju Fan,Zihao Fu,Yueguo Chen,Xiaoyong Du*

Main category: cs.AI

TL;DR: 本文对表格处理的各项任务进行了全面的概述，重点关注大型语言模型（LLM）和视觉语言模型（VLM）在这些任务中的应用，并探讨了相关的训练技术、提示工程和面临的挑战。


<details>
  <summary>Details</summary>
Motivation: 自动化表格处理任务（如数据库查询、电子表格操作、网络表格问答和图像表格信息提取）可以带来显著的公共利益，因此吸引了学术界和工业界的兴趣。

Method: 本文对表格处理相关的任务进行了全面的概述，涵盖了用户场景和技术方面，包括传统的表格问答以及新兴的电子表格操作和表格数据分析领域。同时，总结了用于表格处理的LLM和VLM的训练技术，并讨论了提示工程，特别是LLM驱动的代理在各种表格相关任务中的应用。

Result: 对表格处理任务进行了全面的概述，总结了LLM和VLM的训练技术，并讨论了提示工程和LLM驱动的代理的应用。

Conclusion: 尽管LLM和VLM在表格处理方面取得了进展，但仍存在一些挑战，包括服务多样化用户输入和使用链式思考（chain-of-thought）进行慢速推理。
...
### [21] [FormaT5: Abstention and Examples for Conditional Table Formatting with Natural Language](https://arxiv.org/abs/2310.17306)
...
### [22] [Don't Treat the Symptom, Find the Cause! Efficient Artificial-Intelligence Methods for (Interactive) Debugging](https://arxiv.org/abs/2306.12850)
*Patrick Rodler*
...
### [23] [Benchmark Dataset Generation and Evaluation for Excel Formula Repair with LLMs](https://arxiv.org/abs/2508.11715)
...
TL;DR: 本论文提出了一个用于 Excel 公式修复的数据集构建方法和基线技术。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 在解释 Excel 公式错误方面有潜力，但自动修复语义运行时错误仍是未解决的问题，主要挑战在于缺乏高质量的训练和评估数据集。

Method: 通过一个包含少量精选种子样本的数据生成流程，结合 LLM 的少样本提示和 LLM-as-a-Judge 验证框架，并辅以基于执行的检查，来构建 Excel 公式修复基准数据集。此外，还提出了一种利用 LLM 结合错误公式和相关电子表格上下文的上下文感知基线修复技术。

Result: 生成了一个包含 618 个高质量样本的数据集，涵盖了常见的运行时错误。通过对该数据集进行手动注释和基于执行的指标评估，展示了数据集的质量，并分析了错误和函数分布。

Conclusion: 提出的数据集构建方法是可扩展的，并可应用于其他低资源编程语言的代码修复任务。
...
### [24] [Overcoming Vision Language Model Challenges in Diagram Understanding: A Proof-of-Concept with XML-Driven Large Language Models Solutions](https://arxiv.org/abs/2502.04389)
...
### [25] [On the Workflows and Smells of Leaderboard Operations (LBOps): An Exploratory Study of Foundation Model Leaderboards](https://arxiv.org/abs/2407.04065)
...
### [30] [A Use Case-Engineering Resources Taxonomy for Analytical Spreadsheet Models](https://arxiv.org/abs/2309.00104)
...
### [32] [Structured Spreadsheet Modelling and Implementation with Multiple Dimensions -- Part 2: Implementation](https://arxiv.org/abs/1909.00891)
*Paul Mireault*
...
TL;DR: InvisibleMentor通过分析屏幕录像，主动识别并提出更优的Excel操作流程建议，无需用户主动描述问题。


<details>
  <summary>Details</summary>
Motivation: 现有AI助手需要用户主动描述问题，效率低下且不精确。当用户在使用Excel等功能丰富的工具时，往往难以发现更高效的工作流程。

Method: InvisibleMentor将屏幕录制内容转化为对任务的、与视觉信息相关的反思。它通过一个两阶段的流程：首先，使用视觉-语言模型重建操作和上下文；然后，利用语言模型生成结构化、高保真的建议。该系统直接在屏幕录像上操作，不依赖日志、API或用户提示。

Result: 实验评估表明，InvisibleMentor能够准确识别低效的工作流程。用户认为其建议比基于提示的电子表格助手更具可操作性、更具针对性，并更有助于学习和改进。

Conclusion: InvisibleMentor能够主动地发现并建议更高效的工作流程，解决了现有AI助手在用户难以发现自身问题时的局限性。
...
### [35] [Prompting in the Dark: Assessing Human Performance in Prompt Engineering for Data Labeling When Gold Labels Are Absent](https://arxiv.org/abs/2502.11267)
...
### [36] [When Copilot Becomes Autopilot: Generative AI's Critical Risk to Knowledge Work and a Critical Solution](https://arxiv.org/abs/2412.15030)
...
### [37] [Understanding and Evaluating Trust in Generative AI and Large Language Models for Spreadsheets](https://arxiv.org/abs/2412.14062)
*Simon Thorne*
...
### [38] [Dynamic Prompt Middleware: Contextual Prompt Refinement Controls for Comprehension Tasks](https://arxiv.org/abs/2412.02357)
...
### [39] [Exploring Higher Education Competencies through Spreadsheet Self-Assessment and Time](https://arxiv.org/abs/2409.12974)
...
### [40] ["My toxic trait is thinking I'll remember this": gaps in the learner experience of video tutorials for feature-rich software](https://arxiv.org/abs/2404.07114)
...
### [41] [Supporting Annotators with Affordances for Efficiently Labeling Conversational Data](https://arxiv.org/abs/2403.07762)
...
### [42] [Prompting for Discovery: Flexible Sense-Making for AI Art-Making with Dreamsheets](https://arxiv.org/abs/2310.09985)
...
### [43] [Does Using ChatGPT Result in Human Cognitive Augmentation?](https://arxiv.org/abs/2401.11042)
...
<details>
  <summary>Details</summary>
Motivation: Error code: 429 - [{'error': {'code': 429, 'message': 'You exceeded your current quota, please check your plan and billing details. For more information on this error, head to: https://ai.google.dev/gemini-api/docs/rate-limits. To monitor your current usage, head to: https://ai.dev/rate-limit. \n* Quota exceeded for metric: generativelanguage.googleapis.com/generate_content_free_tier_requests, limit: 20, model: gemini-2.5-flash-lite\nPlease retry in 10.386618503s.', 'status': 'RESOURCE_EXHAUSTED', 'details': [{'@type': 'type.googleapis.com/google.rpc.Help', 'links': [{'description': 'Learn more about Gemini API quotas', 'url': 'https://ai.google.dev/gemini-api/docs/rate-limits'}]}, {'@type': 'type.googleapis.com/google.rpc.QuotaFailure', 'violations': [{'quotaMetric': 'generativelanguage.googleapis.com/generate_content_free_tier_requests', 'quotaId': 'GenerateRequestsPerDayPerProjectPerModel-FreeTier', 'quotaDimensions': {'model': 'gemini-2.5-flash-lite', 'location': 'global'}, 'quotaValue': '20'}]}, {'@type': 'type.googleapis.com/google.rpc.RetryInfo', 'retryDelay': '10s'}]}}]
...
### [44] ["What It Wants Me To Say": Bridging the Abstraction Gap Between End-User Programmers and Code-Generating Large Language Models](https://arxiv.org/abs/2304.06597)
...
### [45] [Are digital natives spreadsheet natives?](https://arxiv.org/abs/1909.00865)
...
### [46] [A Simpler Method for Understanding Emergency Shelter Access Patterns](https://arxiv.org/abs/2210.13619)
*Geoffrey G. Messier*
...
### [47] [A Case Study of Spreadsheet Use within the Finance and Academic Registry units within a Higher Education Institution](https://arxiv.org/abs/1909.07462)
...
### [48] [GeoMCP: A Trustworthy Framework for AI-Assisted Analytical Geotechnical Engineering](https://arxiv.org/abs/2603.01022)
*Yared W. Bekele*

Main category: cs.CE

TL;DR: GeoMCP是一个框架，它将岩土工程方法表示为结构化数据（“方法卡”），并通过约束符号引擎和AI驱动的Agent Skills来执行，以弥合LLM在安全关键计算中的不确定性与工程实践的确定性之间的差距。


<details>
  <summary>Details</summary>
Motivation: 目前的岩土工程分析方法分散在易出错的电子表格和不透明的专有软件中。虽然LLM有潜力简化工作流程，但它们固有的统计性质与安全关键计算所需的严格确定性存在冲突，并且存在幻觉、单位误解和方法变化等问题，导致信任差距。

Method: GeoMCP框架将分析方法表示为结构化数据（“方法卡”），这些是定义公式、单位、适用性限制和文献引用的声明式JSON文件。一个约束符号引擎执行这些方法卡，并进行验证，确保量纲一致性。结构化的“Agent Skills”指导LLM应用工程判断和协调分析。通过模型上下文协议（MCP）公开这些经过验证的能力。

Result: 该框架在计算上与传统方法相当，同时确保了完全的数学透明度。它已通过一个官方JRC Eurocode 7的示例进行了验证。

Conclusion: GeoMCP提供了一个蓝图，用于将该行业从孤立的遗留软件过渡到一个可互操作的、为AI做准备的生态系统，使工程师能够利用现代AI，同时不放弃专业责任。
...
### [49] [Billion-files File Systems (BfFS): A Comparison](https://arxiv.org/abs/2408.01805)
*Sohail Shaikh*
...
### [50] [Reducing Aleatoric and Epistemic Uncertainty through Multi-modal Data Acquisition](https://arxiv.org/abs/2501.18268)
*Arthur Hoarau,Benjamin Quost,Sébastien Destercke,Willem Waegeman*

Main category: cs.LG

TL;DR: 多模态数据可用于减少不确定性，并且可以通过增加数据模态和样本量来进一步优化。


<details>
  <summary>Details</summary>
Motivation: 多模态数据为分离不确定性带来了机遇和挑战。本研究提出的数据采集框架旨在解决这一问题。

Method: 提出了一种创新的数据采集框架，该框架通过增加样本量和数据模态来分离不确定性，并结合了主动学习、主动特征采集和不确定性量化。

Result: 在两个多模态数据集上进行了概念验证，展示了所提出的数据采集框架的有效性。

Conclusion: 本研究提出了一个创新的数据采集框架，通过探索样本量和数据模态两个方向来分离不确定性，并验证了其在多模态数据集上的有效性。
...
### [51] [Oh That Looks Familiar: A Novel Similarity Measure for Spreadsheet Template Discovery](https://arxiv.org/abs/2511.06973)
*Anand Krishnakumar,Vengadesh Ravikumaran*

Main category: cs.LG

TL;DR: 传统方法无法捕捉电子表格的布局和类型模式，我们提出了一种结合语义嵌入、数据类型和空间定位的混合距离度量方法，以量化电子表格相似性。


<details>
  <summary>Details</summary>
Motivation: 需要一种能够捕捉电子表格的空间布局和类型模式的相似性量化方法。

Method: 将电子表格转换为单元格级别的嵌入，并使用 Chamfer 和 Hausdorff 距离等聚合技术来计算电子表格相似性。

Result: 在 FUSTE 数据集上，该方法实现了优于基于图的 Mondrian 基线的无监督聚类性能，并实现了完美的模板重建（调整兰德指数为 1.00，优于 0.90）。

Conclusion: 所提出的方法能够进行大规模的自动化模板发现，从而支持检索增强生成、模型训练和批量数据清理等下游应用。
...
### [52] [A Conditional GAN for Tabular Data Generation with Probabilistic Sampling of Latent Subspaces](https://arxiv.org/abs/2508.00472)
*Leonidas Akritidis,Panayiotis Bozanis*

Main category: cs.LG

TL;DR: ctdGAN是一种用于处理表格数据类不平衡问题的条件生成对抗网络（GAN）。它通过空间划分、概率采样和新的损失函数来生成高保真度样本，并提高分类准确性。


<details>
  <summary>Details</summary>
Motivation: 表格数据存在类别不平衡问题，影响机器学习任务性能。现有GAN模型未考虑输入样本的向量子空间，导致生成数据位置随意，且类别标签与其他类别变量同等对待，降低了条件采样效果。

Method: ctdGAN首先执行空间划分，为输入样本分配聚类标签。然后，利用这些标签通过新颖的概率采样策略和惩罚聚类及类别误预测的新损失函数来合成样本。此外，还引入了聚类感知缩放技术。

Result: ctdGAN在14个不平衡数据集上的广泛评估证明，它在生成高保真度样本和提高分类准确性方面优于现有方法。

Conclusion: ctdGAN通过考虑向量子空间和类别信息，有效地解决了表格数据类别不平衡问题，提高了生成样本的质量和分类器的性能。
...
### [55] [Real-time stock analysis for blending recipes in industrial plants](https://arxiv.org/abs/1909.02960)
...
### [56] [Ensuring Adherence to Standards in Experiment-Related Metadata Entered Via Spreadsheets](https://arxiv.org/abs/2409.08897)
...
TL;DR: 科学家们倾向于使用电子表格输入元数据，但电子表格在确保元数据的一致性和合规性方面存在局限性。本文提出了一种端到端的方法，支持基于电子表格的元数据输入，同时确保严格遵守社区元数据标准并提供质量控制。


<details>
  <summary>Details</summary>
Motivation: 科学家们越来越认识到提供丰富的、符合标准的元数据来描述实验结果的重要性，但研究人员普遍偏好使用电子表格，尽管电子表格在确保元数据一致性和符合正式规范方面存在局限性。

Method: 该方法包括：1. 可定制的模板，代表元数据标准并告知研究人员使用的电子表格；2. 受控术语和本体，用于定义可直接从电子表格访问的元数据值；3. 一个交互式 Web 工具，允许用户快速识别和修复电子表格元数据中的错误。

Result: 该方法已被部署在 HuBMAP 生物医学联盟中，用于定义和收集各种生物学检测的元数据。

Conclusion: 该方法通过支持电子表格输入、确保元数据标准合规性和提供质量控制，解决了元数据输入的一致性和合规性问题。
...
### [58] [A Comprehensive Approach to Ensuring Quality in Spreadsheet-Based Metadata](https://arxiv.org/abs/2312.09107)
...
### [59] [Data-Semantics-Aware Recommendation of Diverse Pivot Tables](https://arxiv.org/abs/2507.06171)
*Whanhee Cho,Anna Fariha*

Main category: cs.DB

TL;DR: SAGE是一个用于推荐多样化透视表的数据语义感知系统，解决了现有方法在处理高维数据集时的不足。


<details>
  <summary>Details</summary>
Motivation: 手动创建有用的透视表，尤其是在处理高维数据集时，仍然是一个挑战。现有的工作未能充分解决透视表多样化的问题，导致推荐结果冗余。

Method: SAGE利用数据语义来衡量单个透视表的效用和一组透视表的多样性，并通过一个可扩展的贪婪算法来高效地选择一组具有高效用且多样化的透视表。

Result: SAGE在三个真实世界数据集上的实验表明，其性能优于其他方法，并且能够有效地处理高维数据集。此外，SAGE在定性方面也优于商业软件和大型语言模型。

Conclusion: SAGE能够推荐有用、可解释且多样化的透视表，解决了现有方法的局限性，并能在实际应用中有效地处理高维数据集。
...
### [60] [Tabularis Formatus: Predictive Formatting for Tables](https://arxiv.org/abs/2508.11121)
*Mukul Singh,José Cambronero,Sumit Gulwani,Vu Le,Gust Verbruggen*

Main category: cs.DB

TL;DR: TaFo是一个利用神经符号学方法为表格生成条件格式化（CF）建议的系统，能够自动学习规则触发器和视觉格式化属性，并能生成比现有系统更准确、更多样化和更完整的格式化建议。


<details>
  <summary>Details</summary>
Motivation: 现有的电子表格软件虽然广泛用于数据管理和分析，但创建条件格式化（CF）规则仍然是一个复杂且需要专业知识的任务，并且用户往往不知道如何创建或现有用户界面不佳。TaFo旨在解决这些常见挑战。

Method: TaFo采用一种神经符号学方法，借鉴了基于组件的合成系统，并结合了语言模型的语义知识和一种保持多样性的规则排序机制。它能够自动学习规则触发器和相关的视觉格式化属性，实现完全预测性和自动化的格式化。

Result: 通过对180万个包含CF和手动格式化的公开工作簿进行评估，TaFo在匹配用户添加的真实规则方面，比包括符号和神经系统在内的现有系统准确率提高了15.6%-26.5%，并且生成的格式化建议更加准确、多样和完整。

Conclusion: TaFo通过其神经符号学方法，能够自动学习并生成比现有方法更优越的条件格式化建议，解决了用户在创建CF规则时面临的复杂性和用户界面问题。
...
### [61] [A System and Benchmark for LLM-based Q&A on Heterogeneous Data](https://arxiv.org/abs/2409.05735)
...
TL;DR: siwarex平台可以无缝地访问数据库和API，解决了现实工业环境中数据源异构性问题。


<details>
  <summary>Details</summary>
Motivation: 用户希望能够直接查询结构化数据（如电子表格、数据库、API等）以获取答案，但往往面临不知道如何识别、访问正确的数据源，甚至需要整合多个数据源的挑战。现有的Text-to-SQL应用在处理工业环境中的数据源异构性方面存在不足。
...
Conclusion: siwarex平台为解决工业环境中数据源异构性问题提供了一个有效的解决方案，并将在近期发布修改后的Spider基准测试。
...
### [62] [Auditable and reusable crosswalks for fast, scaled integration of scattered tabular data](https://arxiv.org/abs/2409.01517)
*Gavin Chait*
...
### [63] [Facilitating Digital Agriculture with Simple Databases](https://arxiv.org/abs/2312.06517)
...
### [64] [Streamlining Knowledge Graph Construction with a façade: The SPARQL Anything project](https://arxiv.org/abs/2310.16700)
...
<details>
  <summary>Details</summary>
Motivation: Error code: 429 - [{'error': {'code': 429, 'message': 'You exceeded your current quota, please check your plan and billing details. For more information on this error, head to: https://ai.google.dev/gemini-api/docs/rate-limits. To monitor your current usage, head to: https://ai.dev/rate-limit. \n* Quota exceeded for metric: generativelanguage.googleapis.com/generate_content_free_tier_requests, limit: 20, model: gemini-2.5-flash-lite\nPlease retry in 17.960872687s.', 'status': 'RESOURCE_EXHAUSTED', 'details': [{'@type': 'type.googleapis.com/google.rpc.Help', 'links': [{'description': 'Learn more about Gemini API quotas', 'url': 'https://ai.google.dev/gemini-api/docs/rate-limits'}]}, {'@type': 'type.googleapis.com/google.rpc.QuotaFailure', 'violations': [{'quotaMetric': 'generativelanguage.googleapis.com/generate_content_free_tier_requests', 'quotaId': 'GenerateRequestsPerDayPerProjectPerModel-FreeTier', 'quotaDimensions': {'model': 'gemini-2.5-flash-lite', 'location': 'global'}, 'quotaValue': '20'}]}, {'@type': 'type.googleapis.com/google.rpc.RetryInfo', 'retryDelay': '17s'}]}}]
...
### [65] [DataVinci: Learning Syntactic and Semantic String Repairs](https://arxiv.org/abs/2308.10922)
...
### [66] [Auto-Tables: Synthesizing Multi-Step Transformations to Relationalize Tables without Using Examples](https://arxiv.org/abs/2307.14565)
...
<details>
  <summary>Details</summary>
Motivation: Error code: 429 - [{'error': {'code': 429, 'message': 'You exceeded your current quota, please check your plan and billing details. For more information on this error, head to: https://ai.google.dev/gemini-api/docs/rate-limits. To monitor your current usage, head to: https://ai.dev/rate-limit. \n* Quota exceeded for metric: generativelanguage.googleapis.com/generate_content_free_tier_requests, limit: 20, model: gemini-2.5-flash-lite\nPlease retry in 24.415787687s.', 'status': 'RESOURCE_EXHAUSTED', 'details': [{'@type': 'type.googleapis.com/google.rpc.Help', 'links': [{'description': 'Learn more about Gemini API quotas', 'url': 'https://ai.google.dev/gemini-api/docs/rate-limits'}]}, {'@type': 'type.googleapis.com/google.rpc.QuotaFailure', 'violations': [{'quotaMetric': 'generativelanguage.googleapis.com/generate_content_free_tier_requests', 'quotaId': 'GenerateRequestsPerDayPerProjectPerModel-FreeTier', 'quotaDimensions': {'location': 'global', 'model': 'gemini-2.5-flash-lite'}, 'quotaValue': '20'}]}, {'@type': 'type.googleapis.com/google.rpc.RetryInfo', 'retryDelay': '24s'}]}}]
...
<details>
  <summary>Details</summary>
Motivation: Error code: 429 - [{'error': {'code': 429, 'message': 'You exceeded your current quota, please check your plan and billing details. For more information on this error, head to: https://ai.google.dev/gemini-api/docs/rate-limits. To monitor your current usage, head to: https://ai.dev/rate-limit. \n* Quota exceeded for metric: generativelanguage.googleapis.com/generate_content_free_tier_requests, limit: 20, model: gemini-2.5-flash-lite\nPlease retry in 16.944063866s.', 'status': 'RESOURCE_EXHAUSTED', 'details': [{'@type': 'type.googleapis.com/google.rpc.Help', 'links': [{'description': 'Learn more about Gemini API quotas', 'url': 'https://ai.google.dev/gemini-api/docs/rate-limits'}]}, {'@type': 'type.googleapis.com/google.rpc.QuotaFailure', 'violations': [{'quotaMetric': 'generativelanguage.googleapis.com/generate_content_free_tier_requests', 'quotaId': 'GenerateRequestsPerDayPerProjectPerModel-FreeTier', 'quotaDimensions': {'model': 'gemini-2.5-flash-lite', 'location': 'global'}, 'quotaValue': '20'}]}, {'@type': 'type.googleapis.com/google.rpc.RetryInfo', 'retryDelay': '16s'}]}}]
...
### [69] [Using a Catenary Trajectory to Reduce Wellbore Friction in Horizontal Extended Reach Drilling](https://arxiv.org/abs/2309.12317)
*Vu Nguyen*
...
<details>
  <summary>Details</summary>
Motivation: 介绍一个完整的伊朗非节拍radif音乐数字语料库，以支持计算音乐学研究。

Method: 构建了一个包含228首音乐的数字语料库，涵盖了伊朗古典音乐的13个组成部分，并提供了MIDI文件（约281分钟）和详细的数据表格，包括音符、时长、音程和层次结构，忠实地呈现了包含四分音符的调性和非节拍特性。

Result: 创建了一个数字语料库，包含228首音乐的MIDI文件和数据，并提供了基础统计数据、复杂度和相似性度量。

Conclusion: 该语料库为伊朗古典音乐的计算研究提供了一个平台，可用于研究旋律模式、即兴风格以及在音乐信息检索、音乐理论和计算（民族）音乐学等领域的应用。
...
### [71] [A costing framework for fusion power plants](https://arxiv.org/abs/2601.21724)
*Simon Woodruff*
...
<details>
  <summary>Details</summary>
Motivation: 为ARPA-E的聚变发电厂项目提供成本核算能力，记录其从早期资本成本估算到标准化、可审计的成本核算能力的演变。

Method: 论文首先回顾了早期基于ARIES的成本缩放关系估算，并与Bechtel和Decysive Systems合作进行试点研究以验证BOP成本和工厂级合理性。随后，借鉴Lucid Catalyst的研究，将间接成本明确化，并通过设计优化、模块化、集中制造和学习效应来评估成本降低途径。随着ARPA-E项目组合的扩展，这些方法被应用于BETHE和GAMOW概念。2023年，该能力被重构以符合IAEA-GEN-IV EMWG-EPRI的账户体系，并用自下而上的子系统模型替代了关键的ARIES缩放关系，这些模型与物理约束的功率平衡和径向构建相结合。最终开发了基于电子表格的Fusion Economics代码（FECONs）和开源Python框架（pyFECONs）。

Result: 开发了一个名为FECONs的电子表格工具和名为pyFECONs的开源Python框架，用于聚变发电厂的成本核算。该框架能够清晰地将子系统估算映射到标准化账户，并一致地计算LCOE。

Conclusion: 该分析框架的演变历程表明，从早期以资本成本为中心的估算方法，发展到如今符合国际标准、可审计、开源的成本核算能力，是聚变发电厂经济性研究的重要进展。
