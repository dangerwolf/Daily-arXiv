...
TL;DR: The paper introduces Toolathlon, a new benchmark for evaluating language agents on complex, multi-step workflows across diverse real-world Apps, featuring 32 applications and 604 tools with realistic environment setup and execution-based evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing language agent benchmarks lack the diversity, realism, and long-horizon complexity required to evaluate agents' real-world performance.

Method: The authors created Toolathlon, a benchmark with 108 tasks requiring interaction with multiple Apps over around 20 turns, using realistic initial environment states from real software and strictly verifiable evaluation scripts.

Result: The best-performing model, Claude-4.5-Sonnet, achieves only a 38.6% success rate, and the top open-weights model DeepSeek-V3.2-Exp reaches 20.1%, highlighting significant shortcomings in current SOTA models.

Conclusion: Toolathlon is expected to drive the development of more capable language agents for real-world, long-horizon task execution.
...
TL;DR: This paper introduces a new benchmark dataset for Excel formula repair, addressing the lack of high-quality data for training and evaluating models in correcting semantic runtime errors. They propose a data generation pipeline using LLMs and a validation framework to create a dataset of 618 samples and evaluate several LLMs on it.


<details>
  <summary>Details</summary>
Motivation: The motivation is the lack of high-quality datasets for training and evaluating models for automated correction of semantic runtime errors in Excel formulas, which is a significant challenge, especially for novice users.

Method: The method involves a data generation pipeline that leverages a small set of curated seed samples from online forums to synthetically expand the dataset. This pipeline integrates few-shot prompting with LLMs and employs a robust LLM-as-a-Judge validation framework, combined with execution-based checks. They also propose a context-aware baseline technique for Excel formula repair that utilizes LLMs to leverage both the faulty formula and relevant spreadsheet context.

Result: The process produced a benchmark dataset of 618 high-quality samples, covering common runtime errors. The performance of various LLMs (GPT-4o, GPT-4.1, Phi-3, Mistral) was evaluated on the generated benchmark using execution-based metrics.

Conclusion: The paper demonstrates the dataset's quality through manual annotation and provides insights into error and function distributions. The proposed generation methodology is highly scalable and can be readily adapted to create evaluation benchmarks for similar code repair tasks in other low-resource programming languages.
