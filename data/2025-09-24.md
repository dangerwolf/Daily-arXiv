...
Motivation: 论文旨在描述一个DICOM数据去识别工作流程，该流程使用XNAT平台管理DICOM图像数据库，并解决研究项目中数据去识别的需求。

Method: 论文采用基于规则的方法结合XNAT及其生态系统工具进行DICOM数据去识别。在地址处理方面，初步尝试了机器学习模型。

Result: 在MIDI-B挑战中，初始得分97.91%（因技术不兼容），后通过反馈提高到99.61%。纯规则方法能有效去除姓名信息，但在地址数据处理上存在不足。机器学习模型在地址处理上部分成功，但对其他自由文本数据“过度激进”，导致整体性能略降至99.54%。目前估计的真实去识别失败率为0.19%。

Conclusion: 基于规则的方法在某些去识别任务中表现良好，但处理地址数据和图像像素中烧录的可识别数据仍是挑战。未来的工作将专注于提高地址识别能力和更好地移除图像像素中的可识别数据。
...
<details>
  <summary>Details</summary>
Motivation: 评估和提升视觉语言模型对电子表格数据的理解能力，特别是在OCR、空间感知和视觉格式识别方面。

Method: 提出了三个自监督挑战和评估指标来评估VLMs在OCR、空间感知和视觉格式识别方面的能力；利用电子表格表格检测任务综合评估VLMs性能；提出了三种电子表格到图像的设置（列宽调整、样式更改、地址增强）以及不同的提示变体；在电子表格边界检测中，通过解码表格四个边界上的单元格值来利用VLMs的文本理解能力。

Result: VLMs在OCR能力方面表现出前景，但由于单元格遗漏和错位导致结果不尽如人意；VLMs在空间和格式识别技能方面明显不足。

Conclusion: 视觉语言模型在电子表格理解方面仍有提升空间，特别是在解决单元格遗漏、错位以及增强空间和格式识别能力方面。未来工作可利用本文方法生成大量不同设置下的电子表格-图像对以增强VLMs的电子表格数据理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 以往的高通量植物表型研究常缺乏带有物理标签的数据集，阻碍了图像数据与实验条件的直接关联。本研究旨在通过使用带有物理标签的数据集来克服这一挑战。

Method: 研究使用了一个包含1,672张毛果杨图像的数据集，图像带有显示处理、区块、行、位置和基因型的物理白标签。利用OCR读取这些标签，结合图像分割技术和机器学习算法进行形态分类（叶片形状、颜色、褐斑程度），并使用机器学习模型根据这些分类预测处理方式。此外，还分析了编码的EXIF标签以寻找叶片大小和表型之间的相关性。

Result: OCR模型在非空文本提取方面达到了94.31%的准确率。分类模型在识别叶片形状、颜色和褐斑程度方面平均准确率为62.82%。植物处理预测准确率为60.08%。然而，EXIF标签中缺少关键信息，导致无法评估叶片大小以及表型与条件之间的相关性。

Conclusion: 本研究成功将OCR和机器学习应用于带有物理标签图像的高通量表型分析，展示了OCR的高准确性和分类的合理准确性。尽管EXIF数据缺失带来局限，但为未来改进叶片大小和表型相关性评估提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 大型预训练模型（如基础模型）因训练数据领域不同而展现出不同的能力，导致它们存储着不同形式的常识知识。本研究旨在证明这种多样性是共生的，并且可以被有效利用。

Method: 提出 Socratic Models (SMs)，这是一个模块化框架，通过多模态提示零样本组合多个预训练模型，使其相互交换信息并捕获新的多模态能力，而无需进行微调。

Result: SMs 不仅在零样本图像字幕和视频到文本检索方面与最先进的技术具有竞争力，还能够实现新的应用，例如回答关于以自我为中心的视频的自由形式问题、与人进行多模态辅助对话（例如烹饪食谱），以及机器人感知和规划。

Conclusion: 不同预训练模型中知识的多样性是共生的，Socratic Models 能够通过零样本多模态提示有效地利用这种多样性，实现先进的多模态能力和新的应用。
...
<details>
  <summary>Details</summary>
Motivation: 表格是存储数据的一种常见结构，存在于PDF、图像、电子表格和CSV等多种格式中。能够解析表格结构并从中提取内容对于许多应用都至关重要。

Method: 本文设计了TableParser系统，能够高精度地解析原生PDF和扫描图像中的表格。研究通过大量实验展示了领域适应在此类工具开发中的有效性。此外，还创建了TableAnnotator和ExcelAnnotator，它们构成了基于电子表格的弱监督机制和实现表格解析的管道。

Result: TableParser系统能够高精度地解析原生PDF和扫描图像中的表格。广泛的实验证明了领域适应在该工具开发中的有效性。同时，创建了TableAnnotator和ExcelAnnotator并与研究社区共享，以促进该方向的进一步研究。

Conclusion: 本文成功开发了TableParser系统，用于高精度地解析来自PDF和扫描图像的表格，并强调了领域适应和弱监督方法的重要性。所提供的工具和资源有望推动相关领域的未来研究。
...
TL;DR: 本文提出使用电子表格公式解决表格问答（TableQA）问题，构建了FormulaQA数据集并提出了TabAF框架，在多个基准测试中取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM驱动的表格问答方法在处理特定问题类型或表格结构时缺乏通用性。而电子表格公式作为表格数据的操作语言，尚未被充分探索用于解决TableQA。

Method: 1. 构建了FormulaQA，一个大型的、带公式标注的TableQA数据集。2. 提出了TabAF，一个通用的表格问答框架，使用单个LLM主干同时解码答案和公式，以解决多种类型表格上的多种任务。

Result: 大量实验证明了TabAF的通用性和泛化能力。在相同模型尺寸下，TabAF在WikiTableQuestion、HiTab和TabFact上达到了新的SOTA性能。

Conclusion: 本文成功地展示了通过FormulaQA数据集和TabAF框架，使用电子表格公式解决TableQA中复杂推理任务的有效性和通用性，并取得了最先进的结果。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在帮助编写电子表格公式方面潜力巨大，但由于公式相关资源稀缺，影响了预训练模型的性能和微调能力。此外，使用LLM生成合成自然语言（NL）用于微调时，验证其准确性至关重要。

Method: 给定一个公式语料库，使用模型生成合成的自然语言语句进行微调。通过代理目标（评估合成标注的准确性）验证这些合成训练示例，并提供经验结果来衡量其影响。

Result: 验证过程在四种模型（2个开源，2个闭源）上均能提高性能，优于使用原始数据。有趣的是，虽然验证会剔除更具挑战性的示例，但经过验证数据微调后的模型能解决更复杂的问题。

Conclusion: 验证LLM生成的合成训练示例对于提高大型语言模型在电子表格公式编写任务上的性能至关重要，即使它会剔除一些复杂的原始数据，但最终能让模型处理更复杂的场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有的通用和专注于表格数据的LLM在处理现实世界办公场景中嵌入在文档或电子表格中的表格数据操作任务时可能存在不足，需要一个更鲁棒的模型来解决这些挑战。

Method: 提出了一种远程监督训练方法，包括推理过程扩展策略（帮助LLM更有效地理解推理模式）和交叉验证策略（确保自动生成数据的质量）。为了评估性能，还针对文档和电子表格格式构建了定制的基准测试和评估流程。

Result: 全面的评估结果表明，与各种现有的通用和专注于表格数据的LLM相比，TableLLM在处理表格数据操作任务方面具有显著优势。

Conclusion: TableLLM在处理表格数据操作方面表现出色，其模型检查点、源代码、基准和Web应用程序均已公开发布，方便用户使用和互动。
...
<details>
  <summary>Details</summary>
Motivation: GiesKaNe项目旨在创建一个历史性、句法深度标注的树库，连接历史和当代语料库。本文探讨了该项目语料库编译的需求，以平衡创新与标准，满足项目内部目标和更广泛的研究兴趣。

Method: 项目通过人类专业知识和机器辅助过程管理方法复杂性。它涵盖了分词、标准化、句子定义、标注、句法分析和标注者间一致性等基础主题。还提出了机器辅助文本分类的新方法，以及从现有标注中推导事实标准标注的方法。工作流程利用现有研究基础设施，特别是简单的电子表格，无需专用标注工具。

Result: 本文展示了GiesKaNe这样雄心勃勃的项目可以使用现有研究基础设施（如简单电子表格）有效实施，无需专门的标注工具。它还提出了机器辅助文本分类和推导事实标准标注的新方法。

Conclusion: 像GiesKaNe这样雄心勃勃的历史性、深度标注参考语料库项目，可以通过平衡创新与标准、结合人机协作以及利用现有研究基础设施来成功实施。提出的新方法有助于文本选择和标注标准化。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型（LLMs）表格推理基准与实际应用中表格和问题的复杂性和多样性之间存在显著差距。

Method: 提出了MiMoTable基准，其特点是包含来自七个领域的真实世界电子表格，并定义了六类元操作的新标准来衡量问题的难度，同时用此标准评估现有基准的难度。

Result: Claude-3.5-Sonnet在MiMoTable上取得了77.4%的最佳准确率，表明LLMs仍有很大改进空间。实验还证明，LLMs的性能随基准难度的增加而下降，证实了新标准的有效性。

Conclusion: MiMoTable是一个具有真实世界复杂性的新挑战性基准，其提出的新难度标准能有效衡量LLMs在表格推理方面的表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基准测试依赖于合成查询和简化电子表格，未能反映真实世界中电子表格用户的复杂需求。因此，需要一个基于真实世界场景的基准测试来更准确地评估LLM在电子表格操作方面的能力。

Method: 研究人员引入了SpreadsheetBench，该基准测试包含912个从在线Excel论坛收集的真实问题，反映了用户复杂的实际需求。相关的电子表格包含多种表格数据（如多个表格、非标准关系表格和丰富的非文本元素）。此外，论文提出了一种类似于在线判题平台的更可靠的评估指标，为每条指令创建多个电子表格文件作为测试用例，以确保评估解决方案在处理不同值电子表格时的鲁棒性。

Result: 对各种LLM在单轮和多轮推理设置下进行的综合评估显示，最先进的模型与人类性能之间存在巨大差距，这突显了该基准测试的难度。

Conclusion: SpreadsheetBench是一个具有挑战性且现实的基准测试，它表明当前LLM在真实的电子表格操作方面与人类表现相去甚远。
...
<details>
  <summary>Details</summary>
Motivation: 在电子表格中编写公式对许多终端用户来说是一项繁琐且容易出错的任务，尤其是在处理复杂操作时。

Method: 引入了一个名为NL2Formula的新型基准任务，旨在根据自然语言(NL)查询生成可执行的、基于电子表格的公式。构建了一个包含70,799个配对的NL查询和对应电子表格公式的综合数据集，涵盖21,670个表格和37种公式函数。通过提供一个名为fCoder的序列到序列基线实现来完成NL2Formula任务。

Result: fCoder的实验结果验证了其有效性，证明其性能优于基线模型。此外，还将fCoder与初始的GPT-3.5模型进行了比较。通过深入的错误分析，识别了NL2Formula任务中潜在的挑战。

Conclusion: fCoder验证了NL2Formula任务的有效性，并识别了该任务中的潜在挑战，倡导进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLMs）的发展，它们可以解决包括电子表格在内的各个领域中日益复杂的自然语言处理（NLP）任务。本文旨在探究LLMs是否能够根据自然语言用户指令，生成解决Excel特定任务的代码（Excel OfficeScripts）。

Method: 研究人员引入了一个名为InstructExcel的新的大规模基准，该基准通过利用Excel的“自动化”功能，从用户操作中自动生成OfficeScripts来创建。该基准包含超过1万个样本，涵盖2,000个公开可用的Excel电子表格中的170多种Excel操作。实验在各种零样本和少样本设置下进行。

Result: 实验结果表明，InstructExcel对于像GPT-4这样的最先进模型来说是一个困难的基准。研究观察到：(1) 使用GPT-4而非GPT-3.5，(2) 提供更多上下文示例，以及 (3) 动态提示可以帮助提高在该基准上的性能。

Conclusion: 大型语言模型在根据自然语言指令生成Excel OfficeScripts代码方面具有潜力，但InstructExcel基准测试证明这仍是一个具有挑战性的任务。未来可以通过改进模型、增加上下文示例和使用动态提示等方法来提高LLMs在这类任务上的表现。
...
<details>
  <summary>Details</summary>
Motivation: 在工业中，表格数据标签获取成本高昂，因为只有专家才能标注专业性强的表格。主动学习有潜力降低标注成本，但目前还没有将主动学习与表格语言模型结合的研究。

Method: 本文在一个真实的工业表格语言模型子单元命名实体识别用例中，研究了不同的获取函数。

Result: 结果表明，内置多样性的单元级获取函数可以显著减少标注工作量，而强制的表格多样性则是有害的。此外，计算效率和人类标注员的视角仍存在开放的基础性问题。

Conclusion: 在表格语言模型中，带有内置多样性的单元级获取函数能有效减少标注工作量，但仍需解决计算效率和人工标注方面的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的仅编码器Transformer模型（如TAPAS）在表格理解任务中表现成功，但受限于分类任务。

Method: TABT5是一个编码器-解码器模型，通过整合解码器组件以及利用表格特定嵌入和预训练来处理输入结构。

Result: TABT5在多个领域取得了新的SOTA结果，包括电子表格公式预测（序列准确率提高15%）、问答（序列准确率提高2.5%）和数据到文本生成（BLEU提高2.5%）。

Conclusion: TABT5通过引入编码器-解码器架构，成功克服了仅编码器模型在表格理解任务中的局限性，并显著提升了多项任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 鉴于可从各种文档类型中轻松收集大量表格，并且表格预训练在文本和图像领域取得成功，因此提出了大量的表格预训练框架。

Method: 本研究采用综述方法，旨在全面回顾表格预训练的不同模型设计、预训练目标和下游任务，并进一步分享对现有挑战和未来机遇的思考和愿景。

Result: 本综述全面回顾了表格预训练的模型设计、预训练目标和下游任务，并提出了对现有挑战和未来机遇的见解。

Conclusion: 表格预训练是一个快速发展的领域，未来的研究应关注现有挑战和新机遇。
...
<details>
  <summary>Details</summary>
Motivation: 现有的表格数据深度学习问答方法有局限性，例如需要将整个表格作为输入，这对于处理大量、实时更新的数据集来说不切实际，会遇到内存限制和序列化开销等问题。

Method: TableQuery利用预训练的自由文本问答深度学习模型，将自然语言查询转换为结构化查询，然后这些查询可以在数据库或电子表格上运行。这种方法避免了将整个数据加载到内存中以及对数据库进行序列化的需要，并且无需重新训练模型。

Result: 该方法消除了将整个数据加载到内存和数据库序列化的需要。此外，它利用了现成的预训练模型，且无需重新训练，当有性能更好的新模型时，可以直接替换。

Conclusion: TableQuery为使用自然语言查询表格数据提供了一种高效实用的解决方案，它利用了现有的自由文本问答模型，克服了传统深度学习方法在处理表格数据时的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 评估计算机辅助翻译（CAT）工具中分段和排列方式的设计选择对翻译人员绩效的影响。

Method: 通过受控实验评估了三种文本处理任务中的速度和准确性。

Result: 分句呈现方式比未分段文本能更快地进行文本复制和句内错误识别；源语和目标语的上下排列比并排排列能更快地进行文本复制。然而，在修订任务中，未分段文本能带来最高的准确性和时间效率。
...
<details>
  <summary>Details</summary>
Motivation: 处理单机无法处理的超大型数据集，并为数据分析师提供快速、多维度的交互式数据探索能力。

Method: Hillview引入了可视化草图（vizketches）。vizketches结合了数据汇总的算法技术和计算机图形学的高效渲染原理，以实现扩展性。

Result: 使用八台服务器运行的Hillview，可以导航和可视化数百亿行和数万亿个单元格的数据集，远超现有竞争系统的能力。

Conclusion: Hillview及其vizketches方法通过并行化计算、减少通信、提供渐进式可视化和精确精度保证，有效扩展了电子表格，成功处理了大规模数据集并提供了高度交互性。
...
Method: 通过设计和优化分层关联数组的实现，该实现通过控制层级中每个条目的数量来级联更新，从而减少内存压力并提高更新速率。这些参数易于调整以达到最佳性能。

Result: 单个实例的分层数组每秒可实现超过40,000次更新。在MIT SuperCloud上，1,100个服务器节点上的34,000个分层D4M关联数组实例实现了每秒1,900,000,000次持续更新。
...
<details>
  <summary>Details</summary>
Motivation: 波斯古典音乐的非节拍形式和radif是其核心。需要一个数字语料库来对这种音乐进行计算研究。

Method: 作者创建了一个完整的非节拍radif曲目数字语料库，涵盖所有13个现有组成部分。提供了MIDI文件（总计约281分钟）和数据表格，描述了228首曲子的音符、音符持续时间、音程和分层结构，忠实地再现了包含四分音的调性以及非节拍方面。此外，还提供了支持性的基本统计数据以及语料库的复杂性和相似性度量。

Result: 建立了一个全面的非节拍radif曲目数字语料库，忠实地再现了波斯古典音乐的调性（包括四分音）和非节拍方面，包含MIDI文件和详细数据。

Conclusion: 该语料库为波斯古典音乐的计算研究提供了一个平台，有助于旋律模式研究、即兴风格调查以及音乐信息检索、音乐理论和计算（民族）音乐学中的其他任务。
...
<details>
  <summary>Details</summary>
Motivation: 现代AI系统需要结合多模态数据（如文本、图像、音频）以实现准确预测。机器学习领域通常认为认知不确定性可通过收集更多数据来减少，而随机不确定性是不可约的。然而，在多模态信息获取时，这一假设面临挑战。

Method: 本文引入了一个创新的数据获取框架，其中不确定性分解可以指导可操作的决策，允许在两个方向进行采样：样本量和数据模态。主要假设是随机不确定性随模态数量增加而减少，而认知不确定性通过收集更多观测数据而减少。该框架结合了主动学习、主动特征获取和不确定性量化。

Result: 在两个多模态数据集上提供了概念验证实现，以展示所提出的数据获取框架。

Conclusion: 本文提出了一个创新的多模态数据获取框架，通过不确定性分解来指导样本量和数据模态的选择，从而挑战了关于不确定性减少的传统假设。
...
<details>
  <summary>Details</summary>
Motivation: 目前表格数据领域缺乏像语言模型和计算机视觉领域那样，通过基础模型实现迁移学习的有效方法。

Method: 该研究从TabLib语料库中提取了一个包含21亿行数据和超过400万个独立表格的大型高质量数据集，并使用新颖的打包和注意力机制对Llama 3-8B大型语言模型进行微调，以实现表格数据的分类和分箱回归预测。

Result: 在329个数据集的测试中，TabuLa-8B在未见过的表格上的零样本准确率比随机猜测高出15个百分点以上；在少量样本（1-32个样本）设置下，无需对目标数据集进行任何微调，TabuLa-8B的准确率比在等量甚至多达16倍数据上显式训练的XGBoost和TabPFN模型高出5-15个百分点。

Conclusion: TabuLa-8B显著缩小了表格数据领域在迁移学习方面的差距，展示了大型语言模型在表格预测任务中的卓越潜力，并提供了模型、代码和数据以供进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习需要高质量、可访问的训练数据，特别是生物医学和金融领域中敏感的表格数据。合成数据生成有望解锁这些敏感数据，但现有的生成模型（如带有差分隐私的GANs）在隐私与数据质量之间存在权衡，且面临不稳定的对抗性训练和模式崩溃等问题。

Method: 本文实施了利用注意力机制学习可逆表格表示的新型端到端模型。我们引入了TableDiffusion，这是第一个用于表格数据合成的差分隐私扩散模型。该模型通过预测添加的噪声，绕过了重建混合类型表格数据的挑战。

Result: TableDiffusion生成了更高保真度的合成数据集，避免了模式崩溃问题，并在私有化表格数据合成方面达到了最先进的性能。扩散范式在数据和隐私效率上都远超对抗性范式。

Conclusion: 扩散范式，特别是TableDiffusion，为差分隐私的表格数据合成提供了一个卓越的解决方案，与对抗性方法（如GANs）相比，它提供了更好的数据质量和隐私效率。
...
<details>
  <summary>Details</summary>
Motivation: 马尼拉捷运3号线（MRT3）每日客流量波动大，受节假日、工作日等因素影响，难以预测。通勤者缺乏当日客流量信息，影响行程规划。交通部门依赖难以分析的历史电子表格数据。

Method: 本研究提出了一种时间序列预测方法，用于预测每日客流量。

Result: 摘要中未提供具体预测结果。

Conclusion: 该研究旨在预测未来特定日期特定车站的客流量。
...
<details>
  <summary>Details</summary>
Motivation: 正确识别文件类型对于法医调查至关重要，尤其是在系统所有者可能希望隐藏文件类型或使其无法访问的情况下。

Method: 论文提出使用半监督生成对抗网络（SGAN），该网络经过对抗训练。

Result: SGAN在对11种不同类型文件进行分类时达到了97.6%的准确率。与传统的独立神经网络和其他三种机器学习算法相比，对抗训练网络被证明是最精确的文件分类器，特别是在只有少量监督样本的情况下。

Conclusion: 基于SGAN的文件分类器能够准确确定文件类型，即使在文件被混淆的情况下，并且优于其他方法，特别是在数据有限的情况下。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究通常采用卷积神经网络（CNN）来建模表格结构的空间信息，但忽略了单元格之间更多样化的关系信息，如层级和并列关系，而理解表格数据的内在语义结构是一个关键问题。

Method: 提出TabularNet神经网络架构。空间编码器利用行/列级池化和双向门控循环单元（Bi-GRU)捕获统计信息和局部位置相关性。关系信息方面，设计了一种基于WordNet树的新图构建方法，并采用基于图卷积网络（GCN）的编码器，关注单元格之间的层级和并列关系。

Result: 在两个真实世界电子表格数据集上的三个分类任务中进行了广泛实验，结果表明TabularNet优于最先进的基线方法。

Conclusion: TabularNet能够有效地同时提取表格数据的空间和关系信息，作为不同理解任务的统一神经骨干，并在多任务场景中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 提供一种无需复杂编程结构即可实现可视化、透明的反向传播方法。

Method: 该方法称为“可视化反向传播”，利用声明式函数式编程、数组工作表公式和手动计算，在电子表格中实现顺序计算。它不使用宏、用户定义函数、循环或与过程程序的链接。

Result: 在电子表格中实现了可视化且透明的反向传播，并通过与Tensorflow (Python) 解决方案在标准回归问题上的比较进行了说明。

Conclusion: 该方法提供了一种在电子表格中实现反向传播的简化且透明的方式。
...
<details>
  <summary>Details</summary>
Motivation: 目前的学习算法实证测试相对较少，研究人员需要一个工具来进行更全面的测试，涵盖更广泛的算法和测试问题。同时，优化学习文献中对先验的调整和构建问题也常常被忽视。

Method: 引入了MOLTE，一个基于Matlab的模拟器，它通过使用.m模块表示学习策略和问题，使得添加新算法和新测试问题变得容易。它提供状态先进的策略和各种问题类别，通过电子表格界面进行选择，并包含不同的图形度量标准。MOLTE兼容并行计算，可从本地桌面扩展到集群和云。

Result: MOLTE作为一个易于使用的工具提供给研究社区，能够进行更全面的测试，涵盖更广泛的算法和测试问题。通过一系列策略比较演示了其能力，并解决了最优学习文献中常被忽视的先验调整和构建问题。

Conclusion: MOLTE旨在为研究人员提供一个便捷的环境，以研究最优学习中的有趣问题，从而推动更全面、更广泛的算法和测试问题的实证测试。
...
<details>
  <summary>Details</summary>
Motivation: 现有的电子邮件社交网络分析方法忽略了附件这一重要数据，而附件占据了电子邮件档案80-90%的存储空间，并且能够体现关系的“亲密性”。因此，作者认为通过附件构建网络可以提供更深入的社交结构洞察。

Method: 从Enron电子邮件语料库中提取通信网络和共享附件网络。在两种网络中分析度中心性、中介中心性、接近中心性和特征向量中心性。使用最近邻算法为五名Enron员工生成相似性群组。

Result: 生成的相似性群组与Enron的组织结构图一致。

Conclusion: 通过将共享附件作为网络边缘的方法，能够有效且准确地揭示电子邮件档案中的社交结构，该方法得到了Enron组织结构图的一致性验证。
...
<details>
  <summary>Details</summary>
Motivation: 需要一个软件工具箱，以实现皮肤癌分类中深度神经网络的快速配置和超参数设置，并允许非技术用户在切换数据集时轻松探索不同的配置。

Method: 描述了一个软件工具箱及其架构，支持配置卷积神经网络（CNN）及其超参数。用户界面设计为简单的电子表格。初步研究使用了两个CNN进行皮肤镜图像上的黑色素瘤检测。

Result: 初步结果量化了图像增强、图像分辨率和缩放滤波器对黑色素瘤检测整体性能和训练时间的影响。

Conclusion: 初步结果表明，图像增强、图像分辨率和缩放滤波器对黑色素瘤检测的性能和训练时间有显著影响。
...
<details>
  <summary>Details</summary>
Motivation: 勒索软件对网络安全构成重大挑战，攻击者利用复杂的加密方法（如Base64编码的熵降低、部分或间歇性加密）来逃避传统检测方法。

Method: 研究利用32.6 GB的庞大数据集，包含11,928个多种格式的文件，由75个不同的勒索软件家族加密。采用在线增量机器学习算法来预测文件加密活动。

Result: Hoeffding Tree算法在检测传统和AES-Base64加密方法方面表现出卓越的增量学习能力。Random Forest分类器与warm-start功能相结合，在识别间歇性加密方法方面表现出色。

Conclusion: 为应对复杂的勒索软件策略，需要量身定制的机器学习解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 通过激励各方如实披露私有信息，实现更和谐的债务和解，并旨在改进所有现有方法，提供一种在私有信息下实现债务减免和豁免的个体理性、激励兼容、真实/策略证明、事后有效、最优的机制。

Method: 通过将安全计算技术引入债务减免，并提供了“安全电子表格”的简单实用实现，以及使用Raziel智能合约在具有Pravuil共识的区块链上实现的另一个实现。

Result: 实现了个体理性、激励兼容、真实/策略证明、事后有效、最优的债务减免和豁免机制，并且首次通过安全计算技术，使得债务和解的预期利润更高，成功概率也高于不使用安全计算的情况。

Conclusion: JUBILEE提供了一种优越的、通过安全计算实现的债务减免和豁免机制，能够更有效地激励信息披露，并带来更高的预期利润和成功率。
...
<details>
  <summary>Details</summary>
Motivation: 在合并工程模型时，无需模型作者之间达成共识，因为理论和模型态射是语义保持的。

Method: 将每个电子表格的公式表示为代数理论，值表示为模型。表格之间的重叠表示为理论和模型态射。然后，使用范畴论的共极限、提升和Kan-扩展构造来计算一个规范通用的集成理论和模型，最终表示为一个电子表格。此外，还需要进行自动定理证明来验证重叠映射的语义保持性和集成表格的保守性/一致性。

Result: 通过一个在大型能源公司进行的真实世界油气计算案例研究，展示了该方法在整合由两名独立工程师构建的不同套管压力测试（MASP）计算电子表格上的应用。

Conclusion: 总结了如何将该方法应用于企业范围内的工程规模化工作。
...
<details>
  <summary>Details</summary>
Motivation: 数据汇总对于从大型数据集中发现洞察力至关重要。枢轴表提供了一种方便的方式来汇总表格数据，但识别有用的属性组合，尤其是在高维数据集中，仍然是一个挑战。传统方法未能充分解决表格多样化问题，导致冗余。

Method: 本文将自动推荐有洞察力和可解释的枢轴表问题形式化。提出了SAGE系统，它是一个数据语义感知的系统，用于推荐K个预算约束下的多样化枢轴表。SAGE确保每个枢轴表都具有洞察力、可解释性，并能适应用户行为和偏好，同时保证枢轴表集彼此不同。主要技术贡献包括：1) 一个数据语义感知模型，用于衡量单个枢轴表的效用和枢轴表集的多样性；2) 一个可扩展的贪婪算法，通过利用数据语义显著减少组合搜索空间，从而高效选择一组高效用的多样化枢轴表。

Result: SAGE在三个真实世界数据集上的广泛实验表明，它优于替代方法，并且能够有效地扩展以适应高维数据集。此外，案例研究突出SAGE在定性方面优于商业软件和大型语言模型（LLMs）。

Conclusion: SAGE系统通过其数据语义感知模型和可扩展的算法，有效地解决了推荐多样化、有洞察力且可解释的枢轴表的问题，并在性能和效率上超越了现有方法和商业工具。
...
<details>
  <summary>Details</summary>
Motivation: 在许多工业环境中，用户希望通过自然语言访问结构化数据源（如电子表格、数据库、API），但现有的大型语言模型驱动的Text-to-SQL应用无法有效处理这种数据源的异构性，导致其实用性受限。

Method: 本文提出了siwarex平台，通过引入该平台来解决数据源异构性问题。siwarex平台能够实现对数据库和API的无缝自然语言访问。为了验证其有效性，作者扩展了流行的Spider数据集和基准，将其中一些表替换为数据检索API。

Result: siwarex平台能够很好地应对数据源异构性问题。修改后的Spider基准数据集即将向研究社区开放。

Conclusion: siwarex平台成功地解决了工业环境中数据源异构性的挑战，通过提供统一的自然语言接口，极大地提升了对数据库和API的访问效率和便利性。
...
<details>
  <summary>Details</summary>
Motivation: 目标是生成结构良好、可互操作的数据，并允许在不具备源数据的情况下进行数据重构。

Method: 该工具包将数据策展分为独立的组件，以模式为中心，通过高级序列脚本描述模式到模式的映射。它以Python包和“无代码”可视化网络应用的形式提供。

Result: 数据得到转换，任何符合模式定义的数据都可以通过交叉映射进行重构。一个实例展示了如何将数百个地方议会的分散源数据集成到单个数据库中。

Conclusion: 该工具包提供了一种有效的方法，可以将复杂和分散的表格数据重构为结构良好、可互操作的数据，并具有任务分离和降低复杂性的优点，这在其集成大量地方议会数据的应用中得到了体现。
...
<details>
  <summary>Details</summary>
Motivation: 尽管电子表格广泛使用，但对于非专业用户来说，编写复杂公式仍然具有挑战性，因为他们需要理解复杂的公式语法。

Method: 开发了一个名为Auto-Formula的系统，该系统利用对比学习技术（受计算机视觉中“相似人脸识别”启发），通过学习和调整相似电子表格中已有的公式，来准确预测用户在目标单元格中想要编写的公式。

Result: 在从真实企业电子表格中提取的2000多个测试公式上进行了广泛评估，结果表明Auto-Formula系统优于其他替代方案。

Conclusion: Auto-Formula系统通过利用现有相似电子表格中的公式并结合对比学习，有效解决了非技术用户编写复杂电子表格公式的难题。
...
<details>
  <summary>Details</summary>
Motivation: 本研究旨在为农业从业者，特别是那些电子表格技能有限的农民，提供一种易于使用的结构化方法来管理其运营数据。这解决了对整洁、机器可读和人工可读数据需求，这些数据可以促进物流、提供上下文元数据并改进企业分析。

Method: 本研究提供了“结构良好的私人数据库模板作为农业从业者的开源资源”。具体来说，这些是“面向农民的AirTable数据库”，它们使用“简单的经过数据验证的表单，具有定制应用程序的外观和感觉”。此外，还提供了一个“解释如何为活动记录构建数据库的录制研讨会”。

Result: 这些模板“生成整洁、机器可读和人工可读、可编辑且可导出到其他软件进行分析的运营数据”。这些数据“可以促进物流、提供上下文元数据并改进企业分析”。

Conclusion: 这些资源“可能通过推广和结构化教育计划促进数字农业原则的注入”。
...
<details>
  <summary>Details</summary>
Motivation: 本文旨在回答“知识工程师的数据集成框架应该是什么样子？”这一问题，并提出将面向对象软件工程中的“façade”概念应用于知识图谱构建，以解决查询异构数据的问题。

Method: 论文描述了SPARQL Anything系统的设计原理和软件架构，该系统通过重载SPARQL 1.1的SERVICE子句，允许将异构资源（从流行格式如CSV、JSON到非主流格式如Markdown、DOCx）作为RDF进行查询。此外，还通过社区调查和行业实地报告来评估其用户价值。

Result: SPARQL Anything成功地支持查询多种文件格式（CSV、JSON、XML、Spreadsheets、Markdown、YAML、DOCx、Bibtex）和Web API，提供了高度灵活性、参数化查询和复杂转换链。论文提供了大量的可重用、真实世界的应用场景，并通过用户调查和行业报告证明了其设计基础对用户的价值。

Conclusion: SPARQL Anything系统基于“façade”概念，为知识工程师提供了一个有价值的数据集成框架，其广泛的格式支持、灵活的查询能力以及来自社区调查和行业报告的用户价值评估都证明了这一点。
...
<details>
  <summary>Details</summary>
Motivation: 真实世界数据中字符串数据普遍存在错误（例如，Excel表格中67.6%的值为文本），但现有系统在错误检测、用户交互需求或单独处理语法/语义错误方面存在局限性，无法有效应对同时包含这两种错误的字符串。

Method: DataVinci是完全无监督的。它学习覆盖列中大多数值的正则表达式模式来检测错误，并基于这些模式和跨列约束自动生成修复。对于同时包含语法和语义子字符串的情况，DataVinci在学习模式和推导编辑之前，使用LLM抽象（并重新具体化）语义部分。此外，它还利用现有程序的执行信息来识别和纠正那些无法通过多数模式识别的数据修复。

Result: DataVinci在4个现有和新基准测试中，在错误检测和修复方面均优于7个基线系统。

Conclusion: DataVinci为字符串数据错误检测和修复提供了一个全面的无监督解决方案，通过结合模式学习、LLM抽象和执行信息，有效处理了复杂的混合错误类型，显著提升了性能。
...
### [42] [Auto-Tables: Synthesizing Multi-Step Transformations to Relationalize Tables without Using Examples](https://arxiv.org/abs/2307.14565)
...
### [43] [Efficient and Compact Spreadsheet Formula Graphs](https://arxiv.org/abs/2302.05482)
*Dixin Tang,Fanchao Chen,Christopher De Leon,Tana Wattanawaroon,Jeaseok Yun,Srinivasan Seshadri,Aditya G. Parameswaran*

Main category: cs.DB

TL;DR: TACO是一个用于高效压缩电子表格公式图的框架，通过利用表格局部性来加速查询和维护。
...
Method: TACO利用表格局部性（即相邻单元格可能具有相似的公式结构）这一特性，提出了四种基于表格局部性的模式，并开发了用于使用这些模式压缩公式图、直接查询压缩图而无需解压缩以及在更新期间逐步维护图的算法。

Result: TACO可以显著减小公式图的大小。在查询公式图方面，TACO在我们的框架中实现的基线系统上实现了高达34,972倍的加速，在商业电子表格系统上实现了高达632倍的加速。

Conclusion: TACO通过压缩电子表格公式图，显著提高了查询和维护效率。
...
### [44] [Sigma Workbook: A Spreadsheet for Cloud Data Warehouses](https://arxiv.org/abs/2204.03128)
*James Gale,Max Seiden,Deepanshu Utkarsh,Jason Frantz,Rob Woollen,Çağatay Demiralp*

Main category: cs.DB

TL;DR: Sigma Workbook是一个交互式系统，它让业务用户可以通过类似电子表格的界面，轻松地对云数据仓库（CDWs）中的大规模数据进行可视化分析，该系统能够动态生成SQL查询并在CDWs上执行，从而实现了易用性、可伸缩性和表达性。


<details>
  <summary>Details</summary>
Motivation: 现有的CDW数据分析工具在即席转换方面受限，或者业务用户难以使用。

Method: Sigma Workbook提供了一个易于访问的类似电子表格的界面，通过直接操作进行分析。它根据用户交互动态构建匹配的SQL查询，并直接在CDWs上执行，以利用CDWs的优越特性（包括可伸缩性）。

Result: 通过3个实际用例（队列分析、会话化和数据增强）展示了Sigma Workbook。

Conclusion: Sigma Workbook强调了其易用性、可伸缩性和表达性。
...
### [45] [Efficient Specialized Spreadsheet Parsing for Data Science](https://arxiv.org/abs/2202.13189)
*Felix Henze,Haralampos Gavriilidis,Eleni Tzirita Zacharatou,Volker Markl*

Main category: cs.DB

TL;DR: 该论文提出了一种新的解析器，用于加载电子表格，该解析器比现有方法更快且内存使用量更少。


<details>
  <summary>Details</summary>
Motivation: 现有的电子表格加载方法运行时或内存使用量高，阻碍了在普通系统上进行数据探索。

Method: 引入了一种通过紧密耦合解压缩和解析来最小化内存使用量的新型解析器。通过优化的电子表格特定解析例程和并行化来减少运行时间。

Result: 将Excel电子表格加载到R环境的原型比现有技术快3倍，内存消耗少40倍。

Conclusion: 这种新方法通过显著提高效率，使在普通系统上加载电子表格变得实用。
...
### [46] [Spread2RML: Constructing Knowledge Graphs by Predicting RML Mappings on Messy Spreadsheets](https://arxiv.org/abs/2110.12829)
*Markus Schröder,Christian Jilek,Andreas Dengel*

Main category: cs.DB

TL;DR: Spread2RML能预测RML映射在杂乱的电子表格上，以减少创建RML映射所需的时间和精力。


<details>
  <summary>Details</summary>
Motivation: RDF映射语言（RML）允许将半结构化数据映射到RDF知识图谱，但由于电子表格数据模型的复杂性和混乱性，其映射创建通常非常耗时。

Method: 本文提出了Spread2RML，通过可扩展的RML对象映射模板集，基于启发式方法为每个列应用模板来预测RML映射。

Result: 在从非常杂乱的合成数据到数据不那么杂乱的data.gov电子表格的三个数据集上的评估中，获得了初步的、有前景的结果，特别是在全自动化和处理杂乱数据方面。

Conclusion: Spread2RML可以显著减少在杂乱电子表格上创建RML映射所需的工作量，并且能够全自动处理数据。
...
### [47] [Supercomputing Enabled Deployable Analytics for Disaster Response](https://arxiv.org/abs/2108.11525)
...
TL;DR: 开发了一种离线地理空间分析方法，将人口普查数据预计算为Google Earth和Excel文件，以帮助应急响应人员在网络受限的环境中进行快速分析，并在COVID-19期间进行了测试。


<details>
  <summary>Details</summary>
Motivation: 应急响应人员需要高级分析工具，但受限于网络接入和软件安全要求，无法使用基于云的微服务平台。

Method: 使用MIT SuperCloud将地理空间人口普查数据（总人口、15岁以下、65岁以上、中位年龄）处理并预计算成数千个Google Earth (kml) 和Microsoft Excel (xlsx) 文件。这些文件不依赖网络或额外软件，可在旧硬件上运行。Excel文件包含坐标转换功能，Google Earth文件则使用多种颜色地图显示人口密度。

Result: 成功生成了数千个Google Earth和Microsoft Excel文件，这些文件包含按县排序的每个普查区的人口普查数据，并通过易于使用的离线界面展示。使用MIT SuperCloud，新的分析可以在几分钟内生成。

Conclusion: 这种预计算和离线交付地理空间数据的方法为应急响应人员提供了一个强大的工具，可以显著提高应急准备能力，尤其适用于网络受限的环境。
...
### [48] [Table2Charts: Recommending Charts by Learning Shared Table Representations](https://arxiv.org/abs/2008.11015)
*Mengyu Zhou,Qingtao Li,Xinyi He,Yuejiang Li,Yibo Liu,Wei Ji,Shi Han,Yining Chen,Daxin Jiang,Dongmei Zhang*

Main category: cs.DB

TL;DR: Table2Charts是一个基于深度Q学习和启发式搜索的框架，用于从（表格，图表）对的大型语料库中学习常见模式，以推荐图表。它在多类型任务和人工评估中均优于其他图表推荐系统。


<details>
  <summary>Details</summary>
Motivation: 在实际应用中推荐常用图表时，需要考虑效率、数据不平衡和表格上下文等挑战。

Method: 本文提出了Table2Charts框架，该框架通过深度Q学习、复制机制和启发式搜索，实现表格到序列的生成，每个序列遵循一个图表模板。

Result: 在包含16.5万个表格和26.6万个图表的大型电子表格语料库上，Table2Charts能够学习表格字段的共享表示，使不同图表类型的推荐任务相互增强。它在多类型任务（R@3=0.61，R@1=0.43，召回率翻倍）和人工评估中均优于其他图表推荐系统。

Conclusion: Table2Charts框架有效解决了图表推荐中的挑战，并通过学习共享表示和优越的性能，显著提升了图表推荐的质量。
...
### [49] [Sigma Worksheet: Interactive Construction of OLAP Queries](https://arxiv.org/abs/2012.00697)
*James Gale,Max Seiden,Gretchen Atwood,Jason Frantz,Rob Woollen,Çağatay Demiralp*

Main category: cs.DB

TL;DR: 该论文介绍了Sigma Worksheet，一个用于云数据仓库中数据即席可视化分析的交互式系统，它通过类似电子表格的界面生成SQL查询，展示了其表达性、性能和用户友好性。


<details>
  <summary>Details</summary>
Motivation: 现有的云数据仓库数据分析工具在即席转换方面受限，或对业务用户来说难以使用，无法充分利用新一代云数据仓库的强大能力。

Method: Sigma Worksheet提供了一个类似电子表格的交互界面，通过直接操作进行数据分析。它能根据用户交互动态构建匹配的SQL查询，并直接在云数据仓库上执行这些查询。

Result: Sigma Worksheet通过两个实际用例（群组分析和会话化）展示了其表达能力；其生成的SQL查询性能与TPC-H基准测试的参考查询相当；通过对100人进行调查和对70人进行访谈，发现Sigma Worksheet更易于使用和学习，提高了用户生产力，并可进一步通过提供指导来改善用户体验。

Conclusion: Sigma Worksheet是一个有效且用户友好的交互式系统，用于云数据仓库中的即席可视化分析，它提高了用户生产力并具有良好的性能，未来还有进一步改进用户体验的潜力。
...
### [50] [Mapping Spreadsheets to RDF: Supporting Excel in RML](https://arxiv.org/abs/2104.13600)
*Markus Schröder,Christian Jilek,Andreas Dengel*

Main category: cs.DB

TL;DR: 该论文扩展了RML Mapper以支持Microsoft Excel文件，从而将电子表格数据映射到RDF图，解决了现有RML规范和实现对常用电子表格格式支持不足的问题。


<details>
  <summary>Details</summary>
Motivation: RDF映射语言（RML）支持将CSV等表格数据映射到RDF图，但目前其规范和常用实现忽略了广泛使用的电子表格格式（如Microsoft Excel）。

Method: 作者扩展了RML Mapper工具，使其支持Microsoft Excel电子表格文件，并通过交互式在线演示展示了其功能。该方法允许在典型的RML映射中访问电子表格单元格的各种元数据，并提供了一些针对特定用例的实验性功能。

Result: 扩展后的RML Mapper现在能够访问电子表格单元格的各种元数据。作者提供了其功能的交互式在线演示，并且实现代码已在GitHub上公开。

Conclusion: 通过扩展RML Mapper以支持Microsoft Excel文件，该工作弥补了RML对电子表格格式支持的空白，为将Excel数据映射到RDF提供了实用的解决方案，并提供了可公开获取的代码和在线演示。
...
### [51] [Dataset Generation Patterns for Evaluating Knowledge Graph Construction](https://arxiv.org/abs/2104.13576)
*Markus Schröder,Christian Jilek,Andreas Dengel*

Main category: cs.DB

TL;DR: 为解决知识图谱构建评估中真实数据集因保密性无法公开的问题，本文提出通过发现知识工作者的数据生成习惯模式来合成逼真的数据。研究从工业界的真实电子表格中提取了11种模式，并开发了名为Data Sprout的生成器来复现这些模式。


<details>
  <summary>Details</summary>
Motivation: 机密性阻碍了个人和企业真实标注数据集的发布，而这些数据集对于评估工业场景中的知识图谱构建方法非常有用。

Method: 基于知识工作者在生产或管理数据时存在特定习惯的假设，本文发现并利用数据生成模式来模仿真实数据集。具体方法是，从工业界的真实电子表格中推导出11种不同的模式，并开发了名为Data Sprout的生成器来重现这些模式。

Result: Data Sprout生成器能够重现所发现的11种模式，并能生成电子表格，展示了所实现的模式所产生的改变效果。

Conclusion: 本文展示了一种通过识别和应用真实世界数据生产模式来生成逼真合成数据的方法和工具（Data Sprout），这可以用于评估知识图谱构建，同时避免机密性问题。
...
### [52] [Interactively Constructing Knowledge Graphs from Messy User-Generated Spreadsheets](https://arxiv.org/abs/2103.03537)
*Markus Schröder,Christian Jilek,Michael Schulze,Andreas Dengel*

Main category: cs.DB

TL;DR: 该论文提出了一种交互式方法，通过图形用户界面批量标注电子表格单元格来解决从非结构化电子表格数据构建知识图谱的挑战，并构建了一个25k三元组图，其方法优于现有的RDF映射语言（RML）方法。


<details>
  <summary>Details</summary>
Motivation: 知识工作者自由填充的电子表格常常包含非结构化内容，这使得机器难以正确解释。将这些“混乱”的数据转换为知识图谱等结构化形式极具挑战性，尤其是在缺乏数据维护策略的情况下。

Method: 作者提出了一种交互式方法，包含一个图形用户界面（GUI），使知识工程师能够批量标注电子表格单元格中的信息。知识图谱最终基于这些单元格的标注形成。

Result: 该方法利用五个工业场景中的电子表格构建了一个包含25k三元组的知识图谱。通过与最先进的RDF映射语言（RML）尝试进行比较，突出了其方法的贡献。

Conclusion: 该论文提供了一种通过交互式GUI批量标注电子表格单元格来构建知识图谱的有效方法，成功解决了从混乱电子表格数据构建知识图谱的挑战，并展示了相对于现有技术的优势。
...
### [53] [Leam: An Interactive System for In-situ Visual Text Analysis](https://arxiv.org/abs/2009.03520)
*Sajjadur Rahman,Peter Griggs,Çağatay Demiralp*

Main category: cs.DB

TL;DR: 该论文提出了 Leam 系统，它结合了计算笔记本、电子表格和可视化工具的优势，旨在将文本分析过程视为一个单一的连续体，以解决现有系统在数据异构性、可复用性、可重现性等方面面临的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的文本分析系统通常只能满足部分分析阶段的需求，并且未能解决与数据异构性、数据来源、工作流可复用性、可重现性以及与既定实践的兼容性相关的挑战。

Method: 作者基于从这些挑战中得出的一系列设计考虑，提出了 Leam 系统。Leam 将文本分析过程视为一个单一的连续体，结合了计算笔记本、电子表格和可视化工具的优势。它具有用于运行文本分析工作流的交互式用户界面、用于管理多种原子和复合数据类型的新数据模型，以及一个富有表现力的代数，可以捕获代表文本分析各个阶段的不同操作集，并实现系统不同组件（包括数据、代码和可视化）之间的协调。

Result: 作者报告了 Leam 开发的当前进展，并通过使用示例展示了其有用性。
...
### [54] [ObjTables: structured spreadsheets that promote data quality, reuse, and integration](https://arxiv.org/abs/2005.05227)
*Jonathan R. Karr,Wolfram Liebermeister,Arthur P. Goldberg,John A. P. Sekar,Bilal Shaikh*

Main category: cs.DB

TL;DR: ObjTables是一个工具包，它通过结合电子表格与模式和对象关系映射系统，使电子表格能够被人类和机器读取，从而促进数据重用和元分析。


<details>
  <summary>Details</summary>
Motivation: 科学研究中，理解复杂网络中的系统行为以及整合异构信息是一个核心挑战。电子表格作为关键数据源，因缺乏模式而难以重新分析，阻碍了数据的有效聚合、重用和集成。

Method: 开发了ObjTables工具包，它包含：模式格式、用于指示电子表格和列所代表的类和属性的标记、针对科学信息的多种数据类型，以及用于读写、验证、比较、合并、修订和分析电子表格的高级软件。

Result: ObjTables使电子表格更易于重用，从而能够进行前所未有的二次元分析。通过简化新数据类型格式和相关软件的构建，ObjTables还能加速新兴科学领域的发展。

Conclusion: ObjTables通过提高电子表格的重用性，不仅能促进二次元分析，还能加速新兴科学领域的发展，使其能够更轻松地构建新的数据格式和相关软件。
...
### [55] [Needles in the 'Sheet'stack: Augmented Analytics to get Insights from Spreadsheets](https://arxiv.org/abs/2006.08224)
...
TL;DR: 该论文演示了一种商业智能（BI）工具，能够无需事先了解数据库或报告模式，即可从周期性电子表格中提取洞察力，主要面向不熟悉数据库模式的用户。


<details>
  <summary>Details</summary>
Motivation: 现有的商业智能工具通常需要用户了解数据库模式或具备相应的分析资源。本文的动机是为那些不熟悉数据库模式或缺乏现代分析资源的用户提供一种获取洞察力的方法，通过分析周期性电子表格来解决这一痛点。

Method: 该演示专注于通过检查不同的周期性报告（即视图）的电子表格来提供洞察力，而无需事先了解数据库或报告的模式，也无需数据信息。

Result: （由于是演示论文，结果是系统能够实现的功能）展示了在缺乏数据库或报告模式先验知识的情况下，从周期性电子表格中提取洞察力的能力。

Conclusion: 该解决方案旨在服务那些不熟悉数据库模式或缺乏进行现代分析所需资源的用户。
...
### [56] [A Billion Updates per Second Using 30,000 Hierarchical In-Memory D4M Databases](https://arxiv.org/abs/1902.00846)
...
TL;DR: 该论文提出了一种使用分层D4M关联数组对大规模网络图表示进行高性能流式更新的方法，在MIT SuperCloud上实现了每秒19亿次更新。


<details>
  <summary>Details</summary>
Motivation: 分析大规模网络需要对图表示进行高性能的流式更新，而关联数组非常适合表示和分析流式网络数据。

Method: D4M库实现了关联数组，并通过分层实现优化内存性能，以处理大规模流式更新。

Result: 在MIT SuperCloud上，1100个服务器节点上运行34,000个分层D4M关联数组实例，实现了每秒1,900,000,000次的持续更新速率。

Conclusion: 这种能力使MIT SuperCloud能够分析极大规模的流式网络数据集。
...
### [57] [Towards Semantically Enhanced Data Understanding](https://arxiv.org/abs/1806.04952)
*Markus Schröder,Christian Jilek,Jörn Hees,Andreas Dengel*

Main category: cs.DB

TL;DR: 该论文提出了一种使用单一语义模型将数据与其文档互联的方法，旨在帮助数据科学家并支持其他应用，并展示了一个早期原型。


<details>
  <summary>Details</summary>
Motivation: 在机器学习领域，数据理解需要大量文档，但这些文档通常分散在各种外部资料中，导致查找开销大，且其他支持应用难以利用这些非结构化数据。

Method: 提出一种使用单一语义模型的方法，该模型将数据与其文档互联。

Result: 数据科学家能够通过链接直接查找相关数据信息，浏览引用数据的文档。该模型还可用于其他方法，以提供额外支持，如数据搜索、比较、集成或可视化。展示了一个早期原型。

Conclusion: 所提出的语义模型通过将数据和文档互联，有效解决了数据文档分散的挑战，从而改善了数据科学家的数据理解，并支持了其他应用。
...
### [58] [WebRelate: Integrating Web Data with Spreadsheets using Examples](https://arxiv.org/abs/1711.05787)
*Jeevana Priya Inala,Rishabh Singh*

Main category: cs.DB

TL;DR: WebRelate是一个系统，它通过输入-输出示例，帮助数据科学家和电子表格用户将半结构化网络数据与关系数据集成起来，解决了URL生成和数据提取的挑战。


<details>
  <summary>Details</summary>
Motivation: 将网络数据与关系数据集成面临两大挑战：一是多数网站不提供直接的表格数据接口，用户需编写逻辑获取不同网页；二是提取所需数据需编写复杂的输入依赖脚本，这超出了许多数据科学家和终端用户的编程专业知识。

Method: WebRelate将网络数据集成任务分解为URL学习和输入依赖的网络提取两个子任务。它通过少量示例URL学习字符串转换程序来生成URL，并通过数据提取示例学习程序来提取其他行的数据。系统设计了表达性强的领域特定语言（DSLs）用于URL生成和网络数据提取，并提出了高效的合成算法，可以从少量输入-输出示例中学习这些DSL程序。

Result: WebRelate在88个真实世界的网络数据集成任务上进行了评估，结果表明，在大多数任务中，WebRelate可以在几秒钟内使用仅一个示例学习到所需的程序。

Conclusion: WebRelate系统通过示例学习的URL生成和数据提取方法，有效解决了非程序员在网络-关系数据集成中遇到的挑战，展现出高效率和易用性。
...
### [59] [Towards a Holistic Integration of Spreadsheets with Databases: A Scalable Storage Engine for Presentational Data Management](https://arxiv.org/abs/1708.06712)
*Mangesh Bendre,Vipul Venkataraman,Xinyan Zhou,Kevin Chang,Aditya Parameswaran*

Main category: cs.DB

TL;DR: 本文介绍DataSpread，一个将电子表格作为前端界面与数据库作为后端数据存储进行整合的系统，以提供电子表格的可扩展性和数据库的交互性，称之为“演示数据管理”（PDM）。文中重点介绍了PDM的存储引擎，旨在灵活地表示电子表格数据并支持按位置访问。


<details>
  <summary>Details</summary>
Motivation: 电子表格具有交互性但缺乏可扩展性；数据库系统具有可扩展性但缺乏交互性。

Method: 开发DataSpread的存储引擎，灵活表示电子表格数据，并支持按位置访问。通过广泛的电子表格使用调查来确定功能需求。开发了一种高效的方法来识别最优表示（尽管通用情况是NP-Hard）。扩展了机制以实现常数时间的按位置访问和修改，避免了级联更新问题。

Result: 在典型电子表格和操作工作负载下，存储空间减少高达20%，公式评估时间减少高达50%。

Conclusion: 本文为实现演示数据管理（PDM）愿景迈出了第一步，通过开发一个高效的存储引擎，将电子表格与数据库系统有效整合。
...
### [60] [Real-time stock analysis for blending recipes in industrial plants](https://arxiv.org/abs/1909.02960)
*Florin Zamfir,Nicolae Paraschiv,Emil Pricop*

Main category: cs.OH

TL;DR: 本文开发了一款C#应用程序，旨在解决Excel库存记录的痛点，该程序能够集中化混合过程的库存数据，帮助操作员进行决策，并计算所需成分和成品数量。


<details>
  <summary>Details</summary>
Motivation: 许多公司使用Excel表格管理库存和过程数据，但这些表格难以理解和追踪，且存在公式被随意更改或擦除的风险。因此，需要一个能够集中数据并辅助操作员决策的应用程序，尤其要考虑实时数据读取。

Method: 开发了一个C#应用程序，集成了规划算法。这些算法旨在识别混合过程所需成分、确定现有成分可生产的成品数量以及确定成品的最佳数量。该应用支持用户逐步构建结果。

Result: 开发了一个易于操作员使用的实时库存分析应用程序，该程序考虑了实时数据，并通过其规划算法辅助操作员进行决策，能够识别所需成分、确定可能的成品数量和优化成品产量。

Conclusion: 通过开发的C#应用程序，实现了库存数据的集中化管理，并利用集成的规划算法优化了混合过程，有效辅助了操作员的决策，克服了传统Excel表格的局限性。
...
### [61] [Billion-files File Systems (BfFS): A Comparison](https://arxiv.org/abs/2408.01805)
*Sohail Shaikh*

Main category: cs.PF

TL;DR: 该研究分析了 EXT4、XFS、BtrFS、ZFS 和 F2FS 等流行 Linux 文件系统在处理十亿个文件时的性能和局限性，重点关注吞吐量、存储利用率和性能下降。


<details>
  <summary>Details</summary>
Motivation: 数据量呈指数级增长，需要快速处理，因此数据需要非常接近计算设备以减少传输延迟。这使得理解本地文件系统的工作原理、性能及其局限性变得至关重要。

Method: 通过创建、存储然后回读十亿个文件，分析了 EXT4、XFS、BtrFS、ZFS 和 F2FS 等流行的 Linux 文件系统。研究还捕获并分析了读/写吞吐量、存储块使用情况、磁盘空间利用率和开销以及其他有用指标。此外，研究还探讨了在创建大量文件和文件夹期间和之后文件系统性能下降等副作用。

Result: 研究捕获并分析了读/写吞吐量、存储块使用情况、磁盘空间利用率和开销。此外，研究还探索了在创建大量文件和文件夹期间和之后文件系统性能下降的现象。

Conclusion: 本研究为系统设计者和集成者提供了关于各种 Linux 文件系统在处理海量文件（包括性能下降）时的性能和局限性的有用指标和见解。
...
### [62] [Why Isn't Relational Learning Taking Over the World?](https://arxiv.org/abs/2507.13558)
*David Poole*

Main category: cs.AI

TL;DR: 本文探讨了关系学习为何未能像基于像素、单词的AI系统那样普及，尽管现实世界和企业数据多为关系型，并提出了提升其重要性的必要措施。


<details>
  <summary>Details</summary>
Motivation: 当前的AI系统主要关注像素、单词和音素的建模，但现实世界和企业最有价值的数据多以实体、属性及关系的形式存在。关系学习旨在处理这类数据，但其应用远不如其他AI领域广泛。本文旨在探讨其原因并指明未来的发展方向。

Method: 本文通过解释和分析的方式，阐述了关系学习未能普及的原因，并探讨了如何使其获得应有的地位。

Result: 本文解释了关系学习目前未能普及的原因，并提出了使其在未来获得显著地位所需采取的措施。

Conclusion: 尽管关系型数据在现实世界和企业中占据重要地位，且关系学习对此类数据至关重要，但其当前影响力有限。未来需要采取措施，以使其发挥应有的潜力并获得广泛应用。
...
### [63] [MMTU: A Massive Multi-Task Table Understanding and Reasoning Benchmark](https://arxiv.org/abs/2506.05587)
*Junjie Xing,Yeye He,Mengyu Zhou,Haoyu Dong,Shi Han,Lingjiao Chen,Dongmei Zhang,Surajit Chaudhuri,H. V. Jagadish*

Main category: cs.AI

TL;DR: 该论文介绍了MMTU，一个包含25个真实世界表格任务、超过3万个问题的大规模基准测试，旨在全面评估LLM在专家级水平上理解、推理和操作表格的能力，并揭示了当前前沿模型在此方面的显著挑战。


<details>
  <summary>Details</summary>
Motivation: 目前LLM处理表格的基准测试有限、稀缺且范围狭窄，仅关注NL-to-SQL和Table-QA等任务，忽略了专业用户面临的广泛实际任务。这一差距限制了我们对这一重要领域的理解和模型进展。

Method: 作者引入了MMTU，一个包含25个真实世界表格任务、超过3万个问题的大规模基准测试。这些任务来源于数十年关于表格数据的计算机科学研究，专注于专业用户面临的复杂表格任务。

Result: MMTU要求模型结合表格理解、推理和编码等多种技能，这对当前的前沿模型来说仍然具有挑战性。即使是像OpenAI o4-mini和DeepSeek R1这样的前沿推理模型也只获得了大约60%的分数。

Conclusion: 模型在处理复杂表格任务方面仍有很大的改进空间。作者希望MMTU能推动对结构化数据处理和分析的基础模型的理解和开发取得进一步进展。
...
### [64] [Fortune: Formula-Driven Reinforcement Learning for Symbolic Table Reasoning in Language Models](https://arxiv.org/abs/2505.23667)
*Lang Cao,Jingxian Xu,Hanbing Liu,Jinyu Wang,Mengyu Zhou,Haoyu Dong,Shi Han,Dongmei Zhang*

Main category: cs.AI

TL;DR: Formula Tuning (Fortune) 是一种强化学习 (RL) 框架，旨在训练大型语言模型 (LLM) 生成可执行的电子表格公式，用于表格数据上的问答。它通过使用二元答案正确性作为奖励信号，显著提高了 LLM 在多步数值和符号推理任务上的性能，使一个 7B 模型在表格理解方面超越了 OpenAI o1。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型 (LLM) 在处理表格数据的准确数值或符号推理方面仍然存在困难，尤其是在复杂场景中，而表格理解对于智能系统至关重要。

Method: 本文提出了 Formula Tuning (Fortune)，这是一个强化学习 (RL) 框架，用于训练 LLM 生成可执行的电子表格公式，以进行通用表格数据的问答。它通过使用二元答案正确性作为奖励信号来减少对监督公式标注的依赖，从而引导模型学习公式推导。

Result: Formula Tuning 显著增强了 LLM 的性能，特别是在多步数值和符号推理任务上，使一个 7B 模型在表格理解方面超越了 OpenAI o1。

Conclusion: 公式驱动的强化学习 (RL) 有潜力推动大型语言模型在符号表格推理方面的进步。
...
### [65] [The AI Risk Repository: A Comprehensive Meta-Review, Database, and Taxonomy of Risks From Artificial Intelligence](https://arxiv.org/abs/2408.12622)
...
TL;DR: 该论文创建了一个AI风险库，包含来自43个分类法的777种风险，通过因果和领域分类法进行分类，旨在为讨论和管理AI风险提供一个通用参考框架。


<details>
  <summary>Details</summary>
Motivation: 当前对AI风险缺乏共识，阻碍了对风险的全面讨论、研究和应对。

Method: 通过系统审查分类法和其他AI风险结构化分类，并结合专家咨询，构建了AI风险库。同时，采用最佳拟合框架综合法开发了高层因果分类法和中层领域分类法。

Result: 建立了一个AI风险库，其中包含一个动态数据库，收录了从43个分类法中提取的777种风险。这些风险可通过高层因果分类法（实体、意图、时机）和中层领域分类法（7个领域，23个子领域）进行过滤和分类。该库可通过网站和在线表格轻松访问、修改和更新。

Conclusion: 该AI风险库首次严格整理、分析并提取了AI风险框架，创建了一个可公开访问、全面、可扩展和分类的风险数据库，为更协调、连贯和完整的AI系统风险定义、审计和管理方法奠定了基础。
...
### [66] [SpreadsheetLLM: Encoding Spreadsheets for Large Language Models](https://arxiv.org/abs/2407.09025)
...
TL;DR: SpreadsheetLLM 提出了一种高效的编码方法 (SheetCompressor)，使大型语言模型 (LLMs) 能有效处理电子表格，显著提高了表格检测和问答等任务的性能，实现了 25 倍的压缩率和最先进的 F1 分数。


<details>
  <summary>Details</summary>
Motivation: 电子表格的复杂性（广泛的二维网格、灵活的布局和多样的格式）对大型语言模型 (LLMs) 构成了重大挑战。

Method: 首先，提出了一种结合单元格地址、值和格式的简单序列化方法。其次，开发了 SheetCompressor 创新编码框架，包含结构锚点压缩、逆向索引转换和数据格式感知聚合三个模块。最后，为下游任务提出了“电子表格链 (Chain of Spreadsheet)”。

Result: SheetCompressor 在 GPT4 的上下文学习中，使电子表格表格检测任务的性能比传统方法提高了 25.6%。经过 SheetCompressor 微调的 LLM 平均压缩比达到 25 倍，F1 分数达到 78.9% 的最新水平，超越现有最佳模型 12.3%。在新的电子表格问答任务中也得到了验证。

Conclusion: SpreadsheetLLM 通过利用电子表格固有的布局和结构，在各种电子表格任务中表现出高效性。
...
### [67] [SheetAgent: Towards A Generalist Agent for Spreadsheet Reasoning and Manipulation via Large Language Models](https://arxiv.org/abs/2403.03636)
*Yibin Chen,Yifu Yuan,Zeyu Zhang,Yan Zheng,Jinyi Liu,Fei Ni,Jianye Hao,Hangyu Mao,Fuzheng Zhang*

Main category: cs.AI

TL;DR: 该论文介绍了SheetRM，一个用于复杂电子表格任务的基准，以及SheetAgent，一个基于LLM的自主代理来解决这些任务，并在性能上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM在电子表格操作方面尚未在存在推理挑战（例如，长周期、多步骤推理和模糊要求）的复杂现实任务中进行过研究。本文旨在弥补这一差距。

Method: 本文提出了SheetAgent，一个利用LLM的自主代理，包含规划器、信息员和检索器三个协作模块。它通过迭代任务推理和反思，在无需人工干预的情况下实现高级推理和精确的电子表格操作。同时引入了SheetRM，一个用于长周期和多类别任务的基准，这些任务的操纵依赖于推理。

Result: SheetAgent在多个基准测试中，通过率比基线提高了20-40%，在电子表格操作中实现了更高的精度，并展示了卓越的表格推理能力。

Conclusion: SheetAgent通过开发一个自主的、基于LLM的代理来解决复杂的现实世界挑战，显著改善了电子表格操作和表格推理能力。
...
### [68] [Large Language Model for Table Processing: A Survey](https://arxiv.org/abs/2402.05121)
*Weizheng Lu,Jing Zhang,Ju Fan,Zihao Fu,Yueguo Chen,Xiaoyong Du*

Main category: cs.AI

TL;DR: 本文综述了LLM和VLM在表格处理相关任务中的应用，涵盖了传统及新兴任务、训练技术、提示工程，并讨论了面临的挑战。


<details>
  <summary>Details</summary>
Motivation: 表格是日常活动中的关键组成部分，使用大型语言模型（LLMs）或视觉语言模型（VLMs）自动化这些以表格为中心的任务具有重大的公共利益，并受到了学术界和工业界的关注。

Method: 本文通过提供一份全面的综述，审查用户场景和技术方面，涵盖传统任务（如表格问答）以及新兴领域（如电子表格操作和表格数据分析），总结LLMs和VLMs的训练技术，并讨论提示工程，特别是LLM驱动代理的应用。

Result: 综述涵盖了表格相关任务的全面概述，包括表格问答、电子表格操作、表格数据分析，总结了LLMs和VLMs的训练技术，并讨论了针对各种表格相关任务的提示工程（特别是LLM驱动代理的使用）。

Conclusion: 文章强调了若干挑战，包括服务时多样的用户输入和使用思维链时的缓慢思考。
...
### [69] [FormaT5: Abstention and Examples for Conditional Table Formatting with Natural Language](https://arxiv.org/abs/2310.17306)
*Mukul Singh,José Cambronero,Sumit Gulwani,Vu Le,Carina Negreanu,Elnaz Nouri,Mohammad Raza,Gust Verbruggen*

Main category: cs.AI

TL;DR: FormaT5是一个基于Transformer的模型，能够根据自然语言描述和表格数据生成条件格式化规则。它通过弃权目标（abstention objective）和两步处理来解决描述不明确的问题，在新基准测试上优于其他神经方法。


<details>
  <summary>Details</summary>
Motivation: 在电子表格软件中编写依赖数据的条件格式化（CF）规则对用户来说往往具有挑战性，因为它需要理解和实现底层逻辑。

Method: FormaT5是一个基于Transformer的模型，给定目标表格和所需格式化逻辑的自然语言描述，它可以生成CF规则。为了解决描述不明确或模糊的问题，FormaT5通过弃权目标来预测占位符。这些占位符可以由第二个模型填充，或者在有示例行的情况下，由编程示例系统填充。

Result: FormaT5通过弃权和填充的方式，在包含1053个CF任务的广泛基准测试中，无论有无示例，都优于8种不同的神经方法。

Conclusion: 结果表明了构建领域特定学习系统的价值。
...
### [70] [Don't Treat the Symptom, Find the Cause! Efficient Artificial-Intelligence Methods for (Interactive) Debugging](https://arxiv.org/abs/2306.12850)
*Patrick Rodler*

Main category: cs.AI

TL;DR: 现代系统日益复杂且依赖性高，故障可能造成严重后果。模型诊断学是一种通用的、领域无关的方法，旨在检测、定位并修复系统故障，以最小化损害。


<details>
  <summary>Details</summary>
Motivation: 现代系统（如汽车、推荐系统、电网、金融软件等）复杂性高，人类对其依赖性强，导致系统故障的可能性及负面影响巨大。因此，将故障损害降至最低，包括减少停机时间和修复成本，是核心需求。

Method: 模型诊断学是一种通用的、领域无关的方法，可应用于各种系统故障排除。它利用知识表示、自动化推理、启发式求解、智能搜索、优化、随机统计、不确定性决策、机器学习、微积分、组合论和集合论等多种技术，进行故障检测、定位和修复。本论文将介绍该领域、指出主要挑战，并讨论作者研究中解决这些问题的方法。

Result: 本论文将介绍模型诊断学的主要挑战，并讨论作者研究中解决这些问题的一些方法。（该抽象未提供具体的实验结果或发现，而是阐述了论文将涵盖的内容。）

Conclusion: 模型诊断学是一个原理性、领域无关的关键方法，能有效应对复杂系统中故障检测、定位与修复的挑战，以最小化故障损害。本论文旨在为该领域提供介绍并展示解决方案。
...
### [71] [CORNET: Learning Table Formatting Rules By Example](https://arxiv.org/abs/2208.06032)
*Mukul Singh,José Cambronero,Sumit Gulwani,Vu Le,Carina Negreanu,Mohammad Raza,Gust Verbruggen*

Main category: cs.AI

TL;DR: CORNET是一个新系统，它利用归纳编程和神经网络从用户示例中自动学习电子表格的条件格式规则。


<details>
  <summary>Details</summary>
Motivation: 用户编写电子表格的格式规则很困难，因为它需要了解规则语言和数据逻辑。

Method: CORNET系统结合了符号规则枚举和神经排序器，从格式化单元格的用户示例中学习条件格式规则。该方法通过从180万个真实工作表中提取的45万个独特格式规则的语料库进行评估，并与现有基线进行比较。

Result: CORNET在不同评估设置下能准确学习规则。它还能找到比用户手写规则更短的规则，并发现用户手动格式化的电子表格中的规则。

Conclusion: CORNET成功解决了自动学习电子表格条件格式规则的挑战，提高了用户效率并简化了格式化过程。
...
### [72] [Named Entity Recognition in Industrial Tables using Tabular Language Models](https://arxiv.org/abs/2209.14812)
*Aneta Koleva,Martin Ringsquandl,Mark Buckley,Rakebul Hasan,Volker Tresp*

Main category: cs.AI

TL;DR: 该研究探索了如何将表格Transformer模型应用于工业NER问题，并开发了一种基于知识图谱的数据增强策略，证明了表格结构归纳偏置对于模型性能的重要性。


<details>
  <summary>Details</summary>
Motivation: 尽管表格Transformer模型在学术界受到关注，但其在工业界的应用仍旧缺乏。本文旨在解决工业场景下表格结构化电子表格中的命名实体识别（NER）问题，尤其是在缺乏标记数据和电子表格技术复杂性带来的挑战下。

Method: 1. 应用表格Transformer模型解决工业NER问题。2. 针对低资源场景和缺乏标注数据的问题，开发了一种基于领域特定知识图谱的专用表格数据增强策略。3. 比较了将表格结构作为归纳偏置和将表格作为线性序列的益处。

Result: 1. 开发的数据增强策略显著提升了模型在低资源场景下的性能。2. 表格Transformer模型优于其他基线模型。3. 表格结构的归纳偏置对于Transformer模型的收敛至关重要。

Conclusion: 表格Transformer模型及其表格结构归纳偏置在处理工业表格NER问题中表现出色，尤其是在数据稀缺的情况下，结合领域知识图谱进行数据增强是有效途径。
...
### [73] [Spreadsheet computing with Finite Domain Constraint Enhancements](https://arxiv.org/abs/2203.10944)
*Ezana N. Beyenne*

Main category: cs.AI

TL;DR: 本文扩展了电子表格计算范式，通过集成有限约束求解器来解决约束满足问题。它允许单元格附带有限域或约束，并提供电子表格特有的约束以提高大型应用的扩展性。


<details>
  <summary>Details</summary>
Motivation: 电子表格应用因其易用性而广受欢迎，但受限于单向数据流，无法解决约束满足问题。

Method: 本文提出一个框架，将有限约束求解器无缝集成到电子表格计算范式中。该框架允许单元格绑定有限域或定义单元格间关系的约束，并提供约束求解接口及一套用于控制大型电子表格应用可扩展性的特定约束。

Result: 该框架成功克服了电子表格的局限性，使其能够解决约束满足问题，并通过提供特定的约束来增强其可扩展性。文中提供了示例来证明其可用性和有效性。

Conclusion: 扩展后的电子表格范式成功克服了传统电子表格的局限性，使其能够解决约束满足问题，并通过新的约束提升了其可扩展性。
...
### [74] [Human-Machine Collaboration for Democratizing Data Science](https://arxiv.org/abs/2004.11113)
*Clément Gautrais,Yann Dauxais,Stefano Teso,Samuel Kolb,Gust Verbruggen,Luc De Raedt*

Main category: cs.AI

TL;DR: 这篇论文介绍了一个名为\textsc{VisualSynth}的新型人机协作框架和系统，旨在通过允许用户在标准电子表格软件中通过颜色标记（Colored Sketches）来执行和自动化各种数据分析任务，从而实现数据科学的普及化。


<details>
  <summary>Details</summary>
Motivation: 只有少数人拥有数据科学专业知识，但每个人都希望分析他们的数据。基于这一观察，作者引入了\textsc{VisualSynth}系统以实现数据科学的民主化。

Method: 用户通过提供彩色草图（即对电子表格的某些部分进行着色）来部分指定数据科学任务，然后系统利用人工智能技术确定并执行这些任务。

Result: 该系统允许用户与标准电子表格软件交互，以执行和自动化各种数据分析任务，包括数据整理、数据选择、聚类、约束学习、预测建模和自动完成。

Conclusion: \textsc{VisualSynth}旨在通过直观的电子表格交互和AI技术，使不具备数据科学专业知识的用户也能进行数据分析，从而实现数据科学的民主化。
...
### [75] [Automated Discovery of Data Transformations for Robotic Process Automation](https://arxiv.org/abs/2001.01007)
*Volodymyr Leno,Marlon Dumas,Marcello La Rosa,Fabrizio Maria Maggi,Artem Polyvyanyy*

Main category: cs.AI

TL;DR: 该论文提出了一种优化方法，用于从用户界面（UI）日志中发现可自动化的数据传输例程，特别关注电子表格或Web表单之间的数据传输，通过将其映射到数据转换发现并优化现有技术。


<details>
  <summary>Details</summary>
Motivation: 公司需要发现哪些特定例程可以通过机器人流程自动化（RPA）进行自动化，以及如何自动化。本文旨在解决分析用户界面（UI）日志的问题，以发现用户将数据从一个电子表格或（Web）表单传输到另一个的例程。

Method: 将问题映射到通过示例发现数据转换。指出直接应用现有最先进的数据转换发现技术效率低下。提出了两种优化方法：1. 利用UI日志中的信息。2. 利用跨应用程序数据传输通常涉及分别复制字母和数字令牌的事实。

Result: 所提出的方法及其优化措施使用复制真实世界重复数据传输例程的UI日志进行了评估。

Conclusion: 本文提出了一种优化的方法，通过利用UI日志信息和数据传输的特定特征，发现适合RPA的数据传输例程，解决了朴素方法计算效率低下的问题。
...
### [76] [Neural-Guided Deductive Search for Real-Time Program Synthesis from Examples](https://arxiv.org/abs/1804.01186)
*Ashwin Kalyan,Abhishek Mohta,Oleksandr Polozov,Dhruv Batra,Prateek Jain,Sumit Gulwani*

Main category: cs.AI

TL;DR: 本文提出了神经引导演绎搜索（NGDS），一种混合程序合成技术，它结合了符号逻辑和统计模型的优点，能够从少量输入-输出示例中高效、准确地合成程序。


<details>
  <summary>Details</summary>
Motivation: 从少量输入-输出示例合成用户意图程序是一个具有挑战性的问题。现有合成系统要么依赖人工设计的演绎逻辑，要么依赖需要大量数据的统计模型，通常无法在复杂基准上提供实时合成。

Method: 本文提出了神经引导演绎搜索（NGDS），这是一种结合了符号逻辑技术和统计模型的混合合成技术。它利用演绎搜索框架将神经组件的学习问题简化为监督学习设置，从而可以在少量真实世界数据上进行训练并利用强大的循环神经网络编码器。

Result: 该方法在真实客户场景中进行了评估，能够合成准确的程序，并且相比最先进的系统，速度提升高达12倍。

Conclusion: NGDS通过结合符号逻辑和统计模型，提供了一种高效、准确的程序合成方法，解决了现有系统在实时合成和数据依赖方面的局限性。
...
TL;DR: Excel已从简单的分类账自动化工具演变为用于分析的集成开发环境（IDE），因此需要扩展当前的风险框架来管理其日益增长的风险。


<details>
  <summary>Details</summary>
Motivation: 许多人尚未意识到Excel已发展成为一个集成了数据库、OLAP引擎、统计编程语言等多种工具的分析IDE。这种转变使得建立一个全面的风险管理框架变得至关重要。

Method: 本文将解释如何扩展当前针对电子表格的风险框架，以管理将Excel用作分析IDE所带来的日益增长的风险。

Result: 通过认识到Excel作为分析IDE的转变，并阐明需要扩展的风险管理框架。

Conclusion: Excel作为分析IDE的新角色要求我们超越传统的电子表格风险管理，建立一个更全面、更具包容性的风险框架。
...
TL;DR: 该论文描述了一个智能辅导系统（ITS），用于自动评估Excel作业并提供个性化反馈。


<details>
  <summary>Details</summary>
Motivation: 尽管电子表格被广泛使用并纳入课程，但用于评估电子表格作业的数字化解决方案在教学环境中仍然稀缺。

Method: 开发了一个ITS，通过值匹配、详细的公式分析和解决方案的质量评估来审查学生的Excel提交。它根据学生的学习水平提供不同信息级别的个性化反馈，讲师只需提供一个参考解决方案。

Result: 更高层次的反馈能提高正确提交的百分比，并被学生认为是易于理解和有帮助的。

Conclusion: 所开发的ITS能有效评估Excel作业并提供有益的反馈，从而改善学生的学习成果。
...
TL;DR: 本文提出了一种新颖的方法，旨在将计算思维和数字问题解决能力融入教育，以蒲福风级为例，并证明与传统方法相比，该方法能更有效地提高学生的学习效果。


<details>
  <summary>Details</summary>
Motivation: 计算思维应成为一项基本技能。需要发展数字问题解决的图式。传统方法在培养数字技能方面效率较低。

Method: 详细介绍了一种支持学科整合和数字图式构建的新颖方法，并将其应用于蒲福风级。在八年级行动研究框架内，将传统的纸质问题转换为数据检索过程。

Result: 学生的内容知识和数字技能的发展比在传统教材和脱离语境的数字环境中更有效。

Conclusion: 本文提出的方法可以适用于任何在数字环境中解决会更有效，并且在学科和信息学中都能提供多种形式来构建图式的纸质问题。
...
TL;DR: SAM是一种新的简化方法，用于衡量紧急避难所客户的脆弱性。它能帮助非技术人员理解避难所的使用模式，并能实时反映外部因素（如“住房优先”计划和COVID-19封锁）的影响，其效果与传统聚类分析相似。


<details>
  <summary>Details</summary>
Motivation: 为避难所运营者提供一种直观、易于非技术人员操作的方法，以理解避难所使用模式并评估客户的脆弱性。

Method: 引入并应用简化访问度量（SAM）方法。使用北美一家大型避难所的客户数据进行验证，并将其结果与传统的过渡性、偶发性和长期性客户聚类分析进行比较。通过九年的避难所客户数据生成时间线，以展示外部因素的影响。

Result: SAM产生了与传统聚类分析相似的结果，但所需数据更少，能够实时描绘避难所访问模式如何受外部因素影响。使用SAM分析九年数据的时间线展示了“住房优先”计划和COVID-19封锁对人们使用避难所方式的影响。SAM的“软”输出可以直接作为脆弱性衡量标准，取代传统的标签分配。

Conclusion: SAM是一种简化且有效的避难所访问模式分析方法，可以作为衡量客户脆弱性的直接指标。它易于实施，提供实时洞察，并超越了传统的客户分类标签。
...
TL;DR: 本文介绍了PL和CA社区为解决疫情期间建立持久联系的困难而创建的两个长期指导项目SIGPLAN-M和CALM的经验和影响。


<details>
  <summary>Details</summary>
Motivation: 编程语言（PL）和计算机体系结构（CA）社区的领导者意识到，在社区中建立新的持久联系的唯一方法是已经拥有持久的联系，而现有的短期指导项目无法满足这一需求。疫情进一步凸显了对长期指导项目的需求。

Method: PL社区的一位研究人员启动了一个非官方的长期指导项目，该项目后来发展成为官方的跨机构项目SIGPLAN-M。同时，CA社区以科学方法提出了一个有证据支持的社区范围长期指导项目，最终形成了CALM。

Result: SIGPLAN-M已发展到覆盖41个国家的328名受指导者和234名指导者，并获得了“改变生活”和“职业救星”的高度评价。CALM项目虽然处于试点阶段，但已获得非常积极的反馈。

Conclusion: 作者分享了这些长期指导项目的设计、影响和挑战，希望能推动计算机科学领域更大范围的长期指导工作。
...
TL;DR: 多伦多一家长期护理机构开发了一种基于电子表格的排班工具，该工具采用分层优化模型，旨在自动化护士排班、减少兼职护士缺勤并纳入其偏好。该工具显著缩短了排班时间，并成功满足了94%以上的护士偏好。


<details>
  <summary>Details</summary>
Motivation: 为了解决多伦多长期护理机构护士排班日益严峻的挑战，并降低兼职护士的高缺勤率。

Method: 开发了一个基于电子表格的排班工具，其核心是一个分层优化模型。该模型在满足最大可能需求的同时，生成具有最高总偏好得分的可行班次表，并遵循复杂的资历要求。

Result: 该工具在多伦多一家拥有391张床位的护理机构中实施。与手动排班（需要数十小时）相比，护士经理可以在不到一小时内生成可行的班次表。此外，班次表成功考虑了偏好，平均94%以上的分配班次被评为最受欢迎。

Conclusion: 该排班工具成功实现了护士排班的自动化，有效整合了护士偏好和资历要求，显著提高了排班效率和护士满意度。
...
TL;DR: 这篇论文旨在回顾ICAEW关于电子表格使用和工作环境最佳实践的三份“思想领导力”报告，探讨其历史、主要经验教训，并分析这些报告的制定如何帮助ICAEW在该领域发展其地位。


<details>
  <summary>Details</summary>
Motivation: 动机在于深入了解ICAEW在五年内发布的关于电子表格使用和工作环境最佳实践的三份“思想领导力”报告，包括它们的演变历程、各自的核心教训，并评估这些报告的发布对ICAEW在该领域地位巩固与发展的影响。

Method: 本文采用回顾性分析方法，对英国特许会计师协会（ICAEW）此前发布的三份关于电子表格使用最佳实践的“思想领导力”报告进行系统性审视。

Result: 本文将阐明这三份报告的详细发展历史、它们各自传达的关键经验教训，并揭示这些报告的制定过程如何助力ICAEW在该领域建立并强化其专业地位。

Conclusion: 结论将概括这三份报告对电子表格最佳实践的贡献，以及ICAEW通过这些努力在专业领域所取得的进展和定位。
...
TL;DR: 本案例研究调查了英国一所高等教育机构的电子表格使用情况，分析了其重要性、培训、用途和共享等方面，并指出需要明确的指导方针以确保数据完整性和效率。


<details>
  <summary>Details</summary>
Motivation: 了解英国一所高等教育机构（特别是学术注册和财务部门）的电子表格使用情况及其对准确报告、数据完整性和效率的影响。

Method: 采用案例研究方法，从重要性、培训、经验、目的、部署技术、创建的电子表格大小和共享等方面探讨电子表格的使用。

Result: 电子表格的创建和使用量很大，开发人员的特征与其他研究相似。机构需要明确的原则和指导方针来开发电子表格模型，以确保数据完整性、减少重复工作并优化电子表格使用。

Conclusion: 该机构需要为电子表格开发制定明确的指导方针，以确保数据完整性、减少重复并优化其使用以实现机构目标。
...
TL;DR: 本文强调了在电子表格环境中开发神经网络计算机模拟训练方法的必要性，回顾了现有方法，并指出了在电子表格中掌握神经网络模拟技能所需掌握的计算神经科学的关键历史模型和人物。


<details>
  <summary>Details</summary>
Motivation: 论证在电子表格环境中开发神经网络计算机模拟训练方法的必要性。

Method: 对电子表格应用于模拟人工神经网络的系统回顾，识别了基本方法；分析了1890-1950年的文献，以确定《数学生物物理学公报》及其社区在计算神经科学模型和方法创建中的作用；识别了创建神经网络的心理物理学基础、神经计算的数学基础和神经工程方法；讨论了Walter Pitts在结合描述性和定量训练理论中的作用；指出掌握基于历史和遗传方法论的模型对于在电子表格环境中获得神经模拟能力至关重要；提出了Rashevsky的连续双因素模型、McCulloch和Pitts的离散模型以及Householder和Landahl的离散-连续模型这三组有前景的模型。

Result: 确定了在电子表格中进行神经网络计算机模拟训练的基本方法；确定了《数学生物物理学公报》及其社区在计算神经科学发展中的作用；识别了心理物理学基础、数学基础和神经工程方法；讨论了Walter Pitts的贡献；证明了历史-遗传模型对于电子表格神经模拟能力至关重要；识别了三组有前景的模型（Rashevsky、McCulloch-Pitts、Householder-Landahl）。

Conclusion: 为了在电子表格环境中掌握神经网络模拟能力，应掌握基于历史和遗传方法的模型，特别是Rashevsky的连续双因素模型、McCulloch和Pitts的离散模型以及Householder和Landahl的离散-连续模型，并理解1890-1950年计算神经科学的历史发展。
...
TL;DR: 本文介绍了面向金融专业人士的电子表格能力框架的教育版（E2SCF），主张尽早通过专家教师在教育中培养电子表格能力。E2SCF 的主要特点是高数学性计算机支持的真实世界问题解决，方法基于双向知识迁移、数据和错误分析与处理以及电子表格的编程方面，旨在帮助基础和普通用户建立坚实的电子表格知识并发展可迁移的问题解决技能和能力。


<details>
  <summary>Details</summary>
Motivation: 应尽早开始在教育中培养电子表格能力，并且在专家教师的支持下，这一过程会更有效。

Method: E2SCF 的方法是基于高数学性计算机支持的真实世界问题解决，从培训一开始就关注双向知识迁移、数据和错误分析与处理以及电子表格的编程方面。

Result: E2SCF 旨在帮助基础和普通用户建立坚实的电子表格知识并发展可迁移的问题解决技能和能力。

Conclusion: E2SCF 专注于早期教育、专家支持、真实世界问题解决和编程方面，旨在为基础和普通用户建立强大的电子表格知识和可迁移的问题解决技能。
...
TL;DR: 本文探讨了电子表格技术的未来，考虑了大数据、终端用户计算和移动计算等因素。它根据一个研讨会，讨论了电子表格成功的原因、驱动变化的趋势、未来的发展方向以及对研究的影响。


<details>
  <summary>Details</summary>
Motivation: 探讨电子表格技术在大数据、终端用户计算和移动计算等新驱动因素影响下的未来发展方向。

Method: 召开了一个研讨会，汇集了学术界和工业界的观点，探讨电子表格技术的未来发展方向及其对用户的影响。本文记录了参与者的观点。

Result: 记录了参与者对电子表格成功原因、驱动变化的趋势以及电子表格可能的发展方向的看法。文章还提出了电子表格演进和使用方面的关键研究方向。

Conclusion: 探讨了这些趋势对终端用户的影响，因为终端用户是电子表格取得显著成功的原因。
...
TL;DR: 本文基于对Medal网站CDSS用户的调查，分析了CDSS对临床实践的影响和使用情况。


<details>
  <summary>Details</summary>
Motivation: 旨在加深对临床决策支持系统（CDSS）如何影响临床实践的理解。

Method: 对来自拥有24,000个不同CDSS的Medal公司网站的CDSS用户进行了一项小型范围调查，并对结果进行了分析、讨论和与其他类似研究的比较。CDSS的初始逻辑在电子表格上制定，然后实现在数据库中执行。

Result: 结果显示，Medal提供的CDSS被临床专业人员在各种环境中用作操作工具、研究工具和参考工具。

Conclusion: 该研究通过调查结果，描述了CDSS的实际使用情况及其对临床实践的影响，并发现CDSS在多种临床场景中发挥着重要作用。
...
### [89] [Using the Abstract Computer Architecture Description Language to Model AI Hardware Accelerators](https://arxiv.org/abs/2402.00069)
*Mika Markus Müller,Alexander Richard Manfred Borst,Konstantin Lübeck,Alexander Louis-Ferdinand Jung,Oliver Bringmann*

Main category: cs.AR

TL;DR: 本文介绍了一种抽象计算机架构描述语言（ACADL），用于建模AI硬件加速器、将深度神经网络（DNNs）映射到这些加速器上并进行性能仿真，以解决实际应用中选择和比较加速器的挑战。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络（DNNs）的快速发展推动了对专用AI硬件加速器的需求。然而，制造商在选择和配置最佳加速器时面临挑战，因为现有方法复杂且只能提供粗略的性能理解。

Method: 本文提出使用抽象计算机架构描述语言（ACADL）对AI硬件加速器进行建模，利用ACADL描述将DNNs映射到加速器上，并通过解释时序仿真语义来获取性能结果。

Result: 本文展示了如何使用ACADL来建模AI硬件加速器、利用其ACADL描述将DNNs映射到加速器上，并解释了用于收集性能结果的时序仿真语义，从而提供了对加速器特性的清晰理解。

Conclusion: ACADL提供了一种简洁而正式的方法来建模AI硬件加速器、映射DNNs并模拟性能，从而简化了比较不同加速器设计方案的复杂任务，并有助于做出明智的选择。
...
### [90] [Online Model Swapping in Architectural Simulation](https://arxiv.org/abs/2012.01571)
*Patrick Lavin,Jeffrey Young,Rich Vuduc,Jonathan Beard*

Main category: cs.AR

TL;DR: 该论文提出了一种在线方法，通过自动将详细的模拟模型替换为更简单的统计近似模型，以弥合简单模拟与详细模拟之间的差距。这种方法在模拟周期计数中仅引入了8%的误差，但在90%以上的模拟时间内使用了近似模型，并将每个模型的计算量减少了2到8倍，从而显著提高了模拟速度。


<details>
  <summary>Details</summary>
Motivation: 随着系统和应用日益复杂，详细模拟耗时越来越长，导致设计迭代变慢。这迫使架构师在快速迭代时使用电子表格等简单模型。然而，从简单模型迁移到详细模型通常需要多次执行来确定简单模型的有效性，这可能比直接运行详细模型更昂贵。此外，架构师通常需要依靠直觉来选择这些简单的模型，使问题进一步复杂化。

Method: 该方法通过在线监控模拟行为，并自动将详细模型替换为更简单的统计近似模型，来弥合简单模拟与详细模拟之间的差距。作者在开源模拟器SVE-Cachesim中实现了该方法，用于替换内存层次结构中的一级数据缓存（L1D）。

Result: 该概念验证表明，该技术不仅可以处理局部时间不变统计量的近似，还可以处理随时间变化的统计量（例如，L1D是一种时间序列函数）以及下游副作用（例如，L1D过滤二级缓存的访问）。在模拟的周期计数中仅产生了8%的误差，而近似缓存模型在90%以上的模拟中被使用，并且更简单的模型每次“执行”所需的计算量减少了2到8倍。

Conclusion: 该方法有效地弥合了简单模拟与详细模拟之间的差距，实现了显著的计算量减少和可接受的精度，特别是对于复杂、时变的组件如缓存。
...
### [91] [Is spreadsheet syntax better than numeric indexing for cell selection?](https://arxiv.org/abs/2505.23296)
*Philip Heltweg,Dirk Riehle,Georg-Daniel Schwarz*

Main category: cs.PL

TL;DR: 该研究比较了数据工程中选择单元格的两种方法：数字索引和电子表格式语法。结果表明，电子表格式语法在阅读和编写代码时能减少错误并提高速度，对于没有软件工程背景的数据从业者来说，这是一种有前景的替代方案。


<details>
  <summary>Details</summary>
Motivation: 在数据工程中选择单元格子集时，存在多种表达方式，例如数字索引和电子表格式语法。本研究旨在比较这两种选项在阅读和编写代码时的速度和正确性。

Method: 通过一项大规模的受控实验，以学生参与者作为数据从业者的代表，比较了数字索引和电子表格式语法这两种单元格选择方式。

Result: 实验结果显示，在阅读代码时，参与者使用电子表格式语法犯的错误更少。此外，在编写代码时，相比于数字语法，使用电子表格语法时，他们犯的错误更少且速度更快。

Conclusion: 因此，对于数据工程而言，领域特定语法（如电子表格语法）似乎是一种有前景的替代方案，值得在未来的工具中探索，以支持没有软件工程背景的从业者。
...
### [92] [Solving Data-centric Tasks using Large Language Models](https://arxiv.org/abs/2402.11734)
...
TL;DR: 大型语言模型在数据中心任务中需要包含数据提示才能更好地工作。本文创建了一个数据集，并提出了一种“先聚类后选择”的提示技术，用于向大型语言模型提示中添加最具代表性的数据，实验证明该技术能提高大型语言模型在此类任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型正在取代StackOverflow等帮助论坛，特别对非专业程序员和终端用户很有帮助。这些用户通常对数据中心任务（如电子表格操作和数据整理）感兴趣，如果只使用自然语言描述而不包含数据，这些任务很难解决。但关键问题是：我们应该在提示中包含多少数据以及哪些数据？

Method: 1. 创建了一个从StackOverflow帖子中挖掘的真实世界自然语言到代码的表格数据操作任务数据集。2. 引入了一种“先聚类后选择”的提示技术，该技术将输入数据中最具代表性的行添加到大型语言模型的提示中。

Result: 实验表明，大型语言模型的性能确实对提示中传递的数据量敏感。对于输入表中存在大量语法变异的任务，我们提出的“先聚类后选择”技术优于随机选择基线。

Conclusion: 本文回答了在大型语言模型提示中包含多少数据以及哪些数据的问题，通过创建数据集并提出“先聚类后选择”的提示技术，显著提高了大型语言模型在处理数据中心任务时的性能，尤其是在数据存在较大变异时。
...
### [93] [FLAME: A small language model for spreadsheet formulas](https://arxiv.org/abs/2301.13779)
*Harshit Joshi,Abishai Ebenezer,José Cambronero,Sumit Gulwani,Aditya Kanade,Vu Le,Ivan Radiček,Gust Verbruggen*

Main category: cs.PL

TL;DR: FLAME是一个小型高效的Transformer模型，专门用于Excel公式辅助编写，性能优于大型通用模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在电子表格公式编写辅助方面训练成本高昂且难以部署，因为它们规模庞大。

Method: FLAME是一个基于Transformer的模型，专门使用Excel公式进行训练。其方法包括：使用草图去重（sketch deduplication）整理训练数据集、引入Excel特定的公式分词器，以及使用领域特定的掩码跨度预测（masked span prediction）和噪声自动编码（noisy auto-encoding）作为预训练目标。

Result: FLAME（60M参数）在显著更小且使用少两个数量级的数据量进行训练的情况下，实现了具有竞争力的性能。在公式修复和补全任务的14个评估设置中，FLAME在10个方面超越了Davinci (175B)、Cushman (12B) 和 CodeT5 (220M) 等更大的模型。在公式检索方面，FLAME优于CodeT5、CodeBERT和GraphCodeBERT。

Conclusion: FLAME证明了针对特定领域训练的小型模型，在电子表格公式任务上可以比大型通用模型取得更优越的性能，提供了一种更高效和易于部署的解决方案。
...
### [94] [Enhanced Spreadsheet Computing with Finite-Domain Constraint Satisfaction](https://arxiv.org/abs/2203.16346)
*Ezana N. Beyenne,Hai-Feng Guo*

Main category: cs.PL

TL;DR: 该论文扩展了电子表格计算范式，以解决其单向数据流的限制，使其能够解决约束满足问题，并为此引入了一个支持有限域约束求解的增强型电子表格系统和一种专用的约束语言。


<details>
  <summary>Details</summary>
Motivation: 电子表格是广泛使用的计算工具，但其单向数据流限制了其应用，主要限于簿记类任务，无法解决更复杂的约束满足问题。

Method: 论文提出了一种增强型电子表格系统，该系统在一个可视化环境中良好支持有限域约束求解。此外，还构建了一种电子表格专用约束语言，供普通用户以声明式和可伸缩的方式指定数据单元格之间的约束。

Result: 新的电子表格系统显著简化了使用可视化表格界面开发许多基于约束的应用程序。论文提供了示例来阐明扩展电子表格范例的可用性和实用性。

Conclusion: 通过扩展电子表格范式以支持约束满足问题，本研究显著提升了电子表格在开发基于约束的应用方面的可用性和实用性。
...
### [95] [Typed Image-based Programming with Structure Editing](https://arxiv.org/abs/2110.08993)
*Jonathan Edwards,Tomas Petricek*

Main category: cs.PL

TL;DR: 本文提出通过类型和结构编辑在基于图像的编程中实现协作，特别关注持久数据的模式变更问题。


<details>
  <summary>Details</summary>
Motivation: 基于图像的编程系统（如Smalltalk）在协作和部署方面表现不佳，限制了其商业成功。

Method: 利用静态类型来表达和执行模式变更，并通过结构编辑来捕获类型定义的变化，从而自动调整数据。论文提出了一个实现结构编辑版本控制的理论。

Result: 主要技术贡献是提出了一个实现结构编辑版本控制的理论，目前侧重于类型编辑。
...
### [96] [ExceLint: Automatically Finding Spreadsheet Formula Errors](https://arxiv.org/abs/1901.11100)
*Daniel W. Barowy,Emery D. Berger,Benjamin Zorn*

Main category: cs.PL

TL;DR: 本文介绍了ExceLint，一个用于发现Microsoft Excel电子表格公式错误的静态分析工具，该工具快速、有效，并且优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 电子表格被广泛使用，尤其是在金融等错误可能导致灾难性后果的领域。因此，需要找到电子表格公式中的错误。

Method: 我们的静态分析专门设计用于查找电子表格公式错误，直接利用电子表格的矩形特性。它使用信息论方法来识别对附近矩形区域造成特别令人惊讶的干扰的公式。

Result: ExceLint是Microsoft Excel的实现，它快速有效。在70个电子表格的语料库中，ExceLint每个电子表格的平均耗时为5秒，并且显著优于最先进的分析。

Conclusion: ExceLint为识别电子表格公式错误提供了一个快速有效的解决方案，改进了现有的静态分析方法。
...
### [97] [Synthesizing Bijective Lenses](https://arxiv.org/abs/1710.03248)
*Anders Miltner,Kathleen Fisher,Benjamin C. Pierce,David Walker,Steve Zdancewic*

Main category: cs.PL

TL;DR: Optician是一个通过类型指导合成双射字符串转换器的工具，它使用两个正则表达式和少量示例作为输入，输出一个Boomerang程序，旨在解决手动构建双向转换的繁琐和现有领域特定语言编程困难的问题。


<details>
  <summary>Details</summary>
Motivation: 在现代软件系统中，不同数据表示之间的双向转换很常见，但手动构建是繁琐且容易出错的。使用领域特定语言是更好的方法，但这些语言编程困难，需要管理复杂的类型系统中的细节。

Method: 本文提出了Optician，一个用于类型指导合成双射字符串转换器的工具。输入是两个表示数据格式的普通正则表达式和一些具体示例。输出是Boomerang语言（基于Lenses理论的双向语言）中的一个类型良好的程序。主要技术挑战在于高效地导航巨大的程序搜索空间。该系统在具有丰富类型等价关系（正则表达式理论）的语言上下文中运行。它合成等价语言的术语，并将其转换为Lenses语言。

Result: 该合成算法被证明是正确的。经验表明，新的语言将合成问题从难以处理的解决方案转变为高效的解决方案。Optician在包含39个示例的基准测试套件上进行了评估，其中包括微基准测试和来自其他数据管理系统（如Flash Fill和Augeas）的实际示例。

Conclusion: Optician成功地通过提供一个高效且正确的类型指导合成工具，解决了构建双向转换的挑战，该工具可以从正则表达式和示例生成Boomerang程序。
...
### [98] [Active Learning of Input Grammars](https://arxiv.org/abs/1708.08731)
*Matthias Höschele,Alexander Kampmann,Andreas Zeller*

Main category: cs.PL

TL;DR: 为了进行系统测试，了解程序输入的精确格式是必要的。本文提出了AUTOGRAM，一种仅需少量样本输入即可推断上下文无关语法的方法，以描述有效输入结构。它为URL、电子表格或配置文件等各种输入生成准确且可读的语法，从而实现全面的自动化测试。


<details>
  <summary>Details</summary>
Motivation: 进行系统测试需要精确了解程序输入格式。

Method: 该方法通过以下步骤实现：1) 跟踪输入数据流，将具有相同数据流的输入片段聚合成词法和句法实体；2) 根据相关的变量和函数标识符为这些实体命名；3) 通过成员查询系统地概括生成规则。该方法原型名为AUTOGRAM。

Result: 仅需最少的样本输入即可获得反映有效输入结构的、人类可读的上下文无关语法。AUTOGRAM原型在URL、电子表格或配置文件等输入上的评估表明，其获得的输入语法既准确又非常可读。

Conclusion: AUTOGRAM生成的语法可以直接输入到测试生成器中，用于全面的自动化测试。
...
### [99] [Synthesis of Data Completion Scripts using Finite Tree Automata](https://arxiv.org/abs/1707.01469)
*Xinyu Wang,Isil Dillig,Rishabh Singh*

Main category: cs.PL

TL;DR: 本文提出了一种基于编程示例（PBE）和轻量级草图的合成技术，通过使用结合空间和关系推理的领域特定语言（DSL）以及基于有限树自动机（FTA）的新型版本空间学习算法，自动化表格数据中的数据补全任务。该方法已在DACE工具中实现并验证，优于现有合成器。


<details>
  <summary>Details</summary>
Motivation: 在表格数据应用领域，终端用户和数据科学家在执行需要非凡编程专业知识的数据补全任务（如缺失值插补和派生数据计算）时面临困难。

Method: 该方法采用编程示例（PBE）和轻量级草图（提供公式草图和输入-输出示例）的合成技术来自动化数据补全。它提出了一种结合表格数据的空间和关系推理的领域特定语言（DSL）。关键技术创新是一种基于有限树自动机（FTA）的新型版本空间学习算法，它能生成与示例一致的DSL程序，并允许更紧凑的表示和程序间共享。

Result: 该方法已在一个名为DACE的工具中实现，并在来自在线帮助论坛的84个基准测试上进行了评估。通过与PROSE和SKETCH这两个现有合成器进行比较，展示了该方法的优势。

Conclusion: 该研究提出的基于PBE和草图的合成技术，利用领域特定语言和基于FTA的新型合成算法，能够有效地自动化表格数据补全任务。DACE工具的实现和实验结果表明，该方法在性能上优于现有技术，具有实际应用价值。
...
### [100] [Pivoting the paradigm: the role of spreadsheets in K-12 data science](https://arxiv.org/abs/2506.03232)
*Oren Tirschwell,Nicholas Jon Horton*

Main category: stat.OT

TL;DR: 该论文探讨了电子表格作为K-12学生和教师培养数据和计算技能的工具的潜力，审查了现有框架，提出了学习成果，并讨论了电子表格如何帮助发展数据洞察力和计算流畅性。


<details>
  <summary>Details</summary>
Motivation: 电子表格在K-12学生和教师中广泛使用，用于数据收集、组织和交互，从而促进数据探索和分析。本研究的动机是强调电子表格在培养K-12学生的数据和计算技能方面的重要作用。

Method: 该论文首先回顾了K-12数据工具的现有框架；其次，提出了通过将电子表格纳入课程可以实现的数据驱动学习成果；最后，讨论了电子表格如何帮助发展数据洞察力和计算流畅性。此外，还提供了示例课堂活动，识别了采用的挑战和障碍，提出了教学方法以降低师生的学习曲线，并讨论了专业发展的必要性。

Result: 该论文的“结果”是其分析和提出的内容：回顾了现有框架，提出了可实现的数据驱动学习成果，讨论了电子表格在发展数据洞察力和计算流畅性方面的作用，并提供了示例活动、识别了挑战、提出了教学建议以及强调了专业发展的需求。

Conclusion: 电子表格可以显著帮助K-12学生发展数据洞察力和计算流畅性。然而，成功的整合需要解决挑战，采用适当的教学方法，并提供专业的教师发展。
...
### [101] [Wordle: A Microcosm of Life. Luck, Skill, Cheating, Loyalty, and Influence!](https://arxiv.org/abs/2309.02110)
*James P. Dilger*

Main category: math.HO

TL;DR: 该论文分析了 Wordle 玩家的数据，揭示了玩家行为模式，包括作弊行为和起始词选择策略。


<details>
  <summary>Details</summary>
Motivation: 超越社交媒体帖子、调查和谷歌趋势，为 Wordle 中的作弊行为提供确凿的定量证据。

Method: 作者从2023年5月至2023年8月收集了 Wordle 玩家首次猜测的数据，这些数据可以方便地从游戏结果显示中复制和粘贴。

Result: A) 每天约有0.2-0.5%的玩家一次猜中单词，这表明每天有4,000-10,000名玩家通过游戏外获取答案进行作弊。B) 至少有三分之一的玩家有偏爱的起始词，即使该词曾作为目标词出现，他们也保持忠诚。C) 2023年8月15日，约有30,000名玩家突然改变了他们的起始词，推测是受填字游戏线索的影响。

Conclusion: 这项研究提供了关于 Wordle 作弊和其他玩家行为的可靠定量证据。
...
### [102] [GeoGebra e situações que envolvem modelação numa abordagem STEAM](https://arxiv.org/abs/1907.02099)
*J. M. D. S. Dos Santos,A. P. Silveira,A. E. S. Trocado*

Main category: math.HO

TL;DR: 本文介绍了为葡语国家教师和学生设计的基于GeoGebra的教学任务，旨在数学教学中融入STEAM方法，重点关注二维和三维几何建模，并将推广至西班牙语和英语用户。


<details>
  <summary>Details</summary>
Motivation: 在葡语国家的数学课堂中实施包含技术（即GeoGebra互动数学软件）的STEAM方法，并为后续学生应用提供教学材料。

Method: 通过GeoGebra软件（包括2D、3D、CAS、电子表格窗口）分析二维和三维几何建模情境，研究实体和曲面中的切割平面。任务设计易于遵循，并提供工具和命令指示脚本。这些任务是为葡语国家GeoGebra培训师课程开发的。

Result: 这些材料使得无论软件知识程度如何的用户都能轻松学习，能够将数学与其他科学和艺术相结合，并通过项目发展和巩固相关数学内容。最初用葡萄牙语开发的材料将被改编为西班牙语和英语版本，以促进全球传播。

Conclusion: 所开发的GeoGebra任务有助于在数学教育中推广STEAM方法，促进跨学科联系，并巩固数学内容，通过教师培训具有广泛的传播和影响力潜力。
...
### [103] [Using a Catenary Trajectory to Reduce Wellbore Friction in Horizontal Extended Reach Drilling](https://arxiv.org/abs/2309.12317)
*Vu Nguyen*

Main category: cs.RO

TL;DR: 该项目介绍了悬链线概念，旨在通过避免钻杆与井壁接触来减少井筒摩擦，从而最大限度地减少扭矩和阻力。通过案例研究将其与传统二维圆弧设计进行比较，并提供了一个易于使用的Excel表格进行设计计算。


<details>
  <summary>Details</summary>
Motivation: 钻井过程中的井筒摩擦是由于其与总成本相关性而引起的最大问题之一。悬链线概念被引入以减少井筒摩擦，但它需要详细的分析。本项目将填补这一空白。

Method: 引入悬链线形状（绳索、链条或钻杆的自然形状），使钻杆在井筒内自由悬挂，理想情况下钻孔与钻杆之间没有接触，从而没有摩擦。通过案例研究比较悬链线轨迹设计与传统二维圆弧设计的结果。计算过程可在MS Excel表格中找到。

Result: 悬链线概念有望通过避免钻孔与钻杆之间的接触来最大限度地减少扭矩和阻力，从而消除摩擦。通过案例研究比较了悬链线轨迹设计和传统二维圆弧设计。计算过程在一个易于使用且可靠的MS Excel电子表格中提供。

Conclusion: 悬链线轨迹设计提供了一种最小化井筒摩擦的方法，并得到了一个实用的Excel工具的支持，可用于大位移井，表明其降低钻井成本的潜力。
...
### [104] [Hazard Analysis of Collaborative Automation Systems: A Two-layer Approach based on Supervisory Control and Simulation](https://arxiv.org/abs/2209.12560)
*Tom P. Huck,Yuvaraj Selvaraj,Constantin Cronrath,Christoph Ledermann,Martin Fabian,Bengt Lennartson,Torsten Kröger*

Main category: cs.RO

TL;DR: 本文提出了一种双层模型化危险分析方法，结合了形式化方法和仿真，以应对日益复杂的系统，并克服了传统和基于测试方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 目前的危险分析方法（人工推理、清单、电子表格）不足以应对复杂系统；基于测试的方法成本高昂或存在危险。现有的模型化方法（形式化或仿真）各有优缺点。

Method: 采用双层方法，首先使用监督控制理论从系统形式化模型中合成不安全行为，然后将结果输入到仿真中，使用特定领域的风险指标进行详细分析。

Result: 该方法具有普适性，并在一个工业人机协作系统上验证了其有效性。

Conclusion: 本文提出了一种有效的双层模型化危险分析方法，结合了形式化方法和仿真的优点，适用于安全关键系统，并在人机协作领域进行了验证。
...
### [105] [Trapped in Transformative Agreements? A Multifaceted Analysis of >1,000 Contracts](https://arxiv.org/abs/2409.20224)
*Laura Rothfritz,W. Benedikt Schmal,Ulrich Herb*

Main category: cs.DL

TL;DR: 研究机构似乎被“转型协议”困住，这强化了混合出版系统，增加了成本，而非导向完全开放存取。


<details>
  <summary>Details</summary>
Motivation: 利用ESAC数据库深入分析转型协议的合同特征和现状，因为研究机构似乎被这些协议“困住”。

Method: 通过网络抓取ESAC倡议数据库中的所有合同细节，扩展其概述表格，然后结合定性与定量方法对合同特征和转型协议格局进行深入分析。

Result: 分析表明研究机构似乎被“转型协议”困住。这些协议并未成为通向完全开放存取世界的桥梁，反而使学术界停留在混合系统中。这赋予了传统出版商巨大的市场权力，提高了进入壁垒，降低了竞争，并增加了图书馆和大学的成本。

Conclusion: 转型协议并未如预期般推动完全开放存取，反而使学术界陷入混合系统，增强了传统出版商的市场力量，并增加了机构的成本。
...
### [106] [Ensuring Adherence to Standards in Experiment-Related Metadata Entered Via Spreadsheets](https://arxiv.org/abs/2409.08897)
...
TL;DR: 科学家们倾向于使用电子表格输入元数据，但电子表格在确保元数据一致性方面存在局限性。本文介绍了一种端到端的方法，支持基于电子表格的元数据输入，同时确保严格遵守社区元数据标准并提供质量控制。


<details>
  <summary>Details</summary>
Motivation: 科学家们越来越认识到提供丰富、符合标准的元数据来描述实验结果的重要性，但尽管有复杂的工具，研究人员通常倾向于使用电子表格来提供元数据，尽管电子表格在确保元数据一致性和符合正式规范方面存在局限性。

Method: 该方法采用了几个关键组件，包括：可定制的模板（用于捕获元数据标准并指导研究人员使用的电子表格）、可直接从电子表格访问的受控术语和本体（用于定义元数据值），以及一个交互式网络工具（允许用户快速识别和修复其基于电子表格的元数据中的错误）。

Result: 该方法正在生物医学联盟 HuBMAP 中部署，用于定义和收集各种生物测定的元数据。

Conclusion: 本文提出的端到端方法有效支持了基于电子表格的元数据输入，同时确保了对社区元数据标准的严格遵守和质量控制，并通过其在HuBMAP中的部署得到了验证。
...
### [107] [A Comprehensive Approach to Ensuring Quality in Spreadsheet-Based Metadata](https://arxiv.org/abs/2312.09107)
*Martin J. O'Connor,Marcos Martínez-Romero,Mete Ugur Akdogan,Josef Hardi,Mark A. Musen*

Main category: cs.DL

TL;DR: 本文提出了一种端到端方法，旨在通过可定制模板、受控术语支持和交互式网络工具，解决电子表格元数据输入的合规性和质量控制问题。


<details>
  <summary>Details</summary>
Motivation: 科学家依赖电子表格描述数据元数据，但其在合规性和质量上存在局限；现有工具又面临学习曲线陡峭和定制受限的问题。

Method: 该方法采用定制化模板来定义元数据，集成支持受控术语的使用，并提供一个交互式网络工具，以快速识别和修复电子表格元数据中的错误。

Result: 该方法正在一个生物医学联盟中部署，用于定义和收集科学实验的元数据。

Conclusion: 本文描述了一种可部署的方法，能够改进科学实验中基于电子表格的元数据收集，确保其合规性和质量。
...
### [108] [Towards Semantic Interoperability in Historical Research: Documenting Research Data and Knowledge with Synthesis](https://arxiv.org/abs/2107.13957)
...
TL;DR: 本文介绍了一个名为“Synthesis”的Web协作文档系统，它利用CIDOC-CRM和RDF标准，旨在帮助历史学家克服传统文档方法（如电子表格和简单关系数据库）在文物文献记录中面临的协作、细节表示、数据结构扩展、多源数据整合以及数据重用等挑战，从而提高数据价值和长期有效性。


<details>
  <summary>Details</summary>
Motivation: 当前历史学研究中，文物及其相关证据的文档和研究主要使用电子表格或简单的关系数据库。这种方法在数据分析和学术解释方面有其优点，但也存在以下问题：1) 大量用户难以进行协作但受控的文档工作；2) 缺乏对推断出的关系细节的表示；3) 难以扩展底层数据结构以及整合来自多个不同信息源的数据；4) 数据在特定研究活动之外的重用受到限制。

Method: 本文描述了“Synthesis”文档系统，该系统是Web化的、协作性的，并利用了现有的信息文档和发布标准（CIDOC-CRM, RDF）。该系统已在一个正在进行的艺术史研究项目中被大量历史学家使用。

Result: “Synthesis”系统支持历史学家应对传统文档方法面临的问题，专注于语义互操作性，并生成具有高价值和长期有效性的数据。

Conclusion: “Synthesis”系统为历史学研究提供了一个先进的文档解决方案，通过支持协作、语义互操作性和采用标准，有效解决了传统方法中的诸多挑战，提升了历史数据的价值和可持续性。
...
### [109] [FAST CAT: Collaborative Data Entry and Curation for Semantic Interoperability in Digital Humanities](https://arxiv.org/abs/2105.13733)
*Pavlos Fafalios,Kostas Petrakis,Georgios Samaritakis,Korina Doerr,Athina Kritsotaki,Yannis Tzitzikas,Martin Doerr*

Main category: cs.DL

TL;DR: 本文介绍了一个名为FAST CAT的协作系统，旨在解决数字人文和实证研究中现有数据管理工具（如电子表格和关系数据库）的局限性，提供辅助数据录入和管理功能。


<details>
  <summary>Details</summary>
Motivation: 描述性和实证科学（如历史学）在数据分析和管理中过度依赖电子表格和关系数据库，导致数据高度依赖初始研究假设，通常不适用于其他研究；缺乏对推断关系的细节表示；以及难以追溯和验证原始数据源。

Method: 本文提出了FAST CAT，一个用于数字人文和类似实证研究的协作式辅助数据录入和管理系统。论文将描述相关挑战以及支持语义互操作性的总体方法。

Result: 论文讨论了FAST CAT在一个名为SeaLiT的欧洲海洋历史项目中的使用，该项目研究了1850年至1920年代地中海地区汽船引入的经济、社会和人口影响。抽象中未给出具体的实验结果或量化成果。

Conclusion: FAST CAT系统被提出作为解决数字人文和实证研究中现有数据管理局限性的方案，并展示了其在具体历史研究项目中的应用潜力，以改善数据录入、管理和语义互操作性。
...
TL;DR: 该论文提出FORTAP，一种利用电子表格公式进行数值推理感知的表格预训练方法，在单元格类型分类和公式预测方面取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 表格中的数值推理仍然是一个挑战，而电子表格公式提供了强大的监督，且大量专家制作的公式易于获取。

Method: FORTAP是首个利用大量电子表格公式语料库进行数值推理感知表格预训练的方法。设计了两个公式预训练任务，以明确指导FORTAP学习半结构化表格中的数值引用和计算。
...
TL;DR: Mondrian方法可以自动识别多区域电子表格中的布局模板并提取区域，在检测区域边界和识别重复布局方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 电子表格的灵活结构导致自动化分析困难，尤其当单个文件包含多个独立区域（多区域文件）时。此外，在大量电子表格中，存在共享相同布局的文件。

Method: 提出Mondrian方法，分三阶段：1. 将文件渲染为图像，检查潜在区域元素；2. 使用聚类算法将识别出的元素分组形成区域；3. 将每个文件布局表示为图，与其他文件比较以找到布局模板。

Result: 该方法在检测文件内可靠的区域边界和正确识别跨文件重复布局方面，表现优于现有最先进的表格识别算法。

Conclusion: Mondrian方法通过识别布局模板和提取对应区域，有效解决了多区域电子表格的自动化分析难题，提高了数据处理效率。
...
TL;DR: TUTA是一个统一的预训练架构，用于理解各种结构化表格。它通过结构感知机制和渐进式预训练目标增强了Transformer，并在表格理解任务中取得了最先进的成果。


<details>
  <summary>Details</summary>
Motivation: 现有的表格理解方法主要关注关系型表格，而忽略了其他常见的表格结构。

Method: 提出了TUTA，一个统一的预训练架构，通过三种新颖的结构感知机制增强了Transformer：1) 设计了一种统一的基于树的结构（二维坐标树）来描述表格的空间和层次信息；2) 提出了基于树的注意力机制和位置嵌入来捕获这些信息；3) 设计了三个渐进式预训练目标，以在token、单元格和表格级别实现表示。

Result: TUTA在表格理解任务中非常有效，在五个广泛研究的数据集上达到了最先进的性能。

Conclusion: TUTA为理解通用结构化表格提供了一种统一且有效的方法，通过整合空间、层次和语义信息，超越了以往的方法。
...
TL;DR: TableSense是一个新颖的端到端框架，利用卷积神经网络和有效的单元格特征化方案，实现了电子表格中表格的高效精确检测，并在多样化数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 电子表格中的表格检测是数据智能的关键技术，但由于表格结构和布局的多样性而面临挑战。

Method: TableSense框架包括三个主要部分：首先，设计了有效的单元格特征化方案来利用每个单元格的信息；其次，开发了增强的卷积神经网络模型以精确检测表格边界；第三，提出了有效的不确定性度量来指导基于主动学习的智能采样算法，高效构建了包含22,176个表格的训练数据集。

Result: TableSense在EoB-2指标上达到了91.3%的召回率和86.5%的精确度，显著优于现有商业电子表格工具中的检测算法和计算机视觉领域最先进的卷积神经网络。

Conclusion: TableSense通过其创新的框架和方法，有效解决了电子表格表格检测的挑战，并在性能上取得了显著提升，为电子表格数据智能奠定了基础。
...
TL;DR: 这篇论文介绍了一种通过在多个语义空间中表示表格元素并结合判别学习模型来计算表格相似度，从而推荐相关表格的方法，并取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 电子表格程序非常流行，用户可以从网络上相关结构化内容（表格）的推荐中受益。核心问题是如何计算表格之间的相似性。

Method: 开发了一个理论上可靠的表格匹配框架。该方法通过在多个语义空间中表示表格元素，并使用判别学习模型结合元素级别的相似性。

Result: 在专门构建的维基百科表格测试集上，所提出的方法展现了最先进的性能。

Conclusion: 论文成功地提出了一种新颖有效的相关表格推荐方法，并证明了其卓越的性能。
...
TL;DR: SmartTable是一个在线表格应用，专注于关系型表格，提供智能辅助功能，帮助用户填充行（实体）和扩展列（属性），并且是开源的，可在http://smarttable.cc访问。


<details>
  <summary>Details</summary>
Motivation: 介绍SmartTable，一个具备智能辅助功能的在线表格应用，特别关注关系型表格，旨在为用户提供填充行和扩展列的智能协助。

Method: 提供两种协助方式：(i) 填充表格中的额外实体（行）；(ii) 扩展额外的实体属性（列）。论文还提供了实现细节，并作为开源项目发布。

Result: SmartTable应用已发布并开源，可在http://smarttable.cc访问。

Conclusion: SmartTable是一个面向关系型表格的智能在线表格应用，能够辅助用户填充数据和扩展表格结构，且已开源并可在线使用。
...
TL;DR: 本文提出了用于在以实体为中心的表格中填充行和列的生成概率模型，利用知识库和大型表格语料库，并展示了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 为电子表格程序配备智能辅助功能，特别是在以实体为中心的表格中填充额外实例的行和新标题的列。

Method: 为填充行和列两项任务开发了生成概率模型。使用知识库和大型表格语料库来估计模型组件。

Result: 实验评估模拟了用户输入实际表格内容的不同阶段。详细结果分析表明，模型组件是互补的，并且我们的方法优于现有方法。

Conclusion: 模型组件是互补的，并且所提出的方法优于现有方法，为实体中心表格的智能辅助提供了有效方案。
...
### [117] [TableTalk: Scaffolding Spreadsheet Development with a Language Agent](https://arxiv.org/abs/2502.09787)
...
TL;DR: TableTalk是一种电子表格编程代理，它通过支架式、灵活性和增量式设计原则，帮助程序员创建高质量电子表格，并显著降低认知负荷和思考时间。


<details>
  <summary>Details</summary>
Motivation: 电子表格编程具有挑战性。大型语言模型中的语言代理在电子表格创建方面显示出前景。

Method: TableTalk代理基于三项设计原则（支架式、灵活性和增量式），这些原则源于对7位电子表格程序员和85个Excel模板的研究。它通过结构化计划指导程序员，生成潜在的下一步，并使用预定义工具逐步构建电子表格。

Result: 在20名程序员的实验中，TableTalk生成的电子表格质量更高，被选择的可能性是基线的2.3倍，并使认知负荷和思考时间减少了12.6%。

Conclusion: 本文提出了代理式电子表格编程工具的设计指南，并讨论了其对电子表格编程、最终用户编程、AI辅助编程和人机协作的影响。
...
### [118] [Large Language Models for Spreadsheets: Benchmarking Progress and Evaluating Performance with FLARE](https://arxiv.org/abs/2506.17330)
*Simon Thorne*

Main category: cs.SE

TL;DR: 大型语言模型在处理表格任务方面的能力尚未充分探索。本研究建立了一个综合基准，评估领先的LLM在表格功能、公式生成和数据操作方面的表现。结果表明，LLM擅长简单任务，但在复杂多步骤操作中常出错，这凸显了当前LLM在处理需要精确逻辑推理的表格任务时的局限性，并提出了整合符号推理能力的需求。为此，引入了FLARE这一新基准。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在各个领域展现出显著能力，但它们在电子表格相关任务中的有效性仍未被充分探索。

Method: 本研究引入了一个全面的基准框架，用于评估领先的LLM在执行电子表格功能、公式生成和数据操作任务方面的性能。该基准涵盖了从基本公式创建到复杂的真实世界电子表格场景的任务。此外，引入了FLARE（公式逻辑、审计、推理和评估）这一新基准，用于评估LLM在真实世界电子表格逻辑、审计和推理任务上的性能。

Result: 研究发现，LLMs在简单的任务中表现出熟练程度，但在复杂的、多步骤的操作中常常失败，经常产生看似合理但错误的输出。

Conclusion: 这些结果强调了当前LLMs在处理需要精确逻辑推理的电子表格任务时的局限性，并突出了将符号推理能力整合到LLM架构中的必要性。为了支持这一点，我们引入了FLARE这一新基准，用于评估LLM在真实世界电子表格逻辑、审计和推理任务上的表现。
...
### [119] [Overcoming Vision Language Model Challenges in Diagram Understanding: A Proof-of-Concept with XML-Driven Large Language Models Solutions](https://arxiv.org/abs/2502.04389)
*Shue Shiinoki,Ryo Koshihara,Hayato Motegi,Masumi Morishige*

Main category: cs.SE

TL;DR: 本研究提出了一种文本驱动的方法，通过从可编辑的源文件（如xlsx）中提取图表元素的文本元数据，并将其作为LLM的输入，从而绕过视觉语言模型的视觉识别局限性，实现对图表结构的准确理解。与基于VLM的方法相比，该方法在需要详细理解图表结构的问题上获得了更准确的答案。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型（VLMs）在各种图像理解任务上取得了进展，但从业务文档中准确识别和提取图表中描绘的结构和关系仍然面临重大挑战。

Method: 本研究提出了一种文本驱动的方法，不依赖于VLM的视觉识别能力。相反，它利用可编辑的源文件（如xlsx、pptx或docx），其中图表元素（如形状、线条、注释）被保存为文本元数据。该方法将提取的形状数据转换为大型语言模型（LLMs）的文本输入，使LLM能够分析关系并生成对业务问题的响应，避免了基于图像处理的瓶颈。

Result: 与基于VLM的方法进行的实验比较表明，所提出的文本驱动框架在需要详细理解图表结构的问题上产生了更准确的答案。该研究结果不仅限于测试的.xlsx文件，还可以扩展到其他带有源文件的文档中的图表，例如Office pptx和docx格式。

Conclusion: 这些发现强调了通过从原始源文件直接进行文本提取来规避VLM限制的可行性。通过LLM实现强大的图表理解，我们的方法为在实际业务场景中提高工作流程效率和信息分析提供了有前景的途径。
...
### [120] [On the Workflows and Smells of Leaderboard Operations (LBOps): An Exploratory Study of Foundation Model Leaderboards](https://arxiv.org/abs/2407.04065)
*Zhimin Zhao,Abdul Ali Bangash,Filipe Roseiro Côgo,Bram Adams,Ahmed E. Hassan*

Main category: cs.SE

TL;DR: 该研究分析了基础模型（FM）排行榜在实际场景中的运作方式，并识别出潜在缺陷（“排行榜异味”），旨在提高软件工程团队在FM选择中的透明度和责任性。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型（FM）对于各种软件工程（SE）任务至关重要，但由于缺乏标准化评估和比较指南，导致FM排行榜的透明度受到威胁，限制了利益相关者有效选择FM的能力。

Method: 研究人员从GitHub、Hugging Face Spaces、Papers With Code、电子表格和独立平台等五个来源收集了1,045个FM排行榜，审查了其文档，并与排行榜运营商沟通以了解其工作流程。通过卡片分类和协商一致，他们识别了五种不同的工作流模式，并开发了一个捕获关键组件及其交互的领域模型。

Result: 研究识别出五种不同的工作流模式并开发了排行榜操作的领域模型。此外，他们还发现了八种独特的“排行榜异味”类型。

Conclusion: 通过减轻这些识别出的“排行榜异味”，软件工程团队可以提高当前排行榜操作实践的透明度、责任性和协作性，从而促进一个更健壮和负责任的FM比较和选择生态系统。
...
### [121] [MaxMind: A Memory Loop Network to Enhance Software Productivity based on Large Language Models](https://arxiv.org/abs/2408.03841)
*Yuchen Dong,XiaoXiang Fang,Yuchen Hu,Renshuang Jiang,Zhe Jiang*

Main category: cs.SE

TL;DR: 这篇论文提出了Memory-Loop网络和增强的RAG机制（MaxMind模型），用于自动化软件操作和工具生成（SOTG），通过利用实时任务经验和记忆循环来提高大型语言模型的生产力。实验表明，该方法显著提高了任务成功率和执行效率，并解决了专业任务中大型语言模型再训练的问题。


<details>
  <summary>Details</summary>
Motivation: 目前将大型语言模型应用于自动化软件操作和工具生成（SOTG）的研究，常常忽视将实时任务经验转化为系统记忆的重要性，以及区分现有知识价值以供未来参考的重要性。

Method: 该论文将外部记忆模型演变为Memory-Loop网络，以实现及时记忆和经验引用。通过知识精确分割增强了RAG机制，以根据价值差异利用记忆，并据此设计了用于SOTG的MaxMind模型。为了验证方法，开发了MaxMind4Sheet。

Result: 与SheetCopilot的对比实验表明，任务记忆的积累和循环利用使任务成功率每轮提高了约3%-6%。记忆循环利用还可以将系统任务执行效率提高高达25%，并通过记忆转移解决大型语言模型在处理专业任务时面临的再训练问题。

Conclusion: MaxMind在增强SOTG领域大型语言模型系统的能力和生产力方面具有巨大潜力。
...
### [122] [Will Dynamic Arrays finally change the way Models are built?](https://arxiv.org/abs/2006.14706)
*Peter Bartholomew*

Main category: cs.SE

TL;DR: 本文探讨了Excel中新的动态数组如何能在一个更专业的开发环境中，取代传统技术来提高电子表格解决方案的完整性并减少错误，尤其是在以前的CSE数组公式较为繁琐的情况下。


<details>
  <summary>Details</summary>
Motivation: 电子表格因其直观性而广受欢迎，但也极易出错。之前的研究尝试通过命名和CSE数组公式来提高透明度，但后者使用起来很笨重。Excel中引入的动态数组为解决这些问题并提高解决方案的完整性提供了新的机会。

Method: 本文旨在论证这样一个观点：在更专业的开发环境中采用动态数组，可以取代那些对解决方案完整性有较高要求的传统技术。

Result: 完全动态的模型主要优势在于它们需要较少的人工干预来保持更新，因此有可能减少随之而来的错误和风险。

Conclusion: 动态数组在Excel中的应用，有望在专业开发环境中提高电子表格的完整性、减少手动干预，并降低错误和风险，从而可能替代旧有技术。
...
### [123] [A Structured Approach to the development of Solutions in Excel](https://arxiv.org/abs/1704.01142)
*Peter Bartholomew*

Main category: cs.SE

TL;DR: 电子表格在数据处理方面实现了民主化，但默认的Excel解决方案缺乏超出单单元格公式的结构，这导致了可伸缩性问题和错误。本文考虑使用非常规技术来创建一种连贯的解决方案策略，其中问题通过一系列类似于编程语言步骤的公式来解决。


<details>
  <summary>Details</summary>
Motivation: 电子表格为非专业人士提供了强大的数据处理能力，但默认的Excel解决方案缺乏超出单单元格公式的结构。这种传统代码中存在的结构缺失会导致错误增加和可伸缩性差。本文旨在解决电子表格中的这种结构缺陷。

Method: 本文探讨了使用有争议或较少使用的技术来创建一种连贯的解决方案策略，其中问题通过一系列类似于编程语言步骤的公式来解决。
...
### [124] [Spreadsheet-based Configuration of Families of Real-Time Specifications](https://arxiv.org/abs/2310.20395)
*José Proença,David Pereira,Giann Spilere Nandi,Sina Borrami,Jonas Melchert*

Main category: cs.SE

TL;DR: 该论文通过利用形式模型和需求的可变性，并结合基于电子表格的工具，简化了实时系统变体模型的检查，解决了状态爆炸问题。这项工作是与阿尔斯通公司合作的成果。


<details>
  <summary>Details</summary>
Motivation: 实时系统模型检查复杂，需要在细节和避免状态爆炸之间取得平衡。本文旨在促进实时规范变体的模型检查，并与铁路公司阿尔斯通合作，以解决实际用例。

Method: 该方法利用形式模型和被检查需求的可变性。配置信息存储在结构化的MS Excel电子表格中，原型工具会自动处理这些表格以生成实例并运行模型检查器。它通过分析有效特征组合来扩展以前的工作，同时保持基于电子表格的简单界面。

Result: 该工作促进了实时规范变体的模型检查。开发了一个原型工具，能够根据Excel电子表格生成实例并运行模型检查器。它提出了通过分析有效特征组合来扩展先前工作的方法，并保留了基于电子表格界面的简洁性。

Conclusion: 本文提出了一种有效且实用的方法，通过利用可变性分析和用户友好的电子表格界面，简化了复杂实时系统的模型检查，特别是在解决状态爆炸挑战方面。该方法还通过分析特征组合扩展了现有工作，并得到了工业合作的验证。
...
### [125] [SheetCopilot: Bringing Software Productivity to the Next Level through Large Language Models](https://arxiv.org/abs/2305.19308)
*Hongxin Li,Jingran Su,Yuntao Chen,Qing Li,Zhaoxiang Zhang*

Main category: cs.SE

TL;DR: 该论文提出了一个名为SheetCopilot的智能体，它利用大型语言模型(LLMs)通过自然语言指令控制电子表格软件，以帮助终端用户自动化重复且易错的任务。


<details>
  <summary>Details</summary>
Motivation: 计算机终端用户在处理表格数据和项目时间表等日常任务上花费了大量时间，这些任务重复且容易出错，但大多数终端用户缺乏自动化这些工作的技能。大型语言模型的出现使得通过自然语言请求直接控制软件成为可能。

Method: 我们提出了SheetCopilot智能体，它接受自然语言任务并控制电子表格以满足要求。我们提出了一组原子动作作为电子表格软件功能的抽象。我们进一步设计了一个基于状态机的任务规划框架，使LLMs能够稳定地与电子表格交互。我们整理了一个包含221个电子表格控制任务的代表性数据集，并建立了一个全自动评估管道来严格基准测试LLMs在软件控制任务中的能力。

Result: 我们的SheetCopilot在单次生成中正确完成了44.3%的任务，大大优于强大的代码生成基线。

Conclusion: SheetCopilot证明了LLMs在电子表格软件控制任务中的有效性，能够帮助终端用户自动化复杂操作。
...
### [126] [Excel as a Turing-complete Functional Programming Environment](https://arxiv.org/abs/2309.00115)
*Peter Bartholomew*

Main category: cs.SE

TL;DR: Excel的动态数组升级彻底改变了电子表格解决方案的构建方式，使其更接近正式编程。本文将探讨新兴趋势及其对风险的潜在影响。


<details>
  <summary>Details</summary>
Motivation: 探索自2018年Excel计算引擎升级以支持动态数组以来，电子表格解决方案构建艺术发生的巨大变化，以及这些变化如何使构建方法更接近正式编程。

Method: 通过展示传统电子表格的临时最终用户实践如何被与正式编程更相似的新方法取代，并讨论Excel社区中先驱工作正在出现的趋势。

Result: 发现了Excel社区中正在出现的趋势，这些趋势表明传统的临时电子表格实践正在被更接近正式编程的方法所取代。

Conclusion: 尽管新功能的采用程度及其对风险的影响尚不明确，但Excel电子表格构建正在向更正式的编程范式演进，并已出现一些值得关注的趋势。
...
### [127] [A Use Case-Engineering Resources Taxonomy for Analytical Spreadsheet Models](https://arxiv.org/abs/2309.00104)
*Thomas A. Grossman,Vijay Mehrotra*

Main category: cs.SE

TL;DR: 本文提出了一个分析电子表格模型的九种类型分类法，考虑了用例和开发资源，并探讨了其应用。


<details>
  <summary>Details</summary>
Motivation: 本文旨在扩展先前的分类法，连接不同的研究文献，区分“分析解决方案”和“工业质量分析电子表格模型”，并帮助识别开发指南的有用性、查看错误和风险以及理解电子表格随时间的变化。

Method: 本文将先前的三类型分类法扩展为九种类型，探讨了每种类型的性质，提出了定义，将其与文献关联，并假设了它们的产生方式。

Result: 提出了一个分析电子表格模型的九种类型分类法，有助于识别各种电子表格开发指南的最有用之处，为查看电子表格错误和风险提供了视角，并为理解电子表格如何随时间变化提供了结构。
...
### [128] [Experimenting with ChatGPT for Spreadsheet Formula Generation: Evidence of Risk in AI Generated Spreadsheets](https://arxiv.org/abs/2309.00095)
*Simon Thorne*

Main category: cs.SE

TL;DR: 本论文探讨了ChatGPT生成电子表格公式的能力。结果显示，在特定情况下，ChatGPT能正确生成公式并进行推理，但当信息有限或问题复杂时，其准确性会下降，并可能产生错误信息或“幻觉”。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）能通过解释自然语言创建复杂的计算机程序，使编程变得普适。本文旨在探索ChatGPT在生成有效电子表格公式方面的能力。

Method: 本文通过一系列实验，探究ChatGPT在需要推断、推理和解决问题的情况下，生成有效电子表格公式和相关计算输出的能力。

Result: 在某些情况下，ChatGPT能以正确的推理、演绎和推断生成正确的电子表格公式。然而，当信息有限、不确定或问题过于复杂时，ChatGPT的准确性及其推理、推断和演绎能力会下降。这还可能导致错误的陈述和“幻觉”，从而影响创建电子表格公式的过程。

Conclusion: ChatGPT在特定条件下能够正确生成电子表格公式并进行推理。但当信息不足或问题复杂时，其准确性会降低，并可能产生错误和“幻觉”。
...
### [129] [Demonstration of CORNET: A System For Learning Spreadsheet Formatting Rules By Example](https://arxiv.org/abs/2308.07357)
*Mukul Singh,Jose Cambronero,Sumit Gulwani,Vu Le,Carina Negreanu,Gust Verbruggen*

Main category: cs.SE

TL;DR: CORNET系统从用户示例中自动学习条件格式化 (CF) 规则，解决了用户手动编写这些规则的痛点。


<details>
  <summary>Details</summary>
Motivation: 用户在电子表格软件中需要手动编写条件格式化 (CF) 规则，这很不方便。

Method: CORNET结合了基于半监督聚类和迭代决策树学习的符号规则枚举，以及一个神经网络排序器来生成准确的条件格式化规则。

Result: CORNET可以从用户提供的一两个格式化单元格示例中，自动生成格式化规则建议并应用于电子表格。

Conclusion: CORNET是一个Microsoft Excel插件，通过少量用户示例即可自动生成条件格式化规则建议，极大地简化了用户操作。
...
### [130] [Excel Spreadsheet Analyzer](https://arxiv.org/abs/2211.06333)
*Amir Nassereldine,Patrick Chen,Jinjun Xiong*

Main category: cs.SE

TL;DR: 该论文提出了一种工具，用于创建电子表格的抽象中间表示（AIR），以便在将电子表格转换为Python等科学编程语言时，保留单元格之间的依赖关系信息。


<details>
  <summary>Details</summary>
Motivation: 电子表格在数值分析中广泛使用，但数据科学家正转向使用Python等科学编程语言进行数据分析，这导致在转换过程中丢失了公式和单元格依赖等信息。

Method: 提出了一种创建电子表格抽象中间表示（AIR）的工具，以保留数据间的相互依赖信息。此外，还在该工具之上构建了一个Python库，用于在Python中执行数据分析。

Result: 该AIR有助于将电子表格传输到科学编程语言中，同时保留数据之间的相互依赖信息。基于该工具构建的Python库可以用于在Python中执行数据分析。

Conclusion: 开发了一种工具和Python库，用于解决电子表格与科学编程语言之间的鸿沟，同时维护关键的依赖信息。
...
### [131] [Exploring Spreadsheet Use and Practices in a Technologically Constrained Setting](https://arxiv.org/abs/2201.07696)
*Khwima Mckinley Mkamanga,Simon Thorne*

Main category: cs.SE

TL;DR: 本研究探讨了电子表格在马拉维一家准政府水务公司业务运营中的影响，旨在通过制定新方法来管理潜在风险，从而明确未来的电子表格使用。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在探索电子表格对马拉维一家准政府水务公司业务运营的影响，并帮助该组织定义未来的电子表格使用方式，通过影响新的方法来管理与电子表格相关的潜在风险。

Method: 研究重点关注电子表格的使用范围、生命周期以及组织政策和治理。

Result: 研究发现，电子表格的普及为业务自动化提供了有利环境，但也因管理、技术和人为因素问题带来了高风险。
...
### [132] [Methodology for Assessing the State of the Practice for Domain X](https://arxiv.org/abs/2110.11575)
*Spencer Smith,Jacques Carette,Peter Michalski,Ao Dong,Olu Owojaiye*

Main category: cs.SE

TL;DR: 该论文提出了一种包含九个步骤的方法论，用于评估研究软件领域的软件开发实践现状，旨在为改进软件开发方法和工具提供基础。该方法包括识别软件包、收集数据、访谈开发者和使用层次分析法（AHP）进行排名等，并估计每次评估需要173个人时。


<details>
  <summary>Details</summary>
Motivation: 为了改进研究软件的开发方法和工具，首先需要理解当前软件开发实践的现状。

Method: 本文开发了一种评估特定研究软件领域软件开发实践现状的方法论。该方法论包括九个步骤：i) 确定领域；ii) 识别候选软件包列表；iii) 将列表筛选至约30个软件包；iv) 收集每个软件包的源代码和文档；v) 收集与每个软件包相关的仓库数据（如星标数、未解决问题数、代码行数）；vi) 填写测量模板（包含评估9个质量（如可安装性、可用性和可见性）的108个问题）；vii) 访谈开发者（包含20个问题，耗时约一小时）；viii) 使用层次分析法（AHP）对软件进行排名；ix) 分析数据以回答预设问题。整个过程需领域专家参与。

Result: 该方法论包括一个包含108个问题的测量模板和20个问题的开发者访谈。基于使用该过程的经验，估计完成一个特定领域的评估所需时间为173个人时。

Conclusion: 该论文提供了一套全面的方法论，包括工具（如电子表格模板和AHP工具），旨在系统地评估研究软件领域的软件开发实践现状，这对于识别改进方法和工具的领域至关重要。
...
### [133] [Agile Elicitation of Scalability Requirements for Open Systems: A Case Study](https://arxiv.org/abs/2108.00567)
*Gunnar Brataas,Antonio Martini,Geir Kjetil Hanssen,Georg Ræder*

Main category: cs.SE

TL;DR: 本文介绍了一种轻量级工件ScrumScale模型，用于在敏捷软件开发中引出可伸缩性需求，并通过一个开放银行案例研究进行了验证。


<details>
  <summary>Details</summary>
Motivation: 在敏捷软件开发中，引出可伸缩性需求是复杂且研究不足的问题。特别是在传统银行系统转变为开放银行系统时，如何引出新的可伸缩性需求是一个挑战。

Method: 采用设计科学研究方法，并利用协调理论。提出了ScrumScale模型，它是一个简单的电子表格。

Result: 在TietoEVRY的开放银行案例研究中，ScrumScale模型提供了一种系统化的方法来生成可伸缩性需求，并在与利益相关者的对话中提供了显著优势。

Conclusion: ScrumScale模型能有效且系统地引出敏捷环境中的可伸缩性需求，并改善利益相关者之间的沟通。
...
### [134] [SpreadsheetCoder: Formula Prediction from Semi-structured Context](https://arxiv.org/abs/2106.15339)
*Xinyun Chen,Petros Maniatis,Rishabh Singh,Charles Sutton,Hanjun Dai,Max Lin,Denny Zhou*

Main category: cs.SE

TL;DR: SpreadsheetCoder是一种基于BERT的模型，它利用表格上下文（包括表头和半结构化数据）来合成电子表格公式。该模型实现了42.51%的top-1预测准确率，并且比基于规则的系统多帮助82%的用户。其主要贡献在于首次将表格上下文引入电子表格公式合成。


<details>
  <summary>Details</summary>
Motivation: 以往的电子表格公式预测方法主要使用输入-输出示例，未能充分捕捉真实电子表格中丰富的上下文信息，如表格结构、行与列之间的依赖关系以及表头的作用。本文旨在通过整合包含表头和半结构化表格数据的表格上下文来解决这一局限性。

Method: 本文提出了SpreadsheetCoder，一个基于BERT的模型架构，用于以行和列两种格式表示表格上下文。该模型在一个大型电子表格数据集上进行训练。

Result: SpreadsheetCoder实现了42.51%的top-1预测准确率，这比不使用丰富表格上下文的基线模型有显著提高。与基于规则的系统相比，SpreadsheetCoder在Google Sheets上辅助用户编写公式的效率提高了82%。

Conclusion: SpreadsheetCoder通过利用丰富的表格上下文，有效地合成了电子表格公式，显著优于现有方法，并提高了用户协助水平。这证明了在电子表格公式合成中考虑表格上下文的重要性。
...
### [135] [From webtables to datatables](https://arxiv.org/abs/2006.14694)
*Mária Csernoch*

Main category: cs.SE

TL;DR: 这篇论文详细介绍了将一个《英雄联盟》LOL Board转换为电子表格格式的过程，并提供了两种解决方案（文字处理器和电子表格），旨在利用网页表格进行计算思维和电子表格教学。


<details>
  <summary>Details</summary>
Motivation: 网页表格是教授电子表格和计算思维的优秀来源，能够促进知识转移、解决实际问题、讨论调试。

Method: 论文详细介绍了LOL Board的转换过程及其算法，并提供了两种具体的转换方案：一种在文字处理器中实现，另一种纯粹在电子表格应用程序中实现。

Result: 提供了两种将LOL Board数据转换为电子表格的解决方案：一种通过文字处理器，另一种纯粹通过电子表格应用程序。这些方案为进一步讨论、发明新解决方案和组合现有方案留下了空间。

Conclusion: 通过将网页表格（特别是LOL Board）转换为电子表格，为教授电子表格和计算思维技能提供了实用的方法和案例，鼓励进一步的探索和创新。
...
### [136] [Implementation Strategies for Multidimensional Spreadsheets](https://arxiv.org/abs/2006.05814)
*Paul Mireault*

Main category: cs.SE

TL;DR: 该论文分析了经验丰富的Excel开发者在实现多维变量电子表格时采用的不同策略，主要有两种：投影法和数据库法。


<details>
  <summary>Details</summary>
Motivation: 了解经验丰富的Excel开发者如何在二维电子表格中实现多维变量。

Method: 邀请经验丰富的Excel开发者参与一个挑战，实现一个包含多维变量的电子表格，然后分析他们的实现策略。

Result: 识别出两种策略：大多数参与者将三维或四维变量投影到Excel的二维平面上；少数参与者采用数据库方法，将多维变量以数据集表的形式呈现，并带有适当的主键。后一种方法能使公式更简单。

Conclusion: 在Excel中实现多维变量时，采用数据库方法可以得到更简单的公式。
...
### [137] [Abstracting spreadsheet data flow through hypergraph redrawing](https://arxiv.org/abs/2006.04794)
*David Birch,Nicolai Stawinoga,Jack Binks,Bruno Nicoletti,Paul Kelly*

Main category: cs.SE

TL;DR: 这篇论文提出了一种通过重新定义单元格边界来提高电子表格抽象级别的方法，将单元格表示为超图边来封装操作符/数据节点，旨在减少错误并更好地反映用户的心智模型。


<details>
  <summary>Details</summary>
Motivation: 传统的电子表格因其抽象级别低而容易出错。单元格仅限于标量值，且链接是隐藏的。

Method: 将电子表格转换为细粒度图，其中运算符和值作为节点。通过围绕一组运算符/数据节点绘制边界“墙”来将“单元格”表示为超图边。通过识别常见子表达式和应用子树同构来检测向量（数组）操作，以说明这种方法。

Result: 论文提出了一种通过重新绘制单元格边界来提高电子表格抽象级别的方法，并用常见子表达式识别和子树同构应用作为向量操作检测的例子进行了说明。

Conclusion: 研究人员应寻求重新绘制这些边界的技术，以创建更高级别的“单元格”，这将更忠实地代表最终用户的真实世界/心智模型，从而扩展单元格可以包含的内容并创建更高级别的电子表格模型。
...
### [138] [Comprehensive review for common types of errors using spreadsheets](https://arxiv.org/abs/1912.09209)
*Ali Aburas*

Main category: cs.SE

TL;DR: 这篇论文全面回顾了电子表格中错误发现和修复的方法。


<details>
  <summary>Details</summary>
Motivation: 电子表格被广泛使用，但错误率很高，这促使研究人员开发了各种错误预防、检测和纠正工具。

Method: 本文对现有发现和修复电子表格错误的方法进行了全面的综述和分类，讨论了它们在定义、工作原理以及可发现的错误类型方面的最新研究工作，并探讨了终端用户常犯的错误类型。

Result: 对电子表格错误发现和修复方法进行了全面审查、分类和讨论，涵盖了它们的工作原理、可检测的错误类型以及终端用户常犯的错误。

Conclusion: 对现有电子表格错误发现和修复方法进行了全面的总结和分类，并讨论了用户常见错误，为未来的研究和工具开发提供了基础。
...
### [139] [A Coding-free Software Framework of Developing Web Data Management Systems](https://arxiv.org/abs/1910.05685)
*Can Yang,Shiying Pan,Runmin Li,Yu Liu,Lizhang Peng*

Main category: cs.SE

TL;DR: 本文提出了一种基于SaaS架构的无代码数据管理系统构建方法，通过通用Web平台和需求表实现快速定制和部署，并验证了其可行性。


<details>
  <summary>Details</summary>
Motivation: 非程序员在云端开发数据管理系统面临困难，但SaaS的发展为无代码开发提供了可能性。

Method: 基于SaaS架构，提出了一套无代码构建数据管理系统的理论和方法。设计了一个通用Web平台，通过解析电子表格中的需求表模型，在运行阶段实现目标系统，快速生成和发布定制的系统实例。同时提出了一套数据交换接口。
...
### [140] [Structured Spreadsheet Modelling and Implementation with Multiple Dimensions -- Part 2: Implementation](https://arxiv.org/abs/1909.00891)
*Paul Mireault*

Main category: cs.SE

TL;DR: 本文详细介绍了实现多维问题的精确步骤，以创建易于维护的电子表格，这是基于第一部分中提出的概念模型。


<details>
  <summary>Details</summary>
Motivation: 在第一部分中展示了如何开发一个涉及多维度变量问题的概念模型，本文的动机是提供精确的步骤来实现多维问题，以生成易于维护的电子表格。

Method: 本文提出的方法是提供精确的步骤来实现一个多维问题。

Result: 实现一个易于维护的电子表格。

Conclusion: 通过遵循本文提出的精确步骤，可以成功实现并维护多维问题的电子表格。
...
### [141] [On the Refinement of Spreadsheet Smells by means of Structure Information](https://arxiv.org/abs/1810.04542)
*Patrick Koch,Birgit Hofer,Franz Wotawa*

Main category: cs.SE

TL;DR: 该论文提出了一种通过利用推断的结构信息来改进电子表格“坏味道”检测的方法，旨在减少不正确和冗余的报告，并引入新的检测技术。


<details>
  <summary>Details</summary>
Motivation: 电子表格用户常常意识不到设计不佳的电子表格所带来的风险。当前的电子表格“坏味道”检测技术存在缺陷，导致不正确或冗余的报告，例如对每个单元格副本报告相同的问题，这可能会让用户不知所措。

Method: 作者提出通过利用推断的结构信息来改进电子表格“坏味道”检测。具体方法包括：首先，提供了一种静态分析方法来推断相关单元格的簇和块；其次，通过提供三个结合了单元格组和计算块信息的现有“坏味道”的改进示例来完善现有检测方法；最后，提出了三种利用推断的电子表格结构的新型“坏味道”检测技术。

Result: 实证评估表明，所提出的改进成功减少了不正确和冗余报告的“坏味道”数量，并且新引入的“坏味道”揭示了新的缺陷。

Conclusion: 通过利用推断的结构信息对现有“坏味道”检测技术进行改进，并引入新的检测方法，可以有效减少不正确和冗余的报告，并发现电子表格中新的潜在问题，从而提高电子表格的质量评估。
...
### [142] [Now You're Thinking With Structures: A Concept for Structure-based Interactions with Spreadsheets](https://arxiv.org/abs/1809.03435)
*Patrick Koch*

Main category: cs.SE

TL;DR: 该论文提出了一种结构感知方法来理解和交互电子表格，以解决复杂电子表格难以理解和适应的问题。它通过利用结构信息丰富可视化、增强用户操作和提供更改电子表格整体结构而非单个单元格的工具，旨在提高用户生产力和电子表格质量。
...
Method: 提出了一种结构感知的电子表格理解和交互概念，扩展了结构推理方面的现有工作。该方法利用结构信息丰富可视化、增强用户操作，并提供主动修改电子表格整体结构的工具。目前正在开发一个结构推理和可视化工具，并计划作为常见电子表格处理器的附加功能提供。

Result: 目标系统将作为现有电子表格工具的附加功能层。正在进行的工作已实现了一个结构推理和可视化工具，并计划引入设想的主动和被动交互机制，最终提供结构感知功能作为附加组件。

Conclusion: 通过提供以结构感知方式思考和交互电子表格的工具，将有助于提高用户的工作效率和电子表格的整体质量。
...
### [143] [Typed Table Transformations](https://arxiv.org/abs/1809.02746)
*Martin Erwig*
...
<details>
  <summary>Details</summary>
Motivation: 电子表格中的标签可以视为数据类型，并控制数据的排布。因此，论文旨在提出一种基于类型转换的表格转换方法。

Method: 论文提出了一种基于行和列类型转换的表格转换新方法，并阐述了类型化表格构建和转换的基本思想。

Result: 论文阐释了类型化表格构建和转换的基本理念，并提出了一系列有待未来研究解决的问题。

Conclusion: 论文提出了一种对电子表格进行类型化转换的新方法，并指明了未来的研究方向。
...
### [144] [Implementing WHERE and ORDER BY as spreadsheet formulas](https://arxiv.org/abs/1809.00025)
*Paul Mireault*

Main category: cs.SE

TL;DR: 论文开发了可在电子表格中实现SQL的WHERE和ORDER BY子句的公式，以解决现有工具无法自动响应数据变化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有电子表格（如Excel）的过滤和排序工具，或者查询/数据透视表工具，无法自动响应计算值变化。

Method: 开发实现SQL WHERE和ORDER BY子句的电子表格公式。

Result: 论文通过开发公式，在电子表格中实现了SQL的WHERE和ORDER BY子句的功能。

Conclusion: 论文成功开发了能自动响应数据变化并在电子表格中实现SQL WHERE和ORDER BY子句的公式。
...
### [145] [The use of Charts, Pivot Tables, and Array Formulas in two Popular Spreadsheet Corpora](https://arxiv.org/abs/1808.10642)
*Bas Jansen,Felienne Hermans*

Main category: cs.SE

TL;DR: 电子表格常出错，但研究多集中于公式。本文分析图表、数据透视表和数组公式在两大语料库中的使用，以全面理解其用法，提升质量。


<details>
  <summary>Details</summary>
Motivation: 工业中电子表格广泛用于决策，但易出错，导致决策失误和金钱损失。现有研究多关注公式，忽视图表、数据透视表等其他重要结构。为提高电子表格质量，需全面理解这些结构的使用。

Method: 分析Enron和EUSES两个流行的电子表格语料库中图表、数据透视表和数组公式的使用情况。

Result: 抽象中未直接给出分析结果，但阐述了研究计划：通过分析两大语料库，考察图表、数据透视表和数组公式的使用情况。

Conclusion: 通过研究图表、数据透视表和数组公式的使用，获得对电子表格使用情况的完整理解，从而提高电子表格的质量。
...
### [146] [Asheetoxy: A Taxonomy for Classifying Negative Spreadsheet-related Phenomena](https://arxiv.org/abs/1808.10231)
*Daniel Kulesz,Stefan Wagner*

Main category: cs.SE

TL;DR: 该论文提出了一种名为 Asheetoxy 的简单、以现象为导向的电子表格分类法，旨在避免使用“错误”这一模糊术语。初步研究表明，即使是非电子表格研究人员也能使用 Asheetoxy 对现实世界中的电子表格现象进行分类。


<details>
  <summary>Details</summary>
Motivation: 电子表格在许多组织中扮演着关键角色，但其错误导致了许多错误的决策。现有电子表格错误分类法存在问题，因为它们使用了模糊的“错误”一词，并且需要对底层过程和用户思维状态有深入了解，这阻碍了研究人员之间的讨论，也难以对实际的错误现象进行分类。

Method: 我们提出了 Asheetoxy，一个简单且以现象为导向的分类法，完全避免了“错误”这个有问题的术语。我们进行了一项初步研究，有 7 名参与者。

Result: 一项由 7 名参与者进行的初步研究表明，即使是非电子表格研究人员也能使用 Asheetoxy 类似地对现实世界中的电子表格现象进行分类。

Conclusion: Asheetoxy 是一种简单、以现象为导向的电子表格分类法，它通过避免“错误”这一模糊术语，并降低对深层背景知识的需求，成功解决了现有分类法的局限性，并能有效分类现实世界中的电子表格现象。
...
### [147] [Proceedings of the 5th International Workshop on Software Engineering Methods in Spreadsheets (SEMS'18)](https://arxiv.org/abs/1808.09174)
*Birgit Hofer,Jorge Mendes*

Main category: cs.SE

TL;DR: 第5届电子表格软件工程方法国际研讨会 (SEMS'18) 的会议记录，于2018年10月1日在葡萄牙里斯本举行，并与2018年IEEE可视化语言和以人为中心的计算研讨会 (VL/HCC) 同时举办。


<details>
  <summary>Details</summary>
Motivation: 此为研讨会会议记录，不涉及具体研究动机。

Method: 此为研讨会会议记录，不涉及具体研究方法。

Result: 此为研讨会会议记录，不涉及具体研究结果。

Conclusion: 此为研讨会会议记录，不涉及具体研究结论。
...
### [148] [Combining Spreadsheet Smells for Improved Fault Prediction](https://arxiv.org/abs/1805.10493)
*Patrick Koch,Konstantin Schekotihin,Dietmar Jannach,Birgit Hofer,Franz Wotawa*

Main category: cs.SE

TL;DR: 本文提出了一种基于机器学习的方法，通过AdaBoost集成分类器结合单个异味的预测，显著提高了电子表格故障预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 电子表格中的故障可能对业务产生严重影响，而现有电子表格异味的个体预测能力有限。

Method: 提出了一种基于机器学习的方法，利用AdaBoost集成分类器结合单个异味的预测。

Result: 在包含真实电子表格故障的两个公共数据集上进行的实验表明，故障预测准确性显著提高。

Conclusion: 基于机器学习结合个体异味预测的方法可以显著提高电子表格故障预测的准确性。
...
### [149] [An Easy & Collaborative RDF Data Entry Method using the Spreadsheet Metaphor](https://arxiv.org/abs/1804.04175)
*Markus Schröder,Christian Jilek,Jörn Hees,Sven Hertling,Andreas Dengel*

Main category: cs.SE

TL;DR: 本文提出了一种易于使用、零配置、基于网络的电子表格编辑器，能够同时将电子表格条目转换为RDF语句，使用户能更轻松地创建语义数据。


<details>
  <summary>Details</summary>
Motivation: 知识工作者广泛使用电子表格，但定义RDF语句对他们来说并不容易。本文旨在为各种用户（无论是RDF专家还是新手）提供一种简单的方式来创建语义数据。

Method: 我们提出了一种易于使用、零配置的基于网络的电子表格编辑器，该编辑器能够同时将电子表格条目转换为RDF语句。

Result: 用户研究表明，与传统方法相比，参与者能够在更短的时间内创建更多语句，并且质量相似或显著更优。

Conclusion: 该工具使专家和新手都能轻松创建语义数据，并显著优于其他方法。
...
### [150] [Structured Spreadsheet Modelling and Implementation with Multiple Dimensions - Part 1: Modelling](https://arxiv.org/abs/1801.09777)
*Paul Mireault*

Main category: cs.SE

TL;DR: 在许多日常模型中，维度是不可或缺的一部分，尤其时间维度被广泛使用。然而，在电子表格中表示第二个维度通常需要重复公式块或创建多个结构相同的工作表，这带来了挑战。


<details>
  <summary>Details</summary>
Motivation: 本文的动机是探讨在电子表格等常用工具中有效表示多维数据（特别是第二个维度）所面临的挑战和效率问题，因为现有方法涉及重复性工作。

Method: 抽象中未提及具体方法。

Result: 抽象中未提及研究结果。

Conclusion: 抽象中未给出明确结论。
...
### [151] [Mitigating Spreadsheet Risk in Complex Multi-Dimensional Models in Excel](https://arxiv.org/abs/1802.01640)
*Steve Litt*

Main category: cs.SE

TL;DR: “PivotModel”是一种旨在减轻复杂多维Excel模型中电子表格风险的解决方案，它利用Excel平台的功能，类似于数据透视表。


<details>
  <summary>Details</summary>
Motivation: 微软Excel虽然功能强大、灵活且易用，但电子表格操作耗时且容易出错，尤其是在“复杂多维模型”中，这使得企业难以控制电子表格风险。

Method: 提出了一种名为“PivotModel”的解决方案。它类似于数据透视表工作，但旨在利用Microsoft Excel平台的强大功能来解决“复杂多维模型”中的电子表格风险问题。

Result: 该解决方案旨在减轻特定类型问题（即需要复杂算法、挑战性层次结构和数据库回写等功能的应用）的电子表格风险，并提供报告、数据输入表单和对结果模型相关不同属性的即席分析等功能。

Conclusion: “PivotModel”是一种利用Microsoft Excel平台强大功能的解决方案，旨在有效管理和减轻复杂多维模型中常见的电子表格风险。
...
### [152] [Proposed Spreadsheet Transparency Definition and Measures](https://arxiv.org/abs/1802.01628)
*Craig Hatmaker*

Main category: cs.SE

TL;DR: 本文提出了一种电子表格建模透明度的定义，旨在使其可测量和可比较，解决了当前缺乏明确定义和客观评估方法的问题。


<details>
  <summary>Details</summary>
Motivation: 审计师对金融模型透明度缺乏精确定义，导致无法明确模型何时“透明”，缺乏量化衡量标准，也无法客观比较建模方法。

Method: 本文提出了一个电子表格建模透明度的定义。

Result: 所提出的定义足够具体，可以为审计师创建衡量标准和自动化工具，并使建模者能够客观地比较各种方法。

Conclusion: 本文为电子表格建模透明度提供了一个定义，使审计师能够评估合规性，并使建模者能够做出明智的选择。
...
### [153] [Alternative Spreadsheet Model Designs for an Operations Management Model Embedded in a Periodic Business Process](https://arxiv.org/abs/1802.00484)
*Thomas A. Grossman,Vijay Mehrotra,Mouwafac Sidaoui*

Main category: cs.SE

TL;DR: 本文评估了运营管理模型的_种电子表格实现（数据驱动、规范和新颖的表驱动技术设计），重点关注其在准确性、修改、分析和转移方面的适用性以及所需的用户复杂程度。结果发现技术设计在修改方面表现更优，并能减少错误。


<details>
  <summary>Details</summary>
Motivation: 由于在周期性业务流程中，供应链和分销规划中广泛使用的运营管理模型需要频繁修改和重用，因此需要探索不同的电子表格实现方式，以应对这些挑战。

Method: 作者考虑了_种备选的电子表格实现方案：数据驱动设计、规范（教科书）设计和新颖的（表驱动）技术设计。他们根据准确性、修改、分析和转移的适用性以及所需的培训和技术复杂程度对每种设计进行了评估。

Result: 数据驱动设计揭示了新手建模者不良的电子表格实践。技术设计在修改方面表现出色，无需手动编写或编辑单元格公式即可适应新数据和新结构元素，从而加快了修改速度并降低了出错风险。

Conclusion: 技术设计有潜力应用于其他模型类别，并为未来的研究提供了机会，表明其在增强运营管理模型灵活性和降低错误风险方面的有效性。
...
### [154] [Mitigating Spreadsheet Model Risk with Python Open Source Infrastructure](https://arxiv.org/abs/1801.09771)
*Oliver Beavers*

Main category: cs.SE

TL;DR: 该论文提出使用Python和开源工具为电子表格模型开发可重现的审计工具，以解决电子表格开发者非软件工程师导致错误率高的问题。


<details>
  <summary>Details</summary>
Motivation: 电子表格模型类似于软件，但电子表格开发者并非软件工程师，因此缺乏传统的软件工程工具和协议，导致最终结果的错误率较高。

Method: 该论文为电子表格建模专业人员使用免费的、开源的Python软件包开发可重现的审计工具奠定了基础。

Result: 使利益相关者能够开发清晰定义的模型“预言机”，用于测试和审计电子表格计算。

Conclusion: 通过使用Python和开源软件包，电子表格建模可以实现更可靠的审计并减少错误。
...
### [155] [Structuring Spreadsheets with the "Lish" Data Model](https://arxiv.org/abs/1801.08603)
*Alan Hall,Michel Wermelinger,Tony Hirst,Santi Phithakkitnukoon*

Main category: cs.SE

TL;DR: 本文提出了一种名为“lish”的新型数据模型，旨在替代传统电子表格的网格结构，通过将单元格组织成嵌套列表并引入可选模板来捕捉更高层次的数据结构，同时保持电子表格的简洁性，以提高理解力并减少错误。


<details>
  <summary>Details</summary>
Motivation: 传统电子表格虽然灵活，但其单个单元格缺乏对整体结构的认知，这阻碍了用户理解并增加了公式重复的风险，从而提高了出错的可能性。

Method: 本文探索了一种名为“lish”的新型数据模型，它将单元格组织成嵌套列表。用户可以在这些列表中选择性地使用模板来原型化重复结构。这些模板元素是表格的正式成员，并且可以包含内部结构。

Result: 一个小型演示应用程序展示了“lish”模型的实际操作效果。

Conclusion: “lish”模型为电子表格提供了一种替代传统网格的方法，它旨在捕捉更高层次的结构，同时保持电子表格的简洁性，从而有望提高数据理解并减少错误。
...
### [156] [Automated Refactoring of Nested-IF Formulae in Spreadsheets](https://arxiv.org/abs/1712.09797)
*Jie Zhang,Shi Han,Dan Hao,Lu Zhang,Dongmei Zhang*

Main category: cs.SE

TL;DR: 电子表格中的“嵌套IF”公式存在可读性差、认知成本高且易出错的问题。本文提出了一种基于AST的自动化重构方法，通过检测和去除冗余，并使用内置函数重组语义。该方法对2700万个嵌套IF公式的真实语料库实现了99%以上的异味消除，超过一半的重构将嵌套级别降低了一半以上，并且用户更喜欢重构后的公式。


<details>
  <summary>Details</summary>
Motivation: 电子表格中的嵌套IF表达式是一种常见的“异味”，具有可读性低、认知成本高、易出错的特点。终端用户缺乏编程知识来解决这个问题，目前也缺乏有效的自动化方法。

Method: 一种基于AST的自动化方法，主要包括两个步骤：1）在AST上检测并去除逻辑冗余；2）识别被碎片化和分散的高级语义，并使用简洁的内置函数重新组合语法。

Result: 该方法在真实语料库中对2700万个嵌套IF公式的异味消除了99%以上。超过50%的重构将嵌套IF的嵌套级别降低了一半以上。一项涉及49名参与者的调查显示，大多数情况下参与者更喜欢重构后的公式，并认同这种自动化重构方法的必要性和实用性。

Conclusion: 本文提出的基于AST的自动化重构方法能够有效解决电子表格中的“嵌套IF”异味问题，提高了可读性并降低了复杂性，并且受到用户的青睐。
...
### [157] [Spreadsheet Guardian: An Approach to Protecting Semantic Correctness throughout the Evolution of Spreadsheets](https://arxiv.org/abs/1612.03813)
*Daniel Kulesz,Verena Käfer,Stefan Wagner*

Main category: cs.SE

TL;DR: 电子表格在商业中至关重要，但容易出错，尤其是在协作维护期间。Spreadsheet Guardian 通过将测试规则的规范与执行分离，自动检测语义错误并保护用户。评估表明它易于学习和应用，使用户对电子表格的正确性有更实际的认识。


<details>
  <summary>Details</summary>
Motivation: 电子表格在许多组织中扮演着业务关键角色，但由于故障电子表格导致的错误决策表明需要严肃的质量保证。此外，在电子表格协作维护期间，几乎没有支持来确保在此过程中电子表格保持正确。

Method: 开发了一种名为 Spreadsheet Guardian 的方法，它将电子表格测试规则的规范与执行分离。通过自动执行用户定义的测试规则，该方法能够检测语义故障，并保护所有协作的电子表格用户在维护期间引入故障。为评估，在 Microsoft Excel 中实现了一个代表性的测试技术作为插件。

Result: 通过对 29 名最终用户和 42 名计算机科学学生进行的两项实证评估，结果表明该技术易于学习和应用。此外，在完成维护后，使用该技术“保护”的电子表格的参与者比仅使用基于静态分析技术的“经典”非交互式测试规则的参与者对电子表格的正确性有更实际的认识。
...
### [158] [On Evidence-based Risk Management in Requirements Engineering](https://arxiv.org/abs/1707.00144)
*Daniel Méndez Fernández,Michaela Tießler,Marcos Kalinowski,Michael Felderer,Marco Kuhrmann*

Main category: cs.SE

TL;DR: 该论文提出并验证了一种基于证据的方法，利用来自228家公司的跨公司调查数据来评估需求工程（RE）中特定于上下文的风险，并使用概率网络预测RE现象。在6家公司进行的初步验证表明，该方法提高了对个体RE风险因素的认识。


<details>
  <summary>Details</summary>
Motivation: 需求工程（RE）对上下文的敏感性使得难以有效控制其中的问题，从而阻碍了有效的风险管理，无法实现早期纠正或预防措施。目前，关于特定于上下文的RE现象的实证知识仍然很少，而这对于RE中有效的上下文敏感风险管理是必不可少的。

Method: 我们使用来自228家公司的调查数据，并构建了一个概率网络，以支持对特定于上下文的RE现象的预测。我们使用电子表格实现了这种方法，以支持轻量级的风险评估。

Result: 我们在6家公司进行的初步验证结果增强了我们的信心，即该方法提高了对RE中个体风险因素的认识，并且反馈进一步促进了该方法在实践中的推广。

Conclusion: 该方法提高了对RE中个体风险因素的认识，并显示出在实践中推广的潜力。
...
### [159] [Tabula: A Language to Model Spreadsheet Tables](https://arxiv.org/abs/1707.02833)
*Jorge Mendes,João Saraiva*

Main category: cs.SE

TL;DR: 该论文介绍了一种名为 Tabula 的新型建模语言，它扩展了现有的电子表格模型，增加了类型约束和带重复的嵌套类等特性，提高了表达能力，并包含一个双向转换引擎以确保模型和电子表格的同步。


<details>
  <summary>Details</summary>
Motivation: 电子表格灵活易用但容易出错。现有的电子表格模型语言表达能力有限，无法建模真实世界电子表格中的许多特性。

Method: 论文引入了 Tabula 建模语言。Tabula 扩展了现有的电子表格模型，增加了类型约束和带重复的嵌套类等特性。此外，Tabula 还包含一个双向转换引擎。

Result: Tabula 比其他模型更具表达力，并且可以进一步扩展。其双向转换引擎保证了模型或电子表格更新后的同步。

Conclusion: Tabula 提供了一种更具表达力且可扩展的电子表格建模解决方案，并通过双向转换引擎确保了模型与电子表格之间的数据一致性。
...
### [160] [LEI2JSON: Schema-based Validation and Conversion of Livestock Event Information](https://arxiv.org/abs/2310.17414)
*Mahir Habib,Muhammad Ashad Kabir,Lihong Zheng*

Main category: eess.SY

TL;DR: LEI2JSON是一个Google Sheets插件，旨在帮助畜牧生产者标准化、转换和验证其牲畜事件数据为JSON格式，从而节省时间和资源。
...
Method: LEI2JSON通过为Google Sheets构建符合牲畜事件信息（LEI）模式的模板，将电子表格数据转换为JSON格式，并根据模式验证输出来实现数据标准化。

Result: 该工具使畜牧生产者能够高效地标准化数据，节省大量时间和资源，并促进牲畜事件信息以JSON格式在本地或Google Drive上无缝存储。

Conclusion: LEI2JSON工具已通过广泛的实验评估证明其有效性。
...
### [161] [The Paradox of Spreadsheet Self-Efficacy: Social Incentives for Informal Knowledge Sharing in End-User Programming](https://arxiv.org/abs/2408.08068)
*Qing,Xia,Advait Sarkar,Duncan P. Brumby,Anna Cox*

Main category: cs.HC

TL;DR: 该研究调查了影响电子表格知识共享意愿的个人、社会和软件相关因素。发现自我效能感和声誉收益会增加知识共享意愿，而编码工作量大会降低意愿。同时注意到用户在通用电子表格熟练度上的自我效能感较低，但在工作相关场景中却较高。


<details>
  <summary>Details</summary>
Motivation: 为了更好地理解个人（自我效能感）、社会（声誉收益、同事间的信任）和软件相关（编码工作量）变量如何影响电子表格知识共享意愿。

Method: 基于对行政和财务角色的电子表格用户（n=100）的调查数据，进行了多元回归分析。

Result: 高水平的电子表格自我效能感和认为分享会带来声誉收益的认知预示着更高的知识共享意愿，但认为知识编码费力的人表现出较低的知识共享意愿。此外，无论职业如何，用户倾向于报告其通用电子表格熟练度方面的自我效能感较低，尽管他们也报告了在工作相关场景中使用电子表格的自我效能感较高。

Conclusion: 研究结果表明，认识并针对这些社会和个人变量进行设计，有助于避免有经验的个体不必要地避免分享的情况，这对电子表格设计具有启示意义。
...
### [162] [Buckaroo: A Direct Manipulation Visual Data Wrangler](https://arxiv.org/abs/2507.16073)
*Annabelle Warner,Andrew McNutt,Paul Rosen,El Kindi Rezig*

Main category: cs.HC

TL;DR: 本文介绍了 Buckaroo，一个可视化系统，旨在通过自动发现异常、建议修正方案以及允许对数据进行可视化操作来简化数据整理过程。


<details>
  <summary>Details</summary>
Motivation: 数据整理是数据科学开发中耗时且易出错的关键阶段，传统的手动方法效率低下且容易导致数据质量问题。

Method: Buckaroo 系统通过以下方式工作：1) 自动发现并推荐存在异常的“有趣”数据组；2) 建议用户可选择的数据整理操作来修复异常；3) 允许用户通过显示整理操作的效果并提供撤销/重做功能来直观地操作数据，支持数据整理的迭代性质。

Result: Buckaroo 作为一个可视化系统，通过突出显示数据差异并支持直接操作视觉对象进行即时更正，解决了数据整理中的挑战。

Conclusion: Buckaroo 是一个可视化系统，通过自动化异常检测、建议修复方案以及提供带有撤销/重做功能的交互式视觉操作，旨在使数据整理更加高效且不易出错。
...
### [163] [SheetMind: An End-to-End LLM-Powered Multi-Agent Framework for Spreadsheet Automation](https://arxiv.org/abs/2506.12339)
*Ruiyan Zhu,Xi Cheng,Ke Liu,Brian Zhu,Daniel Jin,Neeraj Parihar,Zhoutian Xu,Oliver Gao*

Main category: cs.HC

TL;DR: SheetMind 是一个模块化的多智能体LLM框架，通过自然语言指令实现电子表格自动化，单步任务成功率为80%，多步任务成功率约为70%。
...
Method: SheetMind 采用多智能体框架：管理器智能体分解任务，动作智能体通过BNF语法将指令转换为结构化命令，反思智能体验证生成的动作与用户意图的一致性。该系统已集成到Google Sheets中。
...
### [164] ["How do you even know that stuff?": Barriers to expertise sharing among spreadsheet users](https://arxiv.org/abs/2506.09216)
*Qing,Xia,Advait Sarkar,Duncan Brumby,Anna Cox*

Main category: cs.HC

TL;DR: 电子表格专家在知识共享方面面临挑战，这受到社会规范、个人评估和技术设计复杂互动的影响。


<details>
  <summary>Details</summary>
Motivation: 以前的研究表明，电子表格专家往往无法分享他们的知识。本文旨在探讨社会规范和观念如何显著影响用户参与共享行为。

Method: 对来自两个不同样本的31位专业电子表格用户进行了半结构化访谈。

Result: 发现电子表格提供者在调整个性化策略以适应主观标准和评估分享的适当社会时机方面面临挑战。此外，对自身专业知识的冲突性自我评估、对知识价值的轻视性规范信念以及对协作可能带来干扰的担忧进一步阻碍了分享。

Conclusion: 研究结果表明，技术设计和社会动态之间的复杂互动如何塑造了功能丰富软件背景下的协作学习行为，并为应对这种紧张关系提供了设计启示。
...
### [165] [Prompting in the Dark: Assessing Human Performance in Prompt Engineering for Data Labeling When Gold Labels Are Absent](https://arxiv.org/abs/2502.11267)
*Zeyu He,Saniya Naphade,Ting-Hao 'Kenneth' Huang*

Main category: cs.HC

TL;DR: 在没有黄金标准标签的情况下，用户迭代式地提示LLM进行数据标注的可靠性很低。研究发现，在这种“黑暗中提示”的场景下，仅有少数参与者通过多次迭代提高了标注准确性，自动化提示优化工具也表现不佳。这强调了黄金标签的重要性以及人类提示工程中自动化支持的需求和风险。


<details>
  <summary>Details</summary>
Motivation: 探究用户在没有黄金标准标签来衡量进展的情况下，进行提示工程的有效性，以及他们是否能通过多次迭代接近期望结果。特别是，论文关注LLM驱动的数据标注场景，即“黑暗中提示”——用户在没有手动标注基准的情况下迭代地提示LLM来标注数据。

Method: 开发了一个名为PromptingSheet的Google Sheets插件，允许用户通过电子表格编写、修改和迭代地标注数据。通过一项有20名参与者参与的研究来评估该方法。

Result: 研究发现“黑暗中提示”非常不可靠——在四次或更多次迭代后，只有9名参与者提高了标注准确性。当黄金标签很少时，像DSPy这样的自动化提示优化工具也表现不佳。
...
### [166] [When Copilot Becomes Autopilot: Generative AI's Critical Risk to Knowledge Work and a Critical Solution](https://arxiv.org/abs/2412.15030)
*Advait Sarkar,Xiaotong,Xu,Neil Toronto,Ian Drosos,Christian Poelitz*

Main category: cs.HC

TL;DR: 生成式AI对知识工作既有风险（如批判性思维退化）也有机遇。本文提出通过设计AI界面来培养批判性思维，并以一个电子表格原型为例，该原型建议筛选标准并生成“质疑”文本来批判AI的输出。


<details>
  <summary>Details</summary>
Motivation: 生成式AI可能引入错误并导致人类批判性思维退化，尤其是在复杂的知识工作流程（如电子表格）中。本文旨在解决这一风险，并通过精心设计AI界面来促进批判性思维。

Method: 本文讨论了一个用于电子表格中关键筛选活动的A I原型系统。该系统利用生成式A I建议筛选标准，并应用这些标准对行进行排序。它还生成“质疑”（provocations）：简短的文本片段，批判A I生成的标准，突出风险、缺陷和替代方案。

Result: 该原型系统为现代A I辅助知识工作中的批判性思维工具开辟了一个丰富且完全未探索的设计空间。

Conclusion: 本文提出了一个将A I作为批评者或质疑者的研究议程，包括关于质疑出现的位置和时间、形式和内容以及潜在设计权衡的问题。核心结论是，A I界面应旨在培养而非削弱批判性思维。
...
### [167] [Understanding and Evaluating Trust in Generative AI and Large Language Models for Spreadsheets](https://arxiv.org/abs/2412.14062)
*Simon Thorne*

Main category: cs.HC

TL;DR: 本论文提出了一个可信度框架，用于评估生成式AI和大型语言模型在自动化电子表格公式创建方面的输出，旨在解决幻觉和偏见等问题。


<details>
  <summary>Details</summary>
Motivation: 生成式AI和大型语言模型在自动化电子表格公式创建方面具有潜力，但由于幻觉、偏见和用户技能差异，其输出不能被假定为准确或值得信赖。

Method: 提出了一个基于评估公式的透明度（通过可解释性和可见性）和可靠性（通过一致性、准确性、偏见和公平性）的可信度框架。论文还研究了幻觉、训练数据偏见和不良提示等因素对这些指标的影响。

Result: 该论文提出了一个评估生成式AI在电子表格公式创建中可信度的框架，并探讨了影响这些评估指标的驱动因素。

Conclusion: 论文考虑了对技术不信任的例子并探讨了其后果。
...
### [168] [Dynamic Prompt Middleware: Contextual Prompt Refinement Controls for Comprehension Tasks](https://arxiv.org/abs/2412.02357)
*Ian Drosos,Jack Williams,Advait Sarkar,Nicholas Wilson*

Main category: cs.HC

TL;DR: 该论文调查了生成式AI的提示中间件，比较了动态和静态提示细化控制。结果显示，动态控制更受青睐，因为它提供了更多控制并降低了用户障碍。


<details>
  <summary>Details</summary>
Motivation: 对于许多用户来说，有效提示生成式AI具有挑战性，尤其是在表达理解任务的上下文方面。提示中间件旨在解决这一障碍，但用户在表达足够的控制方面仍然存在障碍，以便他们能够收到符合其偏好的AI响应。

Method: 1. 进行了一项形成性调查（n=38），研究了用户在理解任务中对AI生成解释的控制需求，发现标准化但可预测的提示支持与适应性但不可预测的定制支持之间存在权衡。
2. 实现了两种提示中间件方法：动态提示细化控制（Dynamic PRC）和静态提示细化控制（Static PRC）。动态PRC生成上下文相关的UI元素，根据用户提示和需求提供提示细化；静态PRC提供预设的通用细化列表。
3. 通过一项受控用户研究（n=16）评估了这两种方法对用户控制AI响应以生成更好解释的影响。

Result: 用户偏爱动态PRC方法，因为它提供了更多的控制，降低了提供上下文的障碍，并鼓励了任务的探索和反思。然而，推理不同生成控件对最终输出的影响仍然具有挑战性。

Conclusion: 动态提示中间件可以通过提供更大的控制和引导用户获得更好的AI响应来改善生成式AI工作流程的用户体验。研究结果为未来的动态PRC系统设计提供了启示。
...
### [169] [Subject integration with spreadsheets -- Ignoring education is the greatest risk ever](https://arxiv.org/abs/2409.12975)
*Mária Csernoch,Ádám Gulácsi,Júlia Csernoch*

Main category: cs.HC

TL;DR: 在TPACK框架下，学科整合是学校引入有意义的数字化和信息化的可行方案，可以弥合“严肃信息学”与“数字素养”之间的鸿沟。本文详细介绍了如何使用电子表格解决三年级的传统任务，以及由此可以培养教师和学生的哪些技能、能力和计算机科学知识。


<details>
  <summary>Details</summary>
Motivation: 在学校引入有意义的数字化和信息化，弥合“严肃信息学”与“数字素养”之间的差距，并培养学生未来工作所需的技能。

Method: 详细说明如何使用电子表格解决三个传统的小学三年级任务。

Result: 通过电子表格解决任务可以培养教师和学生的技能、能力和计算机科学知识。此外，分析、理解、规划和讨论任务的过程与在电子表格中的操作同样重要。

Conclusion: 学科整合，特别是通过使用电子表格等数字工具，是发展学生数字素养和为未来工作做准备的有效方法，强调了分析和讨论技能与实际应用同样重要。
...
### [170] [Exploring Higher Education Competencies through Spreadsheet Self-Assessment and Time](https://arxiv.org/abs/2409.12974)
*Maria Csernoch,Judit T. Kiss,Viktor Takács,Domicián Máté*

Main category: cs.HC

TL;DR: 学生对自己的电子表格能力评估不准确，且在Excel上表现不如纸质版，所需时间也更长，这违反了数字原住民天生懂电脑的假设。


<details>
  <summary>Details</summary>
Motivation: 旨在探讨高等教育学生电子表格能力和可靠性，并通过自我评估和实际问题解决来验证数字原住民在Excel上的表现。

Method: 通过自我评估和实际问题解决练习，包括纸质版和Excel任务。

Result: 学生往往不准确地评估自己的电子表格能力。在Excel任务中，学生需要两倍的时间才能达到与纸质版相同的高分。研究未能证实学生在Excel上的表现优于纸质版。

Conclusion: 强调了在数字技能发展和时间管理中准确自我评估的重要性，尤其是在技术驱动的学科中。数字原住民也需要计算机科学教育。
...
### [171] [Data Guards: Challenges and Solutions for Fostering Trust in Data](https://arxiv.org/abs/2407.14042)
*Nicole Sultanum,Dennis Bromley,Michael Correll*

Main category: cs.HC

TL;DR: 本论文调查了数据信任的建立方式，发现了数据验证和核实的必要性，并提出了“数据卫士”来促进信任。


<details>
  <summary>Details</summary>
Motivation: 数据驱动的决策面临数据质量问题的威胁，这需要信任或验证。本论文旨在理解建立数据信任的策略和障碍。

Method: 对数据制品（如电子表格、图表和仪表板）的生产者和消费者进行了一系列访谈。

Result: 数据验证和核实存在反复出现的需求，但缺乏现有标准，尤其是在数据消费者中。

Conclusion: 提出了一套“数据卫士”（方法和工具）来促进对数据制品的信任。
...
### [172] [Smart Portable Computer](https://arxiv.org/abs/2405.05292)
*Niladri Das*

Main category: cs.HC

TL;DR: 本文介绍了一种“便携式智能电脑”，它是一种紧凑、节能、经济高效的设备，性能媲美传统台式机，支持多种应用和编程工具，旨在解决疫情期间学生和工作人员获取足够计算设备的困难。


<details>
  <summary>Details</summary>
Motivation: 在COVID-19疫情期间，学生和工作人员在获取价格合理且规格充足的个人电脑（PC）方面遇到困难。此外，传统笔记本电脑在工作时也显得笨重。

Method: 提出并介绍了一种名为“便携式智能电脑”的创新设备。

Result: “便携式智能电脑”具有与传统台式机媲美的速度和性能，同时体积小巧、节能且成本效益高。它提供无缝的桌面体验，并支持Python、C、C++等编程语言以及Keil和Xilinx等编译器。

Conclusion: “便携式智能电脑”提供了一种面向未来的计算解决方案，满足了对经济实惠、便携且高性能计算的需求，尤其是在疫情等特殊时期具有重要意义。
...
### [173] ["My toxic trait is thinking I'll remember this": gaps in the learner experience of video tutorials for feature-rich software](https://arxiv.org/abs/2404.07114)
*Ian Drosos,Advait Sarkar,Andrew D. Gordon*

Main category: cs.HC

TL;DR: 该论文研究了视频教程中阻碍学习的“差距”，通过分析评论、采访创作者并提出设计方案来解决这些问题。


<details>
  <summary>Details</summary>
Motivation: 视频教程在学习中很受欢迎，但学习者在观看和跟随过程中会遇到我们称之为“差距”的问题，这些问题会阻碍学习。

Method: 通过收集和分析来自YouTube、TikTok和Instagram上90个Microsoft Excel视频教程的360条评论，开发了差距理论和分类法。采访了8位有影响力的教程创作者，了解他们观众的体验和应对方法。最后，提出了两种设计方案以获取创作者的反馈和替代设计思路。

Result: 识别并建立了一个关于视频教程中“差距”的理论和分类法，阐明了它们如何成为学习障碍。从有影响力的教程创作者那里获得了关于他们观众遇到的差距、他们如何解决这些问题以及他们的创作过程和挫折的见解。

Conclusion: 论文提出了旨在解决视频教程中学习障碍的设计方案，并期望通过创作者的反馈进一步完善。
...
### [174] [Supporting Annotators with Affordances for Efficiently Labeling Conversational Data](https://arxiv.org/abs/2403.07762)
*Austin Z. Henley,David Piorkowski*

Main category: cs.HC

TL;DR: 本文介绍了一种名为CAL的新型数据标注界面，旨在通过防止不当标签、提供指导、整合文档和高效查看历史标签来提高数据标注的效率和用户体验。用户研究表明，CAL相比传统电子表格能降低认知负荷，易用性更强，并受到用户青睐，同时未增加任务时间。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统严重依赖大量准确标注的数据，但众包标注耗时且昂贵，增加了工作量和枯燥感。

Method: 设计了CAL界面，其关键设计包括：阻止不当标签选择、在需要时指导用户选择适当标签、将标注文档整合到界面中、提供高效查看先前标签的方法。实现了一个生产质量的CAL版本，并报告了一项将CAL与标准电子表格进行比较的用户研究评估。

Result: 用户报告使用CAL时认知负荷较低，未增加任务时间，用户认为CAL更容易使用，并且用户更喜欢CAL而不是电子表格。

Conclusion: CAL作为一种数据标注界面，能够有效降低用户认知负荷、提高易用性并获得用户偏好，是优于传统电子表格的标注工具。
...
### [175] [Prompting for Discovery: Flexible Sense-Making for AI Art-Making with Dreamsheets](https://arxiv.org/abs/2310.09985)
*Shm Garanganao Almeda,J. D. Zamfirescu-Pereira,Kyu Won Kim,Pradeep Mani Rathnam,Bjoern Hartmann*

Main category: cs.HC

TL;DR: 该论文介绍了DreamSheets，一个基于LLM函数的电子表格界面，旨在帮助用户更有效地探索Text-to-Image（TTI）模型的提示词空间。


<details>
  <summary>Details</summary>
Motivation: Text-to-Image（TTI）模型的设计空间探索（DSE）涉及一个庞大而不透明的图像输出空间，以及同样庞大的超参数和提示文本输入空间。对提示输入的微小调整可能会产生意想不到的、截然不同的图像。因此，研究的动机是探索界面如何支持终端用户可靠地引导提示词空间探索，以获得有趣的结果。

Method: 本文提出了DreamSheets，一个设计探针，它在一个电子表格界面中支持LLM（大型语言模型）函数，用于辅助提示词构建和同时显示生成结果。研究通过两项实验进行：一项初步的实验室研究和一项针对五位专家艺术家的长期研究。

Result: 两项研究揭示了参与者应对TTI设计空间探索挑战所使用的一系列策略，以及支持这些策略所需的界面功能，例如使用文本生成来定义局部“探索轴”。

Conclusion: 研究将这些见解提炼成一个UI模型，以指导未来的接口设计。
...
### [176] [Does Using ChatGPT Result in Human Cognitive Augmentation?](https://arxiv.org/abs/2401.11042)
*Ron Fulbright,Miranda Morrison*

Main category: cs.HC

TL;DR: 该论文调查了使用ChatGPT对人类认知增强的影响，通过实验发现ChatGPT并非总能带来认知增强，有时甚至会误导用户。


<details>
  <summary>Details</summary>
Motivation: 最近出现的ChatGPT等认知系统引起了人们对其如何增强人类认知能力的好奇。

Method: 通过两个实验，比较了使用ChatGPT和不使用ChatGPT生成的结果。

Result: 发现使用ChatGPT并不总是导致认知增强，也不能取代人类的判断、辨别和评估。在某些情况下，ChatGPT甚至会导致误导用户，从而产生负面的认知增强。

Conclusion: ChatGPT尚未完全取代人类的判断力，并且在某些任务中可能无法带来认知增强，甚至可能产生负面影响。
...
### [177] [Co-audit: tools to help humans double-check AI-generated content](https://arxiv.org/abs/2310.01297)
...
TL;DR: 随着大型语言模型（LLM）生成的内容日益复杂，用户难以核实其准确性。本文介绍了“协同审计工具”，旨在帮助用户检查AI生成的内容，特别是在电子表格计算等领域。论文提出了协同审计的初步原则，并概述了相关研究挑战。


<details>
  <summary>Details</summary>
Motivation: LLM生成的复杂内容（如摘要、表格、代码）使得用户难以审计其质量或正确性。因此，需要出现辅助工具来帮助用户检查AI生成的内容，即协同审计工具。在生成式AI应用中，如果质量重要且错误后果严重（如电子表格计算），协同审计体验至关重要。

Method: 本文描述了生成模型驱动的电子表格计算协同审计工具的最新研究。提出了协同审计的初步原则。

Result: 协同审计体验对于任何生成式AI应用（在质量重要且错误后果严重的情况下）都是必不可少的。提出了协同审计的初步原则。

Conclusion: 协同审计工具对于核实AI生成内容的质量和正确性至关重要，特别是在复杂且错误后果严重的场景（如电子表格计算）。论文提出了协同审计的初步原则，并指出了未来的研究挑战。
...
### [178] ["What It Wants Me To Say": Bridging the Abstraction Gap Between End-User Programmers and Code-Generating Large Language Models](https://arxiv.org/abs/2304.06597)
*Michael Xieyang Liu,Advait Sarkar,Carina Negreanu,Ben Zorn,Jack Williams,Neil Toronto,Andrew D. Gordon*

Main category: cs.HC

TL;DR: 本文提出了一种“接地抽象匹配”方法，通过将生成的代码翻译回自然语言，帮助非专业用户更好地理解和使用代码生成大模型进行数据分析。


<details>
  <summary>Details</summary>
Motivation: 非专业用户在利用代码生成大模型（如Codex）将自然语言转换为代码时，难以有效构建查询语句，即面临“抽象匹配”的挑战，尤其是在电子表格数据分析场景中。

Method: 研究人员提出了“接地抽象匹配”方法，将生成的Python代码翻译回系统化、可预测的自然语言表达。他们通过一项（n=24）的思考-出声研究，将此方法与基于现有查询框架原则的非接地方法进行对比。

Result: 研究发现，“接地”方法显著提高了终端用户对代码生成模型范围、能力以及有效使用所需语言的理解。

Conclusion: “接地抽象匹配”能够有效弥补非专业用户与代码生成模型之间的抽象鸿沟，帮助他们更好地理解并有效利用这类工具进行代码生成。
...
### [179] [Team OS's System for Dialogue Robot Competition 2022](https://arxiv.org/abs/2210.09928)
*Yuki Kubo,Ryo Yanagimoto,Hayato Futase,Mikio Nakano,Zhaojie Luo,Kazunori Komatani*

Main category: cs.HC

TL;DR: 本文介绍了一个为2022年对话机器人竞赛开发的对话机器人系统OSbot，它结合了手动状态转换、关键词提取和情感分析，并在竞赛预赛中获得第三名。


<details>
  <summary>Details</summary>
Motivation: 开发一个对话机器人系统OSbot，以参加2022年对话机器人竞赛。

Method: 对话流基于手动描述的状态转换，转换条件利用关键词提取（基于命名实体和预定义关键词集）和情感分析（基于SVM文本分析，使用Hazumi多模态对话语料库训练）。对话流在电子表格中管理，并通过日志功能进行编辑。

Result: 该系统在竞赛预赛中获得第三名。

Conclusion: 开发的OSbot系统在2022年对话机器人竞赛的预赛中表现良好，取得了第三名的成绩。
...
### [180] [What is it like to program with artificial intelligence?](https://arxiv.org/abs/2208.06213)
*Advait Sarkar,Andrew D. Gordon,Carina Negreanu,Christian Poelitz,Sruti Srinivasa Ragavan,Ben Zorn*

Main category: cs.HC

TL;DR: 该论文探讨了大型语言模型辅助编程（LLM-assisted programming）与现有编程辅助方式的异同，并将其定义为一种全新的编程方式，具有独特的特性和挑战，尤其是在非专业用户场景中。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型已能生成代码并商业化（例如GitHub Copilot）。作者旨在探究LLM辅助编程与传统编程辅助方式的异同，并识别其独特性和挑战。

Method: 研究方法包括分析公开的LLM辅助编程经验报告、现有可用性与设计研究，以及一项用户研究。该用户研究观察了非专业终端用户程序员使用LLM辅助工具解决电子表格数据任务的情况。

Result: LLM辅助编程与编译、结对编程、通过搜索和重用进行编程等方式有相似之处，但在技术可能性和实际体验上存在根本性差异。因此，LLM辅助编程应被视为一种具有独特属性和挑战的新型编程方式。用户研究还揭示了将大型语言模型应用于终端用户编程（尤其是编程经验不足的用户）时可能出现的问题和开放研究挑战。

Conclusion: LLM辅助编程代表了一种新颖的编程范式，具有独特的特性和挑战，尤其是在非专业终端用户编程中。未来需要进一步研究以解决这些挑战。
...
### [181] [MetaGlyph: Automatic Generation of Metaphoric Glyph-based Visualization](https://arxiv.org/abs/2209.05739)
*Lu Ying,Xinhuan Shu,Dazhen Deng,Yuchen Yang,Tan Tang,Lingyun Yu,Yingcai Wu*

Main category: cs.HC

TL;DR: 本文提出MetaGlyph，一个从电子表格自动生成隐喻字形可视化（MGV）的系统。


<details>
  <summary>Details</summary>
Motivation: 隐喻字形可视化（MGV）的设计复杂，需要深入的数据理解和专业设计技能。

Method: MetaGlyph通过定性分析现有MGV设计，提出了一种新颖的MGV生成框架，包括隐喻图像选择和MGV构建。它利用蒙特卡洛树搜索算法探索设计，并提供编辑反馈。

Result: 通过一系列示例、使用场景和专家访谈验证了MetaGlyph的有效性。

Conclusion: MetaGlyph是一个有效的系统，可以自动生成并允许用户自定义的隐喻字形可视化。
...
### [182] [PoVRPoint: Authoring Presentations in Mobile Virtual Reality](https://arxiv.org/abs/2201.06337)
...
TL;DR: 该论文介绍了 PoVRPoint，一个用于在移动设备上创作演示文稿的 VR 工具集，并在幻灯片识别和对象重新排序方面显示出优势。


<details>
  <summary>Details</summary>
Motivation: 受先前 VR 知识工作研究的启发，本文探讨了在移动 VR 环境中创作演示文稿的设计空间。

Method: 我们提出了 PoVRPoint——一套将移动设备（如平板电脑）上的笔触式演示文稿编辑与 VR 提供的交互功能相结合的工具。我们研究了扩展显示空间在帮助用户识别目标幻灯片、支持幻灯片上对象的空间操作、创建动画以及促进多个可能被遮挡的形状的排列方面的效用。

Result: 研究结果表明，与仅使用平板电脑的界面相比，VR 提供的宽视野能够显著加快视觉显著目标的目标幻灯片识别时间；VR 中的三维视图与两个基线界面相比，在存在遮挡的情况下能够显著加快对象重新排序。用户研究进一步证实了交互技术是可用且令人愉快的。

Conclusion: PoVRPoint 的 VR 功能改进了移动设备上的演示文稿创作，尤其是在幻灯片导航和对象操作方面。
...
### [183] [Untidy Data: The Unreasonable Effectiveness of Tables](https://arxiv.org/abs/2106.15005)
*Lyn Bartram,Michael Correll,Melanie Tory*

Main category: cs.HC

TL;DR: 本研究探讨了数据工作者如何与表格数据交互，发现表格不仅仅用于数据清理，而是贯穿整个分析过程，通过直接交互和可读表示帮助用户理解数据。


<details>
  <summary>Details</summary>
Motivation: 通常认为表格数据处理是准备性工作，但对于许多数据工作者（非专业分析师），电子表格仍是其信息生态系统的关键部分。本研究旨在了解这些工作者如何与数据交互并进行推理。

Method: 我们对数据工作者如何与数据交互并进行推理进行了定性研究。

Result: 研究发现，数据表格除了在分析流程初始阶段进行数据清理外，还具有更广泛的用途：用户希望在整个分析过程中查看和“接触”底层数据，以支持意义构建。他们对数据进行重组、标记、分层和生成替代方案。这些直接交互和人类可读的表格表示对于理解数据含义及其用途至关重要。

Conclusion: 交互式表格本身就是一种重要的可视化表达方式；它们提供的直接数据交互为视觉分析提供了丰富的HCI设计空间；并且通过比现有视觉分析工具更灵活的人机数据交互，可以丰富意义构建。
...
### [184] [Defining and Adopting an End User Computing Policy: A Case Study](https://arxiv.org/abs/1909.00855)
*Roger Turner*

Main category: cs.HC

TL;DR: 本文是一项案例研究，介绍了卫斯理保险协会实施更新的最终用户计算（EUC）政策的经验。文章概述了计划，识别了挑战，并解释了如何克服这些挑战，包括开发了一个基于复杂性、重要性和控制情况计算风险等级的EUC风险评估应用程序，并采用风险导向方法优先处理最高风险。
...
Method: 本文是一个案例研究，描述了卫斯理保险协会引入更新的最终用户计算（EUC）政策的过程。它概述了计划，识别并解释了如何克服遇到的挑战。文中介绍了一个开发的“最终用户计算风险评估应用程序”，该应用程序根据任何给定应用程序的复杂性、重要性和控制情况（或缺乏控制）计算风险等级带。该政策采用了基于风险的方法。

Result: 成功引入了更新的最终用户计算（EUC）政策。开发并使用了“最终用户计算风险评估应用程序”，该应用程序能够根据复杂性、重要性和控制情况计算风险等级。政策采用了一种基于风险的方法，旨在优先评估和缓解最高风险，以获得最快的效益。

Conclusion: 通过引入更新的EUC政策、开发定制的风险评估应用程序并采用基于风险的方法，可以有效管理和缓解最终用户计算中的风险。
...
### [185] [Calliope: Automatic Visual Data Story Generation from a Spreadsheet](https://arxiv.org/abs/2010.09975)
*Danqing Shi,Xinyue Xu,Fuling Sun,Yang Shi,Nan Cao*

Main category: cs.HC

TL;DR: Calliope系统能根据电子表格自动生成可视化数据故事，并提供在线编辑器进行修订。


<details>
  <summary>Details</summary>
Motivation: 由于技术门槛高，生成可视化数据故事非常困难，现有工具效率低下且需要用户技能。

Method: Calliope采用逻辑导向的蒙特卡洛树搜索算法探索数据空间，生成并组织数据事实。它基于信息论衡量数据事实的重要性，将每个事实可视化为图表并自动生成描述。系统还提供一个在线故事编辑器方便修订。

Result: 通过三个示例故事、两次对照实验和对10位领域专家的访谈评估表明，Calliope有助于高效地生成可视化数据故事。

Conclusion: Calliope系统能有效自动化并简化可视化数据故事的创建，克服了当前的技术和效率障碍。
...
### [186] [Pen-based Interaction with Spreadsheets in Mobile Virtual Reality](https://arxiv.org/abs/2008.04543)
*Travis Gesslein,Verena Biener,Philipp Gagel,Daniel Schneider,Per Ola Kristensson,Eyal Ofek,Michel Pahud,Jens Grubert*

Main category: cs.HC

TL;DR: 该研究提出了一种利用虚拟现实（VR）、平板电脑和手写笔输入来增强移动场景下电子表格交互的工具集，通过沉浸式显示和空间交互提高效率。


<details>
  <summary>Details</summary>
Motivation: 电子表格在移动设备上操作困难。VR提供了沉浸式大显示空间，但移动工作者（如在飞机上）的物理交互空间有限，本研究旨在弥合这一差距。

Method: 本文提出一个工具集，结合沉浸式VR头显和手写笔输入，增强平板电脑上的电子表格交互。它利用平板周围和前方的空间进行数据和元数据可视化（如扩展显示、调试依赖），并提供高效的创建和编辑工具（如分层菜单、依赖可视化、凝视与触控切换标签页）。

Result: 通过基于视频的在线调查和专家评估人类表现潜力的方法，研究了所提出工具集的可行性。

Conclusion: VR、平板电脑和手写笔输入的结合，通过利用沉浸式显示和空间交互，为提高电子表格交互的生产力开辟了许多可能性。
...
### [187] [EQUS -- helping to see formulae](https://arxiv.org/abs/2007.00003)
*Chris Roast*

Main category: cs.HC

TL;DR: 本文介绍了EQUS，一种用于电子表格公式的交互式可视化工具，旨在帮助用户理解复杂的电子表格数据。该工具最初为青少年学习者设计，通过迭代改进，现在已推广到更广泛的电子表格用户，并已作为MS Excel的插件开发。


<details>
  <summary>Details</summary>
Motivation: 电子表格公式广泛用于数值处理和建模，但很容易被误解。需要一种方法来简化信息并帮助人们理解复杂的数据，特别是在电子表格中。

Method: 本文描述了EQUS的设计、开发和评估过程。开发过程涉及迭代改进，与初始目标受众（青少年学习者）互动，进行重新设计和形成性评估。

Result: EQUS的可视化技术已被证明对初始目标受众之外的电子表格用户也具有广泛的相关性。EQUS已开发成为MS Excel的完全集成插件。

Conclusion: EQUS作为一种交互式可视化工具，成功地简化了电子表格公式，使其更容易被广泛的用户理解，并最终被集成到MS Excel中。
...
### [188] [Taggle: Combining Overview and Details in Tabular Data Visualizations](https://arxiv.org/abs/1712.05944)
*Katarina Furmanova,Samuel Gratzl,Holger Stitz,Thomas Zichner,Miroslava Jaresova,Alexander Lex,Marc Streit*

Main category: cs.HC

TL;DR: Taggle 是一种表格可视化技术，用于探索和展示大型复杂表格，它以项目为中心，采用类似电子表格的方法，通过视觉编码单独可视化每一行，并引入数据驱动的聚合、多列排序、数据选择和过滤等交互方法。


<details>
  <summary>Details</summary>
Motivation: 大多数表格数据可视化技术侧重于概览，但许多实际分析任务需要调查感兴趣的单个项目，并且将一个项目与可能很大的表格的其余部分关联起来也很重要。

Method: Taggle 采用以项目为中心、类似电子表格的方法，使用单元格的视觉编码单独可视化源数据中的每一行。同时，Taggle 引入了数据子集的数据驱动聚合。聚合策略辅以专门用于回答特定分析问题的交互方法，例如基于多列的排序以及丰富的数据选择和过滤功能。

Result: 通过一个领域专家在复杂基因组数据分析中用于药物发现的案例研究展示了 Taggle。

Conclusion: Taggle 为探索和展示大型复杂表格提供了一种有效的解决方案，特别适用于以项目为中心的分析以及将单个项目与更广泛的数据集关联起来的需求。
...
### [189] [Are digital natives spreadsheet natives?](https://arxiv.org/abs/1909.00865)
*Maria Csernoch,Piroska Biró*

Main category: cs.HC

TL;DR: 本研究测试了信息学一年级学生在电子表格环境中算法技能和知识迁移能力，发现他们尽管接受过培训且是“数字原住民”，但在解决问题时仍面临严重困难。使用基于算法的多级数组公式的学生表现更好，表明需要正式的、高数学能力、基于算法的培训。


<details>
  <summary>Details</summary>
Motivation: 调查信息学一年级学生在电子表格环境中的算法技能和知识迁移能力，挑战他们因接受过培训或作为“数字原住民”而具备熟练技能的假设。

Method: 通过电子表格问题测试信息学一年级学生，并观察他们解决问题的表现，特别是他们使用的解决方案类型（基于算法的数组公式与特定问题的内置函数）。

Result: 学生在解决电子表格问题时遇到严重困难，结果不佳。使用基于算法、多级数组公式的学生比使用特定问题、不相关内置函数的学生表现更好。

Conclusion: 学生，无论其出生日期和所属的数字代际如何，都非常需要由专家教师提供的正式的、高数学能力、基于算法的培训。
...
### [190] [Somewhere Around That Number: An Interview Study of How Spreadsheet Users Manage Uncertainty](https://arxiv.org/abs/1905.13072)
*Judith Borghouts,Andrew D. Gordon,Advait Sarkar,Kenton P. O'Hara,Neil Toronto*

Main category: cs.HC

TL;DR: 该研究通过访谈发现，电子表格用户在处理不确定性数据时面临挑战，他们处理不确定性的方式受电子表格用途和自身目标影响。当前电子表格工具对管理不确定性的支持有限，用户常需采用变通方法。


<details>
  <summary>Details</summary>
Motivation: 电子表格用户在处理不确定性数据时常因使用非正式启发式方法而导致错误结论，或忽略、简化不确定性，影响决策质量。现有研究尚未充分理解用户在电子表格中如何处理不确定性。

Method: 对来自不同领域的11名电子表格用户进行了访谈研究。

Result: 用户处理不确定性的方式受电子表格在其工作中的角色（数据库、模板、计算工具、记事本、探索工具）和用户目标（计算和比较不同情景、理解不确定性本质、简化不确定性以便展示）影响。当前电子表格提供的工具支持有限，用户需要各种变通方法。

Conclusion: 电子表格用户在处理不确定性时面临工具限制和复杂情境。为了更好地支持用户，未来的电子表格工具需要更有效地解决用户在处理不确定性数据时的多样化需求和目标。
...
### [191] [Characterizing Scalability Issues in Spreadsheet Software using Online Forums](https://arxiv.org/abs/1801.03829)
*Kelly Mack,John Lee,Kevin Chang,Karrie Karahalios,Aditya Parameswaran*

Main category: cs.HC

TL;DR: 本文通过抓取Reddit论坛数据分析了Excel用户面临的挑战，特别是处理大量数据时的问题，旨在为下一代电子表格软件设计提供参考。


<details>
  <summary>Details</summary>
Motivation: 传统可用性研究成本高昂，促使研究人员转向利用在线论坛数据进行大规模、低成本的工具分析。本文旨在运用此方法研究Excel用户在使用过程中遇到的挑战，特别是因其多功能性和数据量大而导致的错误。

Method: 通过抓取Reddit网站上关于Excel的帖子，收集了用户问题和抱怨的数据集。研究具体探讨并描述了用户在使用电子表格软件时普遍遇到的问题，以及特别是处理大量数据时产生的问题。

Result: 研究探索并描述了用户在使用电子表格软件时遇到的各种问题，尤其关注了因电子表格中包含大量数据而导致的问题。

Conclusion: 本文讨论了研究结果对下一代电子表格软件设计所具有的启示和影响。
...
### [192] [The Reification of an Incorrect and Inappropriate Spreadsheet Model](https://arxiv.org/abs/1801.10249)
*Grenville J. Croll*

Main category: cs.HC

TL;DR: 当信息被加载到电子表格中时，它会获得其不应有的特性，即使这些信息是错误或不恰当的，也会变得“具体化”。一项案例研究在一个小型非营利组织中观察到了这种现象。


<details>
  <summary>Details</summary>
Motivation: 了解电子表格中的信息是如何获得不应有的特性，并即使在不正确或不恰当的情况下也能被具体化。

Method: 通过一个案例研究，在一个小型非营利组织中近距离观察了一个明显不正确和不恰当的电子表格模型的具体化过程。

Result: 在一个小型非营利组织中观察到了一个明显不正确和不恰当的电子表格模型的具体化。

Conclusion: 电子表格中的信息即使是错误的或不恰当的，也会获得不应有的属性，变得具体化，从而被赋予可信度、正确性等。
...
### [193] [Dynamic Kernel Matching for Non-conforming Data: A Case Study of T-cell Receptor Datasets](https://arxiv.org/abs/2103.10472)
*Jared Ostmeyer,Scott Christley,Lindsay Cowell*

Main category: q-bio.QM

TL;DR: 本文介绍了一种名为动态核匹配 (DKM) 的方法，旨在修改现有统计分类器以处理非标准数据。该方法在T细胞受体 (TCR) 序列和库数据上进行了测试，用于疾病诊断，并成功识别出与实验观察结果一致的模式。


<details>
  <summary>Details</summary>
Motivation: 大多数统计分类器设计用于处理规整的数据结构，但许多现实世界的数据并不符合这种结构，因此需要一种方法来发现这些非规整数据中的模式，尤其是在疾病诊断领域。

Method: 提出了动态核匹配 (DKM) 方法，用以改造现有统计分类器，使其能够处理非规整数据。作者将此方法应用于T细胞受体 (TCR) 序列数据集（按疾病抗原标记）和TCR库数据集（按患者巨细胞病毒血清状态标记）。

Result: 成功地将增强了DKM的统计分类器拟合到两个数据集中。使用标准和允许不确定诊断的指标报告了在保留数据上的性能。此外，识别了分类器用于生成预测的模式。

Conclusion: 发现的模式与实验研究的观察结果一致，表明DKM是一种处理非规整数据的有效方法，并且能够揭示有意义的生物学见解。
...
### [194] [Mathematics of Digital Hyperspace](https://arxiv.org/abs/2103.15203)
*Jeremy Kepner,Timothy Davis,Vijay Gadepally,Hayden Jananthan,Lauren Milechin*

Main category: cs.MS

TL;DR: 这篇论文探讨了半链接（semilink）这一新数学概念，结合半环对，并通过GraphBLAS标准，实现对数字超空间中非结构化数据的有效表示、遍历和转换，旨在替代电子表格和数据库。


<details>
  <summary>Details</summary>
Motivation: 数字超空间（如社交媒体、电子商务等）中存在大量非结构化数据，传统类型和维度概念难以处理，导航这些数据面临挑战。

Method: 论文提出并探索了一种新颖的数学概念——半链接（semilink），它结合了成对的半环，为图分析、数据库操作和机器学习提供基本操作。此外，论文利用超图、超稀疏矩阵和关联数组代数，并基于GraphBLAS标准。

Result: 通过引入基于键的索引和半链接，GraphBLAS可以成为更丰富的关联数组代数，并作为电子表格、数据库表和以数据为中心的操作系统的一种插件式替代方案。

Conclusion: 半链接和GraphBLAS结合，能够增强对数字超空间中非结构化数据的导航能力，并有望取代现有的数据管理工具，提供更高效的数据处理方式。
