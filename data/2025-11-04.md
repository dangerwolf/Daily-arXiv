...
TL;DR: CEDAR技术通过元数据模板帮助科学家标准化数据描述，以实现FAIR数据原则并促进开放科学。


<details>
  <summary>Details</summary>
Motivation: 科学家致力于实现数据的FAIR原则，但难以记住所有指导方针。关键在于数据需要丰富的、特定领域的标准化元数据。

Method: CEDAR中心开发了一项技术，使科学家能够将元数据标准编码为模板，这些模板列举了不同类型实验的属性，并捕获了数据应如何描述以及第三方理解数据集所需的信息偏好。

Result: CEDAR模板已被用于标准化各种科学联盟的元数据，作为通过Web表单或电子表格获取元数据的数据注释系统的基础，并能帮助纠正元数据以确保符合标准。

Conclusion: CEDAR模板以符号形式捕获知识，为科学社区提供了一种机制，用于创建共享元数据标准，编码其应用偏好，并将这些标准部署到一系列智能系统中以促进开放科学。
