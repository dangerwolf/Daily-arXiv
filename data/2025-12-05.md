...
TL;DR: This paper describes a deidentification workflow for DICOM data using XNAT and related tools, developed for the Medical Image De-Identification Benchmark (MIDI-B) challenge.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop and validate a robust deidentification method for DICOM images, addressing the need for privacy in research data.

Method: The method involves a rule-based approach and machine-learning models for removing identifying information from DICOM data within the XNAT platform.

Result: The initial result in the MIDI-B test phase was 97.91%, which improved to 99.61% after addressing technical issues. Further experiments with machine learning models resulted in 99.54% accuracy. The estimated genuine deidentification failure rate is 0.19%.

Conclusion: The paper concludes that a rule-based approach can remove name-related information effectively, but address data requires further improvement. Future work will focus on enhancing address recognition and removing identifiable data burned into image pixels.
...
TL;DR: Socratic Models (SMs) leverage diverse pretrained models via multimodal prompting for zero-shot multimodal capabilities.


<details>
  <summary>Details</summary>
Motivation: Different pretrained models store different commonsense knowledge across domains.

Method: Compose multiple pretrained models zero-shot via multimodal-informed prompting.

Result: SMs are competitive in image captioning and video-to-text retrieval, and enable new applications like egocentric video QA, multimodal dialogue, and robot perception.

Conclusion: Diversity in pretrained models is symbiotic and can be leveraged through SMs for new multimodal capabilities without finetuning.
...
TL;DR: This paper introduces TableParser, a system for parsing tables in PDFs and scanned images, along with TableAnnotator and ExcelAnnotator for weak supervision and pipeline creation.


<details>
  <summary>Details</summary>
Motivation: Parsing table structures and extracting content is important in many applications.

Method: The authors devise TableParser and use domain adaptation. They also create TableAnnotator and ExcelAnnotator for weak supervision.

Result: The experiments show the efficacy of domain adaptation.

Conclusion: The authors share resources to facilitate further research.
...
TL;DR: SQuARE is a hybrid retrieval framework for question answering over spreadsheets that combines chunk retrieval and SQL-based methods based on sheet complexity, improving accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for question answering over spreadsheets struggle with complex structures like multi-row headers and merged cells, and lack flexibility in handling inconsistent schemas.

Method: SQuARE uses a complexity-aware routing mechanism based on header depth and merge density to choose between structure-preserving chunk retrieval and SQL over a relational representation. A lightweight agent supervises and combines results.

Result: SQuARE outperforms single-strategy baselines and ChatGPT-4o in retrieval precision and answer accuracy on various datasets, including corporate balance sheets and World Bank workbooks.

Conclusion: SQuARE's hybrid approach and compatibility with tabular foundation models offer a practical solution for robust table understanding.
...
TL;DR: This paper explores using LLMs to help with writing formulas in spreadsheets, addressing the scarcity of resources by generating synthetic natural language utterances for fine-tuning. They validate these synthetic examples to ensure accuracy.


<details>
  <summary>Details</summary>
Motivation: Resources for spreadsheet formulas are scarce, hindering the performance of LLMs in writing them.

Method: The paper validates synthetic natural language utterances generated by an LLM using surrogate objectives to evaluate the accuracy of the synthetic annotations.

Result: Validation improves performance over raw data across four models and increases the complexity of problems that models can solve.

Conclusion: Validating synthetic training examples enhances the performance and problem-solving capabilities of models fine-tuned on the validated data.
...
TL;DR: TableLLM: An 8B parameter LLM for tabular data manipulation in documents/spreadsheets.


<details>
  <summary>Details</summary>
Motivation: Real-world office scenarios require robust tabular data handling.

Method: Distant supervision with reasoning process extension and cross-way validation.

Result: TableLLM outperforms existing LLMs on document and spreadsheet benchmarks.

Conclusion: TableLLM demonstrates advantages; model, code, benchmarks, and web app are released.
...
TL;DR: GiesKaNe project explores corpus compilation for a syntactically annotated historical corpus, balancing innovation and standards using human expertise and machine assistance.


<details>
  <summary>Details</summary>
Motivation: To create a reference and historical corpus with syntactic annotations that connects historical and contemporary linguistic contexts.

Method: Combining human expertise and machine-assisted processes for tokenization, normalization, tagging, parsing, and classification of texts based on orality and literacy.

Result: Demonstrates that ambitious projects like GiesKaNe can be implemented using existing research infrastructure and a simple spreadsheet-based workflow.

Conclusion: The project effectively manages the complexity of corpus compilation by integrating human and machine collaboration and leveraging existing infrastructure.
...
TL;DR: This paper introduces MiMoTable, a new multi-scale spreadsheet benchmark for table reasoning that uses real-world spreadsheets and meta-operations to evaluate question difficulty.


<details>
  <summary>Details</summary>
Motivation: Existing table reasoning benchmarks don't reflect the complexity and diversity of real-world tables and questions.

Method: The authors created MiMoTable, a benchmark using real-world spreadsheets across seven domains and defined six categories of meta-operations to measure question difficulty.

Result: Claude-3.5-Sonnet achieved the best performance on MiMoTable with 77.4% accuracy, indicating room for improvement. The paper also graded existing benchmarks by difficulty, showing LLM performance decreases as difficulty increases.

Conclusion: The MiMoTable benchmark and the proposed meta-operation criteria effectively highlight the limitations of current LLMs in handling complex, real-world table reasoning tasks.
...
TL;DR: SpreadsheetBench is a new benchmark for spreadsheet manipulation using real-world data from Excel forums.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks use synthesized queries and simplified spreadsheets, which don't reflect real-world user needs.

Method: The benchmark is built from 912 real questions from online Excel forums, with associated spreadsheets containing diverse data. A new evaluation metric with multiple test cases per instruction is proposed.

Result: Evaluation of LLMs shows a significant gap between SOTA models and human performance.

Conclusion: SpreadsheetBench is a challenging benchmark that highlights the difficulty of real-world spreadsheet manipulation for LLMs.
...
TL;DR: This paper introduces NL2Formula, a new benchmark task for generating spreadsheet formulas from natural language queries, along with a dataset and a baseline model called fCoder.


<details>
  <summary>Details</summary>
Motivation: Writing spreadsheet formulas is tedious and error-prone, especially for complex operations.

Method: The authors created a dataset of 70,799 NL query/formula pairs and developed a sequence-to-sequence model, fCoder, as a baseline.

Result: fCoder outperforms baseline models and shows promising results compared to GPT-3.5.

Conclusion: The NL2Formula task presents challenges, and further research is needed.
...
### [11] [InstructExcel: A Benchmark for Natural Language Instruction in Excel](https://arxiv.org/abs/2310.14495)
...
TL;DR: This paper introduces InstructExcel, a new benchmark for evaluating LLMs' ability to generate Excel OfficeScripts from natural language instructions. The benchmark is large-scale, covering a wide range of Excel operations.


<details>
  <summary>Details</summary>
Motivation: To assess whether LLMs can generate code (Excel OfficeScripts) that solves Excel-specific tasks from natural language instructions.

Method: A new large-scale benchmark, InstructExcel, was created by leveraging the 'Automate' feature in Excel to automatically generate OfficeScripts from users' actions.

Result: Experiments show that InstructExcel is a challenging benchmark for state-of-the-art models like GPT-4. Performance can be improved by using GPT-4 over GPT-3.5, providing more in-context examples, and using dynamic prompting.

Conclusion: The study demonstrates the difficulty of generating Excel OfficeScripts from natural language instructions and suggests methods for improving performance on the InstructExcel benchmark.
...
### [12] [Active Learning with Tabular Language Models](https://arxiv.org/abs/2211.04128)
*Martin Ringsquandl,Aneta Koleva*

Main category: cs.CL

TL;DR: This paper explores active learning for tabular language models in a real-world industrial setting, focusing on sub-cell named entity recognition.


<details>
  <summary>Details</summary>
Motivation: The motivation is to reduce labeling costs for tabular data in industry, where acquiring large labeled datasets is expensive due to the need for expert annotation.

Method: The paper investigates different acquisition functions for active learning in conjunction with tabular language models.

Result: Cell-level acquisition functions with built-in diversity can significantly reduce labeling effort, while enforced table diversity is detrimental. The paper also identifies open questions regarding computational efficiency and human annotator perspective.

Conclusion: The study demonstrates the potential of active learning to reduce labeling costs in industrial tabular language model applications, but also highlights challenges related to diversity, efficiency, and human factors.
...
### [13] [Table-To-Text generation and pre-training with TabT5](https://arxiv.org/abs/2210.09162)
*Ewa Andrejczuk,Julian Martin Eisenschlos,Francesco Piccinno,Syrine Krichene,Yasemin Altun*

Main category: cs.CL

TL;DR: TABT5 is an encoder-decoder model for table understanding that generates natural language text, overcoming limitations of encoder-only models.


<details>
  <summary>Details</summary>
Motivation: Encoder-only models are limited to classification tasks. TABT5 aims to generate natural language from tables.

Method: TABT5 uses an encoder-decoder architecture with table-specific embeddings and pre-training.

Result: TABT5 achieves state-of-the-art results on spreadsheet formula prediction, QA, and data-to-text generation.

Conclusion: TABT5, an encoder-decoder model, improves table understanding by generating natural language and achieves state-of-the-art results.
...
### [14] [Table Pre-training: A Survey on Model Architectures, Pre-training Objectives, and Downstream Tasks](https://arxiv.org/abs/2201.09745)
*Haoyu Dong,Zhoujun Cheng,Xinyi He,Mengyu Zhou,Anda Zhou,Fan Zhou,Ao Liu,Shi Han,Dongmei Zhang*

Main category: cs.CL

TL;DR: 本篇论文综述了表格预训练领域的研究进展，包括模型设计、预训练目标和下游任务。


<details>
  <summary>Details</summary>
Motivation: 为了充分利用无标签表格中的监督信号，各种表格预训练框架被提出，并在各种任务上取得了新的技术水平。

Method: 本文对不同的模型设计、预训练目标和下游任务进行了全面的回顾。

Result: 总结了表格预训练在表格问答、表格类型识别、列关系分类、表格搜索、公式预测等任务上的最新进展。
...
### [15] [TableQuery: Querying tabular data with natural language](https://arxiv.org/abs/2202.00454)
*Abhijith Neil Abraham,Fariz Rahman,Damanpreet Kaur*

Main category: cs.CL

TL;DR: TableQuery使用预训练的深度学习模型将自然语言查询转换为结构化查询，从而查询表格数据，无需将整个表格加载到内存中。


<details>
  <summary>Details</summary>
Motivation: 现有的表格数据问答深度学习方法有局限性，例如必须将整个表格作为输入，不适合实际应用。实际数据可能包含数百万行，无法全部放入内存。而且，实时更新的数据库不适合每次更新都序列化。

Method: 使用预训练的深度学习模型将自然语言查询转换为结构化查询，然后针对数据库或电子表格运行。

Result: 无需将整个数据加载到内存中，也无需序列化数据库。

Conclusion: TableQuery无需重新训练，可以使用HuggingFace Model Hub上性能更好的问答模型来替换现有模型。
...
### [16] [The Impact of Text Presentation on Translator Performance](https://arxiv.org/abs/2011.05978)
*Samuel Läubli,Patrick Simianer,Joern Wuebker,Geza Kovacs,Rico Sennrich,Spence Green*

Main category: cs.CL

TL;DR: This paper evaluates the design choices of CAT tools, specifically sentence segmentation and source-target arrangement, on translator performance.


<details>
  <summary>Details</summary>
Motivation: To understand how different CAT tool designs impact translator speed and accuracy.

Method: Controlled experiment measuring speed and accuracy in text processing tasks, comparing sentence-by-sentence vs. unsegmented text and top-and-bottom vs. side-by-side arrangement.

Result: Sentence-by-sentence presentation is faster for text reproduction and error identification. Top-and-bottom arrangement is faster for text reproduction. Unsegmented text is best for revision accuracy and efficiency.

Conclusion: The findings provide insights for optimizing CAT tool design to improve translator performance.
...
TL;DR: This paper introduces a digital corpus of non-metrical radif repertoire in Iranian classical music, covering all 13 components with MIDI files and data spreadsheets.


<details>
  <summary>Details</summary>
Motivation: To provide a platform for computational studies of Iranian classical music, including melodic patterns, improvisational styles, and other tasks in music information retrieval, music theory, and computational (ethno)musicology.

Method: Creating a digital corpus representing the complete non-metrical radif repertoire, covering all 13 existing components. Providing MIDI files and data spreadsheets describing notes, note durations, intervals, and hierarchical structures for 228 pieces of music.

Result: A digital corpus of Iranian classical music is created, with supporting basic statistics, and measures of complexity and similarity over the corpus.

Conclusion: The corpus provides a platform for computational studies of Iranian classical music.
...
TL;DR: 提出了一种新的数据采集框架，通过解耦不确定性来指导采样，包括样本大小和数据模态两个方向。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统需要结合来自多种模态的数据，多模态数据为解耦不确定性带来了新的机遇和挑战。当从不同模态获取信息时，机器学习社区中关于可以通过收集更多数据来减少认知不确定性，而偶然不确定性是不可约的这一假设受到了挑战。

Method: 结合主动学习、主动特征获取和不确定性量化的思想。

Result: 在两个多模态数据集上提供了概念验证实现，以展示数据采集框架。

Conclusion: 偶然不确定性随着模态数量的增加而减少，而认知不确定性通过收集更多的观察结果而减少。
...
TL;DR: TabuLa-8B, a Llama 3-8B LLM fine-tuned for tabular data prediction, achieves state-of-the-art zero-shot and few-shot accuracy on unseen tables.


<details>
  <summary>Details</summary>
Motivation: Existing transfer learning paradigms have not had similar impact in the tabular domain compared to language modeling and computer vision.

Method: Fine-tuned a Llama 3-8B LLM using a large, high-quality training dataset extracted from the TabLib corpus, with novel packing and attention schemes for tabular prediction.

Result: TabuLa-8B achieves over 15 pp higher zero-shot accuracy than random guessing and 5-15 pp higher few-shot accuracy than XGBoost and TabPFN.

Conclusion: TabuLa-8B demonstrates the potential of LLMs for tabular prediction, significantly outperforming existing state-of-the-art models in zero-shot and few-shot settings.
...
TL;DR: This paper introduces TableDiffusion, a differentially-private diffusion model for synthesizing high-quality tabular data, outperforming GANs in quality, privacy, and stability.


<details>
  <summary>Details</summary>
Motivation: Generating synthetic tabular data from sensitive biomedical and financial datasets is challenging due to privacy concerns and the limitations of existing generative models like GANs under differential privacy.

Method: The authors propose TableDiffusion, a novel end-to-end model using attention mechanisms and a diffusion process to learn reversible tabular representations and predict added noise.

Result: TableDiffusion achieves state-of-the-art performance in privatized tabular data synthesis, producing higher-fidelity datasets and avoiding mode collapse compared to GANs.

Conclusion: The diffusion paradigm is more data and privacy-efficient than the adversarial paradigm for synthetic tabular data generation due to better data reuse and smoother training.
...
TL;DR: This paper focuses on predicting daily traffic in Metro Rail Transit Line 3 (MRT3) in Metro Manila, Philippines.


<details>
  <summary>Details</summary>
Motivation: The motivation is that the MRT3's daily ridership fluctuates, commuters lack information for efficient itinerary planning, and the government relies on historical data spreadsheets which are hard to examine.

Method: The study uses time series prediction.

Result: The study presents a time series prediction of daily traffic.

Conclusion: Anticipate future attendance at a particular station on specific days.
...
TL;DR: This paper introduces an adversarially-trained machine learning neural network (SGAN) for file type identification, even when file extensions or headers are obfuscated.


<details>
  <summary>Details</summary>
Motivation: Identifying the true type of a file is critical in forensic investigations, but system owners may conceal file types.

Method: A semi-supervised generative adversarial network (SGAN) is used to classify files.

Result: The SGAN achieved 97.6% accuracy across 11 file types, outperforming traditional neural networks and other machine learning algorithms, especially with limited supervised samples.

Conclusion: The adversarially-trained network is the most precise file classifier, particularly when supervised samples are scarce. Implementation is available on GitHub.
...
TL;DR: This paper introduces TabularNet, a new neural network architecture for understanding tabular data by extracting both spatial and relational information.


<details>
  <summary>Details</summary>
Motivation: Existing methods like CNNs don't capture diverse relationships between cells in tables, such as hierarchical and paratactic relationships.

Method: TabularNet uses a spatial encoder with row/column-level pooling and Bi-GRU to capture statistical and local positional correlations. It also uses a GCN-based encoder with a new graph construction method based on WordNet to focus on hierarchical and paratactic relationships.

Result: Experiments on three classification tasks with two real-world datasets show that TabularNet outperforms state-of-the-art baselines.

Conclusion: TabularNet is an effective unified neural backbone for different understanding tasks in a multitask scenario.
...
TL;DR: This paper introduces Visual Backpropagation, a spreadsheet-based implementation of backpropagation.


<details>
  <summary>Details</summary>
Motivation: To provide a visual and transparent implementation of backpropagation.

Method: Using array worksheet formulas and manual calculation in spreadsheets.

Result: Demonstrates a Visual Backpropagation solution on a standard regression problem and compares it to a Tensorflow solution.

Conclusion: Visual Backpropagation offers a transparent alternative to traditional backpropagation implementations.
...
### [25] [Data-Semantics-Aware Recommendation of Diverse Pivot Tables](https://arxiv.org/abs/2507.06171)
*Whanhee Cho,Anna Fariha*

Main category: cs.DB

TL;DR: SAGE is a system for recommending diverse pivot tables that are insightful and interpretable.


<details>
  <summary>Details</summary>
Motivation: Identifying useful pivot tables in high-dimensional datasets is challenging and manual. Existing methods don't adequately address table diversification.

Method: A data-semantics-aware model measures the utility and diversity of pivot tables. A scalable greedy algorithm efficiently selects a diverse set of high-utility pivot tables.

Result: SAGE outperforms alternative approaches on real-world datasets and scales to high-dimensional datasets.

Conclusion: SAGE effectively recommends diverse and insightful pivot tables, outperforming commercial software and LLMs.
...
### [26] [Facilitating Digital Agriculture with Simple Databases](https://arxiv.org/abs/2312.06517)
*Dennis Buckmaster,Sami Basir,Hanae Sakata*

Main category: cs.DB

TL;DR: This paper introduces open-source Air table database templates for agriculturalists with spreadsheet skills.


<details>
  <summary>Details</summary>
Motivation: To provide well-structured private database templates as open source resources for agriculturalists.

Method: Using simple data-validated forms within Air table databases.

Result: Yielding operational data that is tidy, machine- and human-readable, editable, and exportable.

Conclusion: These resources may facilitate infusion of digital agriculture principles through Extension and structured educational programming.
...
### [27] [Streamlining Knowledge Graph Construction with a façade: The SPARQL Anything project](https://arxiv.org/abs/2310.16700)
*Luigi Asprino,Enrico Daga,Justin Dowdy,Paul Mulholland,Aldo Gangemi,Marco Ratta*

Main category: cs.DB

TL;DR: This paper introduces SPARQL Anything, a data integration framework that allows querying heterogeneous resources as RDF using SPARQL 1.1.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for a data integration framework for knowledge engineers, drawing inspiration from the concept of a façade in object-oriented software engineering.

Method: The paper presents the design and software architecture of SPARQL Anything, highlighting its ability to query various file formats and Web APIs through overloading the SERVICE clause in SPARQL.

Result: The paper includes references to real-world scenarios, a community survey, and a field report from the industry to demonstrate the value and usability of SPARQL Anything.

Conclusion: The paper concludes by discussing the value of the design assumptions of SPARQL Anything compared to alternative solutions, based on user feedback and industry experiences.
...
TL;DR: DataVinci is an unsupervised string data error detection and repair system that learns regular-expression-based patterns and uses an LLM to handle strings with both syntactic and semantic substrings.


<details>
  <summary>Details</summary>
Motivation: Existing systems are limited to error detection or require user input, and they often focus on either syntactic or semantic errors independently.

Method: DataVinci learns regular-expression-based patterns, uses an LLM to abstract semantic substrings, and leverages execution information from existing programs.

Result: DataVinci outperforms 7 baselines on both error detection and repair on 4 benchmarks.

Conclusion: DataVinci is a fully unsupervised system that effectively detects and repairs string data errors by combining regular expressions, LLMs, and execution information.
...
TL;DR: The paper introduces Auto-Tables, a system that automatically transforms non-relational tables into standard relational forms for easier analysis.


<details>
  <summary>Details</summary>
Motivation: Many real-world tables don't conform to the relational standard, requiring complex transformations that are difficult for users to program.

Method: The system synthesizes multi-step transformation pipelines in Python or other languages.

Result: Auto-Tables successfully synthesizes transformations for over 70% of test cases at interactive speeds.

Conclusion: Auto-Tables is an effective tool for preparing data for analytics, benefiting both technical and non-technical users.
...
TL;DR: TACO is a framework for efficiently compressing spreadsheet formula graphs by exploiting tabular locality.


<details>
  <summary>Details</summary>
Motivation: Querying and maintaining large, complex formula graphs in spreadsheets is time-consuming, reducing interactivity.

Method: TACO leverages tabular locality-based patterns to compress formula graphs, directly querying the compressed graph without decompression, and incrementally maintaining the graph during updates.

Result: TACO significantly reduces formula graph sizes and achieves speedups of up to 34,972x over a baseline and 632x over a commercial spreadsheet system for querying formula graphs.

Conclusion: TACO framework can significantly reduce formula graph sizes and improve the efficiency of querying and maintenance by leveraging tabular locality.
...
### [31] [Sigma Workbook: A Spreadsheet for Cloud Data Warehouses](https://arxiv.org/abs/2204.03128)
*James Gale,Max Seiden,Deepanshu Utkarsh,Jason Frantz,Rob Woollen,Çağatay Demiralp*

Main category: cs.DB

TL;DR: Sigma Workbook is a new interactive system that enables business users to easily perform a visual analysis of data in CDWs at scale.


<details>
  <summary>Details</summary>
Motivation: Existing tools for analyzing data in CDWs are either limited in ad-hoc transformations or difficult to use for business users.

Method: Sigma Workbook provides an accessible spreadsheet-like interface for analysis through direct manipulation and dynamically constructs matching SQL queries from user interactions.

Result: Demonstrated Sigma Workbook through 3 real-life use cases -- cohort analysis, sessionization, and data augmentation -- and underline Workbook's ease of use, scalability, and expressivity.

Conclusion: Sigma Workbook enables business users to easily perform a visual analysis of data in CDWs at scale.
...
### [32] [Efficient Specialized Spreadsheet Parsing for Data Science](https://arxiv.org/abs/2202.13189)
*Felix Henze,Haralampos Gavriilidis,Eleni Tzirita Zacharatou,Volker Markl*

Main category: cs.DB

TL;DR: This paper introduces a novel parser for loading spreadsheets into data science environments like R, addressing the high runtime and memory usage of existing methods.


<details>
  <summary>Details</summary>
Motivation: Current spreadsheet loading methods are inefficient in terms of speed and memory, limiting data exploration on commodity systems.

Method: The paper proposes a parser that minimizes memory usage by tightly coupling decompression and parsing. It also includes optimized parsing routines and parallelism to reduce runtime.

Result: The prototype implementation shows up to 3x speed improvement and up to 40x memory reduction compared to existing methods.

Conclusion: The proposed approach makes spreadsheet loading more practical on commodity systems by significantly improving speed and reducing memory usage.
...
### [33] [Spread2RML: Constructing Knowledge Graphs by Predicting RML Mappings on Messy Spreadsheets](https://arxiv.org/abs/2110.12829)
*Markus Schröder,Christian Jilek,Andreas Dengel*

Main category: cs.DB

TL;DR: Spread2RML predicts RML mappings on messy spreadsheets using RML object map templates and heuristics, achieving promising results in the evaluation.


<details>
  <summary>Details</summary>
Motivation: Mapping spreadsheets to RDF knowledge graphs using RML is time-consuming due to the complexity of spreadsheet data models.

Method: Spread2RML uses an extensible set of RML object map templates applied to each column based on heuristics to predict RML mappings.

Result: The evaluation on three datasets, including messy synthetic data and spreadsheets from data.gov, shows promising results, especially considering the approach is fully automatic and handles messy data.

Conclusion: Spread2RML can automatically generate RML mappings for messy spreadsheets with promising results.
...
### [34] [Supercomputing Enabled Deployable Analytics for Disaster Response](https://arxiv.org/abs/2108.11525)
...
TL;DR: This paper explores a method for providing advanced analytics to first responders and essential workers in situations where network access is limited and software security is paramount.


<details>
  <summary>Details</summary>
Motivation: The motivation is to provide advanced analytics to first responders and essential workers who often lack reliable network access and require secure software solutions.

Method: The method involves precomputing a wide range of analytics as files (Google Earth and Microsoft Excel) that can be used with standard preinstalled software, eliminating the need for network access or additional software.

Result: The approach was tested during the COVID-19 pandemic, providing geo-spatial census data for quick analysis of demographic data. The fast mapping of census data using Google Earth and Microsoft Excel has the potential to give emergency responders a powerful tool to improve emergency preparedness.

Conclusion: The study concludes that precomputing analytics as files and using standard software like Google Earth and Microsoft Excel can provide a valuable tool for emergency responders, enabling them to quickly analyze demographic data and improve emergency preparedness, especially in situations with limited network access and stringent security requirements.
...
### [35] [Table2Charts: Recommending Charts by Learning Shared Table Representations](https://arxiv.org/abs/2008.11015)
*Mengyu Zhou,Qingtao Li,Xinyi He,Yuejiang Li,Yibo Liu,Wei Ji,Shi Han,Yining Chen,Daxin Jiang,Dongmei Zhang*

Main category: cs.DB

TL;DR: Table2Charts is a framework for recommending charts from tables using deep Q-learning and heuristic searching.


<details>
  <summary>Details</summary>
Motivation: Recommending charts for multi-dimensional datasets is challenging due to efficiency, imbalanced data, and table context.

Method: Deep Q-learning with copying mechanism and heuristic searching for table-to-sequence generation based on chart templates.

Result: Table2Charts outperforms other chart recommendation systems in both multi-type task and human evaluations.

Conclusion: Table2Charts effectively learns common patterns from (table, charts) pairs for chart recommendation.
...
### [36] [Sigma Worksheet: Interactive Construction of OLAP Queries](https://arxiv.org/abs/2012.00697)
*James Gale,Max Seiden,Gretchen Atwood,Jason Frantz,Rob Woollen,Çağatay Demiralp*

Main category: cs.DB

TL;DR: Sigma Worksheet is a new interactive system that enables users to easily perform ad-hoc visual analysis of data in CDWs at scale.
...
Method: Sigma Worksheet provides an accessible spreadsheet-like interface for data analysis through direct manipulation and dynamically constructs matching SQL queries from user interactions.

Result: The performance of compiled SQL queries is comparable to that of the reference queries of the benchmark. Sigma Worksheet is easier to use and learn, improving the productivity of users.
...
### [37] [Mapping Spreadsheets to RDF: Supporting Excel in RML](https://arxiv.org/abs/2104.13600)
*Markus Schröder,Christian Jilek,Andreas Dengel*

Main category: cs.DB

TL;DR: 该论文扩展了RML Mapper工具，使其能够支持Microsoft Excel电子表格文件到RDF图的映射。


<details>
  <summary>Details</summary>
Motivation: 当前RML规范和实现忽略了广泛使用的电子表格格式。

Method: 扩展RML Mapper工具以支持Microsoft Excel电子表格文件，并提供在线演示。

Result: 该工具能够访问电子表格单元格的各种元数据，并提供了一些实验性功能。

Conclusion: 该论文通过扩展RML Mapper，填补了RML对电子表格格式支持的空白，并公开了实现代码。
...
### [38] [Dataset Generation Patterns for Evaluating Knowledge Graph Construction](https://arxiv.org/abs/2104.13576)
*Markus Schröder,Christian Jilek,Andreas Dengel*

Main category: cs.DB

TL;DR: This paper introduces a method for generating synthetic datasets that mimic real-world spreadsheets, addressing the lack of authentic labeled data due to confidentiality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create realistic synthetic data for evaluating knowledge graph construction, as real-world datasets are often unavailable due to confidentiality.

Method: The method involves identifying and reproducing 11 distinct patterns found in real spreadsheets using a generator called Data Sprout.

Result: The Data Sprout generator is able to reproduce the identified patterns.

Conclusion: The paper demonstrates a suitable generator (Data Sprout) capable of reproducing patterns found in real spreadsheets, offering a solution to the data scarcity problem.
...
### [39] [Interactively Constructing Knowledge Graphs from Messy User-Generated Spreadsheets](https://arxiv.org/abs/2103.03537)
*Markus Schröder,Christian Jilek,Michael Schulze,Andreas Dengel*

Main category: cs.DB

TL;DR: This paper tackles the challenge of converting unstructured spreadsheets into knowledge graphs by proposing an interactive approach with a GUI for annotating cells and forming a knowledge graph.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the difficulty of interpreting unstructured spreadsheet data and converting it into a structured knowledge graph, especially when data maintenance is lacking.

Method: The method involves an interactive approach with a graphical user interface (GUI) that allows knowledge engineers to bulk-annotate spreadsheet cells with extracted information. These annotations are then used to construct a knowledge graph.

Result: The approach was evaluated using five spreadsheets from an industrial scenario, resulting in a 25k-triple knowledge graph. The method was compared to RML, highlighting the contributions of the approach.

Conclusion: The paper proposes an interactive approach to convert unstructured spreadsheets into knowledge graphs, offering a solution to the challenges posed by messy, user-generated data. The evaluation demonstrates the effectiveness of the approach compared to existing methods.
...
### [40] [Leam: An Interactive System for In-situ Visual Text Analysis](https://arxiv.org/abs/2009.03520)
*Sajjadur Rahman,Peter Griggs,Çağatay Demiralp*

Main category: cs.DB

TL;DR: This paper introduces Leam, a new system for text analysis that addresses challenges related to data heterogeneity, provenance, workflow reusability, and compatibility with established practices.


<details>
  <summary>Details</summary>
Motivation: Enterprises use text analytics to improve their services and products, but existing systems fail to address challenges related to data heterogeneity, provenance, workflow reusability and reproducibility, and compatibility with established practices.

Method: The paper proposes Leam, a system that combines computational notebooks, spreadsheets, and visualization tools, featuring an interactive user interface, a new data model, and an expressive algebra.

Result: The paper reports current progress in Leam development and demonstrates its usefulness with usage examples.

Conclusion: The paper outlines enhancements to Leam and identifies research directions for developing an interactive visual text analysis system.
...
### [41] [ObjTables: structured spreadsheets that promote data quality, reuse, and integration](https://arxiv.org/abs/2005.05227)
*Jonathan R. Karr,Wolfram Liebermeister,Arthur P. Goldberg,John A. P. Sekar,Bilal Shaikh*

Main category: cs.DB

TL;DR: ObjTables is a toolkit that makes spreadsheets human- and machine-readable by combining spreadsheets with schemas and an object-relational mapping system.


<details>
  <summary>Details</summary>
Motivation: Spreadsheets are often difficult to reanalyze because they capture data ad hoc without schemas that define the objects, relationships, and attributes that they represent.

Method: ObjTables combines spreadsheets with schemas and an object-relational mapping system. It includes a format for schemas; markup for indicating the class and attribute represented by each spreadsheet and column; numerous data types for scientific information; and high-level software for using schemas to read, write, validate, compare, merge, revision, and analyze spreadsheets.

Result: ObjTables makes spreadsheets easier to reuse, enable unprecedented secondary meta-analyses and accelerate emerging scientific fields.

Conclusion: ObjTables could enable unprecedented secondary meta-analyses. By making it easy to build new formats and associated software for new types of data, ObjTables can also accelerate emerging scientific fields.
...
### [42] [Needles in the 'Sheet'stack: Augmented Analytics to get Insights from Spreadsheets](https://arxiv.org/abs/2006.08224)
...
TL;DR: This paper demos a BI tool that provides insights from periodic spreadsheets without requiring database schema knowledge.


<details>
  <summary>Details</summary>
Motivation: To provide a BI solution for users unfamiliar with database schemas who need to analyze periodic reports.

Method: Examining periodic spreadsheets of different reports (aka views) without prior knowledge of the database schema or reports, or data information.

Result: Ready insights or visual query explorations

Conclusion: Aimed at users without database schema familiarity, offering an alternative to contemporary analytics methods.
...
### [43] [A Billion Updates per Second Using 30,000 Hierarchical In-Memory D4M Databases](https://arxiv.org/abs/1902.00846)
...
TL;DR: This paper introduces a hierarchical implementation of D4M associative arrays for high-performance streaming updates of large-scale networks, achieving 1.9 billion updates per second on the MIT SuperCloud.


<details>
  <summary>Details</summary>
Motivation: Analyzing large scale networks requires high performance streaming updates of graph representations of these data.

Method: A hierarchical implementation of the Dynamic Distributed Dimensional Data Model (D4M) library.

Result: Achieved a sustained update rate of 1,900,000,000 updates per second running 34,000 instances on 1,100 server nodes.

Conclusion: This capability allows the MIT SuperCloud to analyze extremely large streaming network data sets.
...
### [44] [Towards Semantically Enhanced Data Understanding](https://arxiv.org/abs/1806.04952)
*Markus Schröder,Christian Jilek,Jörn Hees,Andreas Dengel*

Main category: cs.DB

TL;DR: This paper introduces a methodology that uses a single semantic model that interlinks data with its documentation, allowing data scientists to easily access connected information and enabling additional support from other applications.


<details>
  <summary>Details</summary>
Motivation: Data understanding in machine learning requires extensive documentation which is often separate from the data, causing lookup overhead and limiting its use by other applications.

Method: The paper proposes a methodology that uses a single semantic model to interlink data with its documentation.

Result: The paper showcases an early prototype to demonstrate the approach.

Conclusion: The proposed semantic model allows data scientists to directly look up connected information about the data and enables other approaches to provide additional support, like searching, comparing, integrating or visualizing data.
...
### [45] [Using the Abstract Computer Architecture Description Language to Model AI Hardware Accelerators](https://arxiv.org/abs/2402.00069)
*Mika Markus Müller,Alexander Richard Manfred Borst,Konstantin Lübeck,Alexander Louis-Ferdinand Jung,Oliver Bringmann*

Main category: cs.AR

TL;DR: This paper introduces using Abstract Computer Architecture Description Language (ACADL) to model AI hardware accelerators for performance analysis.


<details>
  <summary>Details</summary>
Motivation: Selecting the right AI accelerator and its parameters is challenging due to complex comparisons and reliance on inefficient methods.

Method: The paper uses ACADL to model AI hardware accelerators and map DNNs onto them.

Result: Explains the timing simulation semantics to gather performance results.

Conclusion: Demonstrates the use of ACADL for modeling AI hardware accelerators and gathering performance results through timing simulation.
...
### [46] [Online Model Swapping in Architectural Simulation](https://arxiv.org/abs/2012.01571)
*Patrick Lavin,Jeffrey Young,Rich Vuduc,Jonathan Beard*

Main category: cs.AR

TL;DR: This paper introduces a method for bridging the gap between simple and detailed simulations by monitoring simulation behavior online and automatically swapping out detailed models with simpler statistical approximations.


<details>
  <summary>Details</summary>
Motivation: Detailed simulation takes an ever increasing amount of time, forcing architects to use simpler models, such as spreadsheets, when they want to iterate quickly on a design. The task of migrating from a simple simulation to one with more detail often requires multiple executions to find where simple models could be effective, which could be more expensive than running the detailed model in the first place. Also, architects must often rely on intuition to choose these simpler models, further complicating the problem.

Method: The paper presents a method of bridging the gap between simple and detailed simulation by monitoring simulation behavior online and automatically swapping out detailed models with simpler statistical approximations. The method is implemented in the open-source simulator SVE-Cachesim to swap out the level one data cache (L1D) within a memory hierarchy.

Result: The simulation swaps out the built-in cache model with only an 8% error in the simulated cycle count while using the approximated cache models for over 90% of the simulation, and the simpler models require two to eight times less computation per execution of the model

Conclusion: The paper demonstrates the potential of the methodology by implementing it in the open-source simulator SVE-Cachesim to swap out the level one data cache (L1D) within a memory hierarchy. This proof of concept demonstrates that the technique can handle a non-trivial use-case in not just approximation of local time-invariant statistics, but also those that vary with time, and downstream side-effects.
...
### [47] [Real-time stock analysis for blending recipes in industrial plants](https://arxiv.org/abs/1909.02960)
*Florin Zamfir,Nicolae Paraschiv,Emil Pricop*

Main category: cs.OH

TL;DR: This paper introduces a C# application for real-time stock analysis in blending processes, addressing the limitations of using Excel spreadsheets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the difficulties in understanding and tracking stock records in Excel spreadsheets, and to prevent accidental changes to formulas.

Method: The method involves developing a C# application with integrated planning algorithms to manage stock data and assist operators in decision-making.

Result: The application provides a centralized platform for stock data, assists operators in deciding, and uses algorithms to identify needed ingredients, determine quantities of finished products, and optimize product quantities.

Conclusion: The implemented C# application effectively manages stock data, assists operators, and optimizes blending processes through integrated algorithms.
...
### [48] [Pivoting the paradigm: the role of spreadsheets in K-12 data science](https://arxiv.org/abs/2506.03232)
*Oren Tirschwell,Nicholas Jon Horton*

Main category: stat.OT

TL;DR: This paper explores the use of spreadsheets in K-12 education for data analysis and computational fluency.


<details>
  <summary>Details</summary>
Motivation: Spreadsheets are widely used in K-12 but their potential for data-driven learning isn't fully realized.

Method: Review of frameworks, proposal of learning outcomes, and discussion of spreadsheet use.

Result: Presents example activities and identifies challenges to adoption.

Conclusion: Spreadsheets can foster data skills but require pedagogical support and professional development.
...
### [49] [Buckaroo: A Direct Manipulation Visual Data Wrangler](https://arxiv.org/abs/2507.16073)
*Annabelle Warner,Andrew McNutt,Paul Rosen,El Kindi Rezig*

Main category: cs.HC

TL;DR: This paper introduces Buckaroo, a visualization system designed to streamline data wrangling by highlighting discrepancies, suggesting wrangling actions, and enabling visual data manipulation.


<details>
  <summary>Details</summary>
Motivation: Traditional data wrangling methods are time-consuming, error-prone, and can negatively impact data quality.

Method: Buckaroo automatically identifies anomalous data groups, suggests wrangling actions, and allows users to visually manipulate data with undo/redo capabilities.

Result: The paper presents Buckaroo as a solution to improve the efficiency and accuracy of data wrangling.

Conclusion: Buckaroo offers a visual and interactive approach to data wrangling, addressing the limitations of traditional methods.
...
### [50] [SheetMind: An End-to-End LLM-Powered Multi-Agent Framework for Spreadsheet Automation](https://arxiv.org/abs/2506.12339)
*Ruiyan Zhu,Xi Cheng,Ke Liu,Brian Zhu,Daniel Jin,Neeraj Parihar,Zhoutian Xu,Oliver Gao*

Main category: cs.HC

TL;DR: SheetMind is a multi-agent system using LLMs to automate spreadsheet tasks from natural language, achieving 80% success on single-step and 70% on multi-step tasks.


<details>
  <summary>Details</summary>
Motivation: To enable spreadsheet automation using natural language instructions without scripting or formula knowledge.

Method: A multi-agent framework with Manager, Action, and Reflection Agents, integrated into Google Sheets via a Workspace extension. The Action Agent uses a BNF grammar to translate instructions.

Result: Achieved 80% success rate on single-step tasks and approximately 70% on multi-step instructions on benchmark datasets, outperforming baselines.

Conclusion: Multi-agent decomposition and grammar-based execution are effective for bridging natural language and spreadsheet functionalities.
...
### [51] ["How do you even know that stuff?": Barriers to expertise sharing among spreadsheet users](https://arxiv.org/abs/2506.09216)
*Qing,Xia,Advait Sarkar,Duncan Brumby,Anna Cox*

Main category: cs.HC

TL;DR: This paper explores the challenges of expertise sharing in spreadsheet collaboration among colleagues.


<details>
  <summary>Details</summary>
Motivation: The motivation is that spreadsheet experts often fail to disseminate their knowledge, which is essential for retaining technical skillsets.

Method: The method used was 31 semi-structured interviews with professional spreadsheet users.

Result: The results show that spreadsheet providers face challenges in adapting personalized strategies, evaluating social timing of sharing, conflicted self-evaluations, dismissive normative beliefs, and concerns about potential disruptions.

Conclusion: The conclusion is that the complex interaction between technology design and social dynamics shapes collaborative learning in feature-rich software, and the paper provides design implications to navigate this tension.
...
### [52] [Prompting in the Dark: Assessing Human Performance in Prompt Engineering for Data Labeling When Gold Labels Are Absent](https://arxiv.org/abs/2502.11267)
*Zeyu He,Saniya Naphade,Ting-Hao 'Kenneth' Huang*

Main category: cs.HC

TL;DR: This paper studies how well people perform prompt engineering, especially in scenarios without gold-standard labels. It finds that iterative prompting without benchmarks is unreliable, and automated tools also struggle with limited gold labels.


<details>
  <summary>Details</summary>
Motivation: To understand how effective users are at prompt engineering when no gold-standard labels are available to measure progress.

Method: A user study with 20 participants using PromptingSheet, a Google Sheets add-on for iterative data labeling, and evaluation of automated prompt optimization tools like DSPy.

Result: Only 9 out of 20 participants improved labeling accuracy after four or more iterations. Automated prompt optimization tools also struggled with few gold labels.

Conclusion: Prompting in the dark is highly unreliable, highlighting the importance of gold labels and the needs and risks of automated support in human prompt engineering.
...
### [53] [When Copilot Becomes Autopilot: Generative AI's Critical Risk to Knowledge Work and a Critical Solution](https://arxiv.org/abs/2412.15030)
*Advait Sarkar,Xiaotong,Xu,Neil Toronto,Ian Drosos,Christian Poelitz*

Main category: cs.HC

TL;DR: This paper discusses the risks and opportunities of using generative AI in knowledge work, specifically spreadsheets. It argues that the greatest risk is the degradation of human critical thinking and proposes designing AI systems to foster critical thinking.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the potential risks of generative AI in knowledge work, particularly the degradation of human critical thinking when users delegate tasks to AI.

Method: The paper presents a prototype system for critical shortlisting in spreadsheets that uses generative AI to suggest criteria and generate provocations to critique those criteria. It also discusses the design of interfaces of generative AI systems deliberately to foster and encourage critical thinking in knowledge work, building primarily on a long history of research on critical thinking tools for education.

Result: The prototype system opens up a rich design space for critical thinking tools in AI-assisted knowledge work.

Conclusion: The paper concludes by outlining a research agenda for AI as a critic or provocateur, including questions about the timing, form, content, and design trade-offs of provocations.
...
### [54] [Understanding and Evaluating Trust in Generative AI and Large Language Models for Spreadsheets](https://arxiv.org/abs/2412.14062)
*Simon Thorne*

Main category: cs.HC

TL;DR: This paper proposes a framework for evaluating the trustworthiness of spreadsheet formulas generated by AI, focusing on transparency and dependability.


<details>
  <summary>Details</summary>
Motivation: The motivation is that AI-generated spreadsheet formulas may be inaccurate or untrustworthy due to hallucinations and biases.

Method: The method involves evaluating the transparency (explainability and visibility) and dependability (reliability and ethical considerations) of the generated formulas.

Result: The paper examines the drivers to these metrics in the form of hallucinations, training data bias and poorly constructed prompts. Finally, examples of mistrust in technology are considered and the consequences explored.

Conclusion: The paper concludes by considering examples of mistrust in technology and exploring the consequences.
...
### [55] [Dynamic Prompt Middleware: Contextual Prompt Refinement Controls for Comprehension Tasks](https://arxiv.org/abs/2412.02357)
*Ian Drosos,Jack Williams,Advait Sarkar,Nicholas Wilson*

Main category: cs.HC

TL;DR: This paper explores how to improve user control over AI-generated explanations in comprehension tasks by using dynamic prompt middleware.


<details>
  <summary>Details</summary>
Motivation: Many users struggle to effectively prompt generative AI, especially when providing context for comprehension tasks. Prompt middleware can help, but users need more control to get AI responses that match their preferences.

Method: The authors conducted a formative survey to understand user needs, and then implemented and evaluated two prompt middleware approaches: Dynamic Prompt Refinement Control (Dynamic PRC) and Static Prompt Refinement Control (Static PRC).

Result: The Dynamic PRC approach, which generates context-specific UI elements for prompt refinement, was preferred by users as it afforded more control, lowered barriers to providing context, and encouraged exploration and reflection. However, reasoning about the effects of different generated controls remains challenging.

Conclusion: Dynamic prompt middleware can improve the user experience of generative AI workflows by affording greater control and guiding users to better AI responses. Future systems should focus on enhancing user control of AI responses.
...
### [56] [Exploring Higher Education Competencies through Spreadsheet Self-Assessment and Time](https://arxiv.org/abs/2409.12974)
*Maria Csernoch,Judit T. Kiss,Viktor Takács,Domicián Máté*

Main category: cs.HC

TL;DR: This paper investigates spreadsheet skills of higher education students.


<details>
  <summary>Details</summary>
Motivation: To explore the spreadsheet competencies and reliability of students through self-assessment and problem-solving.

Method: Comparing student performance in Excel and paper-based tasks.

Result: Students inaccurately assess their skills and take longer in Excel to achieve similar scores.

Conclusion: Digital natives' skills are not as strong as assumed, highlighting the need for accurate self-assessment and time management skills in technology-driven disciplines.
...
### [57] [Supporting Annotators with Affordances for Efficiently Labeling Conversational Data](https://arxiv.org/abs/2403.07762)
*Austin Z. Henley,David Piorkowski*

Main category: cs.HC

TL;DR: This paper introduces CAL, a novel interface to improve data labeling efficiency and user experience.


<details>
  <summary>Details</summary>
Motivation: Crowdsourced labeling is time-consuming and expensive, hindering the development of machine learning systems.

Method: The authors designed CAL with features to prevent incorrect labels, guide users, incorporate documentation, and provide efficient label viewing.

Result: A user study showed that CAL users reported lower cognitive load, maintained task time, found CAL easier to use, and preferred it over spreadsheets.

Conclusion: CAL is a valuable tool for data labeling that improves efficiency and user experience compared to standard spreadsheets.
...
### [58] [Prompting for Discovery: Flexible Sense-Making for AI Art-Making with Dreamsheets](https://arxiv.org/abs/2310.09985)
*Shm Garanganao Almeda,J. D. Zamfirescu-Pereira,Kyu Won Kim,Pradeep Mani Rathnam,Bjoern Hartmann*

Main category: cs.HC

TL;DR: This paper introduces DreamSheets, a spreadsheet interface that helps users explore the design space of Text-to-Image (TTI) models.


<details>
  <summary>Details</summary>
Motivation: Exploring the vast and opaque design space of TTI models is challenging due to the sensitivity of image outputs to prompt inputs.

Method: The authors developed DreamSheets, an interface with LLM-based prompt construction assistance and simultaneous result display, and conducted two user studies.

Result: The studies identified strategies users employ for TTI design space exploration and the interface features needed to support them.

Conclusion: The paper distills insights into a UI mockup to guide future TTI interfaces.
...
### [59] [Does Using ChatGPT Result in Human Cognitive Augmentation?](https://arxiv.org/abs/2401.11042)
*Ron Fulbright,Miranda Morrison*

Main category: cs.HC

TL;DR: This paper investigates the cognitive augmentation effects of using ChatGPT, finding that it doesn't always enhance human performance and can sometimes be misleading.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore how ChatGPT, a new cognitive system, impacts human cognitive abilities, especially in comparison to traditional tools and expert performance.

Method: The method involves conducting two experiments comparing responses generated with and without ChatGPT to assess cognitive augmentation.

Result: The results indicate that using ChatGPT does not consistently lead to cognitive augmentation and may even result in negative augmentation due to misleading information.

Conclusion: The conclusion is that ChatGPT, while powerful, does not yet replace human judgment and evaluation in certain tasks and can sometimes be detrimental to cognitive performance.
...
### [60] [Co-audit: tools to help humans double-check AI-generated content](https://arxiv.org/abs/2310.01297)
...
TL;DR: This paper discusses the increasing need for co-audit tools to help users check the correctness and quality of AI-generated content, especially as the output becomes more complex. It focuses on spreadsheet computations as an example and proposes principles and research challenges for co-audit tool development.


<details>
  <summary>Details</summary>
Motivation: The increasing complexity of AI-generated content makes it difficult for users to evaluate its quality and correctness, necessitating tool-assisted experiences for double-checking.

Method: The paper describes recent research on co-audit tools for spreadsheet computations powered by generative models. It proposes a preliminary list of principles for co-audit and outlines research challenges.

Result: The paper does not present specific empirical results but rather proposes principles and outlines research challenges for co-audit tool development.

Conclusion: Co-audit experiences are essential for any application of generative AI where quality is important and errors are consequential, as is common in spreadsheet computations.
...
### [61] ["What It Wants Me To Say": Bridging the Abstraction Gap Between End-User Programmers and Code-Generating Large Language Models](https://arxiv.org/abs/2304.06597)
*Michael Xieyang Liu,Advait Sarkar,Carina Negreanu,Ben Zorn,Jack Williams,Neil Toronto,Andrew D. Gordon*

Main category: cs.HC

TL;DR: This paper explores how to help non-expert programmers use code-generating language models for data analysis in spreadsheets by bridging the gap between natural language and code.


<details>
  <summary>Details</summary>
Motivation: Non-expert programmers struggle to effectively guide code generation due to the challenge of abstraction matching, where only a small portion of natural language is effective.

Method: The authors propose grounded abstraction matching, which translates the generated code back into a systematic naturalistic utterance to improve understanding.

Result: A user study (n=24) showed that the grounded approach improves end-users' understanding of the code-generating model's scope, capabilities, and required language.

Conclusion: Grounded abstraction matching helps bridge the abstraction gap and improves the usability of code-generating models for non-expert programmers in data analysis tasks.
...
### [62] [Team OS's System for Dialogue Robot Competition 2022](https://arxiv.org/abs/2210.09928)
*Yuki Kubo,Ryo Yanagimoto,Hayato Futase,Mikio Nakano,Zhaojie Luo,Kazunori Komatani*

Main category: cs.HC

TL;DR: OSbot is a dialogue robot system based on state transitions, keyword extraction, and sentiment analysis, which achieved third place in the Dialogue Robot Competition 2022 preliminary round.


<details>
  <summary>Details</summary>
Motivation: To develop a dialogue robot system for the Dialogue Robot Competition 2022.

Method: The dialogue flow is based on manually described state transitions using keyword extraction and sentiment analysis. Keyword extraction uses named entity extraction and a predefined keyword set. Sentiment analysis is text-based and uses SVM trained with the Hazumi corpus.

Result: The system achieved third place in the competition's preliminary round.

Conclusion: The system's dialogue flow can be easily viewed and edited, and the logging function facilitates quick checking and editing.
...
### [63] [What is it like to program with artificial intelligence?](https://arxiv.org/abs/2208.06213)
*Advait Sarkar,Andrew D. Gordon,Carina Negreanu,Christian Poelitz,Sruti Srinivasa Ragavan,Ben Zorn*

Main category: cs.HC

TL;DR: LLM-assisted programming is a new way of programming with its own distinct properties and challenges.


<details>
  <summary>Details</summary>
Motivation: Explore how programming with large language models (LLM-assisted programming) is similar to, and differs from, prior conceptualisations of programmer assistance.

Method: Draw upon publicly available experience reports of LLM-assisted programming, as well as prior usability and design studies and observations from a user study.

Result: LLM-assisted programming shares some properties of compilation, pair programming, and programming via search and reuse, there are fundamental differences both in the technical possibilities as well as the practical experience.

Conclusion: LLM-assisted programming ought to be viewed as a new way of programming with its own distinct properties and challenges and discuss the issues that might arise, and open research challenges, in applying large language models to end-user programming, particularly with users who have little or no programming expertise.
...
### [64] [MetaGlyph: Automatic Generation of Metaphoric Glyph-based Visualization](https://arxiv.org/abs/2209.05739)
*Lu Ying,Xinhuan Shu,Dazhen Deng,Yuchen Yang,Tan Tang,Lingyun Yu,Yingcai Wu*

Main category: cs.HC

TL;DR: MetaGlyph: An automatic system for generating metaphoric glyph-based visualizations (MGVs) from spreadsheets.


<details>
  <summary>Details</summary>
Motivation: Creating metaphoric glyph-based visualization (MGV) is difficult and requires deep understanding of data and professional design skills.

Method: A novel framework for generating MGVs by metaphoric image selection and an MGV construction. It uses Monte Carlo tree search algorithm that explores the design of an MGV by associating visual elements with data dimensions given the data importance, semantic relevance, and glyph non-overlap.

Result: The system provides editing feedback that allows users to customize the MGVs according to their design preferences. The effectiveness of MetaGlyph is validated through a series of expert interviews.

Conclusion: MetaGlyph is effective in generating MGVs.
...
### [65] [PoVRPoint: Authoring Presentations in Mobile Virtual Reality](https://arxiv.org/abs/2201.06337)
...
TL;DR: This paper explores using VR to help mobile knowledge workers create presentations by combining pen and touch input on mobile devices with VR interaction.


<details>
  <summary>Details</summary>
Motivation: To improve presentation authoring in mobile settings using VR's extended display space and spatial input capabilities.

Method: The authors developed PoVRPoint, a set of tools combining pen- and touch-based editing on mobile devices with VR interaction, and conducted a user study to evaluate its utility.

Result: The study showed that VR significantly speeds up target slide identification and object reordering compared to tablet-only interfaces, and users found the interaction techniques usable and enjoyable.

Conclusion: VR has the potential to improve presentation authoring for mobile knowledge workers.
...
### [66] [Untidy Data: The Unreasonable Effectiveness of Tables](https://arxiv.org/abs/2106.15005)
*Lyn Bartram,Michael Correll,Melanie Tory*

Main category: cs.HC

TL;DR: This paper explores how data workers use spreadsheets for sensemaking, highlighting the importance of direct data interaction and flexible human-data interaction.


<details>
  <summary>Details</summary>
Motivation: To understand how data workers interact with and reason about data tables, particularly spreadsheets, in their information ecosystem.

Method: A qualitative study was conducted to observe and analyze how data workers interact with their data in table form.

Result: Data tables serve a broader purpose than just data cleanup; users reshape, augment, and reorganize data to support sensemaking throughout the analytics process. Direct interaction with human-readable tables is cognitively important for understanding the data.

Conclusion: Interactive tables are an important visualization idiom, and more flexible human-data interaction can enrich sensemaking in visual analytics.
...
### [67] [Defining and Adopting an End User Computing Policy: A Case Study](https://arxiv.org/abs/1909.00855)
*Roger Turner*

Main category: cs.HC

TL;DR: This paper introduces an updated End User Computing policy at the Wesleyan Assurance Society.


<details>
  <summary>Details</summary>
Motivation: To address the significant risks associated with uncontrolled End User Computing.

Method: A risk-based approach is used, supported by an End User Computing Risk Assessment Application that calculates a risk rating based on Complexity, Materiality, and Control.

Result: The policy enables prioritizing and mitigating the highest risks first.

Conclusion: The risk-based policy helps to obtain the quickest benefit by addressing the most pressing risks in End User Computing.
...
### [68] [Calliope: Automatic Visual Data Story Generation from a Spreadsheet](https://arxiv.org/abs/2010.09975)
*Danqing Shi,Xinyue Xu,Fuling Sun,Yang Shi,Nan Cao*

Main category: cs.HC

TL;DR: Calliope is a system that automatically generates visual data stories from spreadsheets, using a Monte Carlo tree search algorithm to find and organize data facts, and then visualizes them with charts and descriptions.


<details>
  <summary>Details</summary>
Motivation: Generating visual data stories is difficult due to technique barriers in data analysis, visualization, and scripting. Existing tools are inefficient and difficult to use.

Method: A logic-oriented Monte Carlo tree search algorithm explores the data space to generate story pieces and organize them logically. Data fact importance is measured using information theory, and each fact is visualized and captioned automatically.

Result: Calliope was evaluated through example stories, controlled experiments, and expert interviews, demonstrating its benefits for efficient visual data story generation.

Conclusion: Calliope facilitates efficient visual data story generation by automating the process and providing an online story editor for easy revision.
...
### [69] [Pen-based Interaction with Spreadsheets in Mobile Virtual Reality](https://arxiv.org/abs/2008.04543)
*Travis Gesslein,Verena Biener,Philipp Gagel,Daniel Schneider,Per Ola Kristensson,Eyal Ofek,Michel Pahud,Jens Grubert*

Main category: cs.HC

TL;DR: This paper explores the use of VR to improve spreadsheet interaction on mobile devices, addressing the challenge of limited interaction space by using pen-based input and spatial sensing.


<details>
  <summary>Details</summary>
Motivation: Spreadsheets are widely used but challenging to interact with, especially on mobile devices. VR offers a large display space, but mobile scenarios have limited interaction space.

Method: The authors propose a tool-set for enhancing spreadsheet interaction on tablets using immersive VR headsets and pen-based input. This includes extending sheet display beyond the physical screen and using off-screen menus.

Result: The feasibility of the tool-set is studied using a video-based online survey and an expert-based assessment of human performance potential.

Conclusion: The combination of VR, pen-based input, and spatial sensing opens up possibilities for enhancing spreadsheet interaction productivity.
...
### [70] [EQUS -- helping to see formulae](https://arxiv.org/abs/2007.00003)
*Chris Roast*

Main category: cs.HC

TL;DR: 本文介绍了一个电子表格公式交互式可视化工具（EQUS）的设计、开发和评估。


<details>
  <summary>Details</summary>
Motivation: 电子表格被广泛用于重要的数值处理和建模，但公式容易被误解。

Method: 通过迭代改进，最初针对青少年学习者，涉及重新设计和形成性评估。

Result: 可视化技术被发现与最初的目标受众之外的电子表格用户广泛相关。EQUS 已经被开发为 MS Excel 的完全集成插件。

Conclusion: EQUS 有效地简化了电子表格公式的理解。
...
### [71] [Taggle: Combining Overview and Details in Tabular Data Visualizations](https://arxiv.org/abs/1712.05944)
*Katarina Furmanova,Samuel Gratzl,Holger Stitz,Thomas Zichner,Miroslava Jaresova,Alexander Lex,Marc Streit*

Main category: cs.HC

TL;DR: Taggle is a tabular visualization technique for exploring and presenting large and complex tables.


<details>
  <summary>Details</summary>
Motivation: Many practical analysis tasks are concerned with investigating individual items of interest, relating an item to the rest of a potentially large table is important.

Method: Visualizing each row in the source data individually using visual encodings for the cells, data-driven aggregation of data subsets, interaction methods tailored to answer specific analysis questions.

Result: Demonstrate Taggle using a case study conducted by a domain expert on complex genomics data analysis for the purpose of drug discovery.

Conclusion: Taggle is a tabular visualization technique for exploring and presenting large and complex tables.
...
### [72] [Are digital natives spreadsheet natives?](https://arxiv.org/abs/1909.00865)
*Maria Csernoch,Piroska Biró*

Main category: cs.HC

TL;DR: This paper assesses informatics students' algorithmic skills and knowledge transfer in spreadsheets, finding deficiencies despite their supposed digital native status.


<details>
  <summary>Details</summary>
Motivation: To evaluate the spreadsheet skills of first-year informatics students after their official training.

Method: Testing students with spreadsheet problems and analyzing their solutions based on their use of array formulas vs. built-in functions.

Result: Students struggled significantly, with those using algorithm-based array formulas performing better.

Conclusion: Students need algorithm-based training with expert teachers, regardless of their digital native status.
...
### [73] [Somewhere Around That Number: An Interview Study of How Spreadsheet Users Manage Uncertainty](https://arxiv.org/abs/1905.13072)
*Judith Borghouts,Andrew D. Gordon,Advait Sarkar,Kenton P. O'Hara,Neil Toronto*

Main category: cs.HC

TL;DR: This paper investigates how spreadsheet users deal with uncertainty in their data and identifies the limitations of current spreadsheet tools in supporting their needs.


<details>
  <summary>Details</summary>
Motivation: Spreadsheet users frequently encounter uncertainty, and understanding how they handle it is crucial for developing better tools, as people tend to use incorrect heuristics or ignore uncertainty.

Method: The study involved interviews with 11 spreadsheet users from various fields to understand their experiences with uncertainty.

Result: The study found that users' approaches to uncertainty are influenced by the spreadsheet's role and their aims, which include computing scenarios, understanding uncertainty, and simplifying presentations for decision-makers. Current tools are inadequate for these aims, leading to workarounds.

Conclusion: Spreadsheets lack sufficient tools to support users in dealing with uncertainty effectively, resulting in the use of workarounds.
...
### [74] [Characterizing Scalability Issues in Spreadsheet Software using Online Forums](https://arxiv.org/abs/1801.03829)
*Kelly Mack,John Lee,Kevin Chang,Karrie Karahalios,Aditya Parameswaran*

Main category: cs.HC

TL;DR: 本研究利用在线论坛数据分析用户在使用 Excel 电子表格时遇到的挑战，重点关注大量数据导致的问题。


<details>
  <summary>Details</summary>
Motivation: 传统可用性研究成本高昂，因此研究人员开始转向利用在线论坛的现有数据，以实现更大规模、更低成本和更广覆盖面的工具调查。

Method: 通过抓取 Reddit 网站上的帖子，收集关于 Excel 问题的提问和抱怨的数据集，探索和描述用户在使用电子表格软件时遇到的问题，特别是因大量数据导致的问题。

Result: 研究结果揭示了用户在使用 Excel 电子表格时遇到的各种问题，尤其是在处理大量数据时。
...
### [75] [The Reification of an Incorrect and Inappropriate Spreadsheet Model](https://arxiv.org/abs/1801.10249)
*Grenville J. Croll*

Main category: cs.HC

TL;DR: 电子表格中的信息容易被过度信任和神化。


<details>
  <summary>Details</summary>
Motivation: 研究一个小型非营利组织中不正确的电子表格模型如何被神化。

Method: 案例分析

Result: 观察到不正确的电子表格模型被组织内部人员过度信任。

Conclusion: 强调了电子表格信息被神化的风险。
...
### [76] [Hillview: A trillion-cell spreadsheet for big data](https://arxiv.org/abs/1907.04827)
*Mihai Budiu,Parikshit Gopalan,Lalith Suresh,Udi Wieder,Han Kruiger,Marcos K. Aguilera*

Main category: cs.DC

TL;DR: Hillview is a distributed spreadsheet for big data visualization, enabling interactive exploration of massive datasets.


<details>
  <summary>Details</summary>
Motivation: Exploring large datasets interactively is challenging due to single-machine limitations and responsiveness requirements.

Method: Hillview introduces visualization sketches (vizketches) that combine data summarization and efficient rendering techniques.

Result: Hillview can navigate and visualize datasets of tens of billions of rows and trillions of cells using eight servers.

Conclusion: Vizketches effectively scale the spreadsheet by parallelizing computation, reducing communication, providing progressive visualizations, and offering accuracy guarantees.
...
### [77] [Streaming 1.9 Billion Hypersparse Network Updates per Second with D4M](https://arxiv.org/abs/1907.04217)
...
TL;DR: This paper introduces a hierarchical associative array implementation within the D4M library to improve memory management and update speed for large streaming network datasets.


<details>
  <summary>Details</summary>
Motivation: Analyzing large streaming network data sets requires efficient memory management and high update rates, which are challenging for traditional D4M associative arrays.

Method: The paper designs and optimizes a hierarchical associative array implementation that reduces memory pressure and increases the update rate. Parameters control the number of entries in each level before updates cascade.

Result: The hierarchical arrays achieve over 40,000 updates per second in a single instance and scale to 1,900,000,000 updates per second on 1,100 server nodes.

Conclusion: The hierarchical D4M associative arrays enable the MIT SuperCloud to analyze extremely large streaming network data sets efficiently.
...
TL;DR: This paper introduces an Intelligent Tutoring System (ITS) for assessing students' Excel submissions automatically, providing individualized feedback based on value matching, formula analysis, and solution quality.


<details>
  <summary>Details</summary>
Motivation: Digital solutions for assessing spreadsheet assignments are scarce in teaching, motivating the development of an ITS for automated feedback.

Method: The ITS analyzes student submissions through value matching, formula analysis, and solution quality assessment. It uses feedback levels to provide gradually more detailed information about errors.

Result: Higher-level feedback led to a higher percentage of correct submissions and was perceived as understandable and helpful by students.

Conclusion: The developed ITS effectively reviews students' Excel submissions and provides individualized feedback automatically, enhancing the learning experience.
...
TL;DR: This paper introduces a method for integrating computational thinking into education using the Beaufort scale.


<details>
  <summary>Details</summary>
Motivation: To promote computational thinking as a fundamental skill and address the gap in digital problem-solving schemata.

Method: A novel approach is presented to support subject integration and building digital schemata, based on the Beaufort scale, within a Grade 8 action research study.

Result: Students' content knowledge and digital skills developed more efficiently compared to traditional methods.

Conclusion: The presented method can be adapted to other paper-based problems to enhance digital problem-solving skills and build schemata in both subject matter and informatics.
...
TL;DR: SAM: a new metric for characterizing shelter access patterns and client vulnerability, easily implemented by shelter staff.


<details>
  <summary>Details</summary>
Motivation: Provide shelter operators with an intuitive way to understand shelter access patterns, assess vulnerability, and evaluate the impact of external factors.

Method: Using client data from a large North American shelter to demonstrate that SAM produces similar results to traditional transitional, episodic, and chronic client cluster analysis.

Result: SAM produces similar results to traditional cluster analysis, and SAM demonstrates the impact of Housing First programming and the COVID-19 lockdown on shelter access patterns.
...
TL;DR: This paper discusses the creation and impact of two long-term mentoring programs, SIGPLAN-M and CALM, in the programming languages (PL) and computer architecture (CA) communities, respectively.


<details>
  <summary>Details</summary>
Motivation: The motivation was to address the lack of lasting connections in the PL and CA communities, which was exacerbated by the pandemic.

Method: The PL community impulsively launched SIGPLAN-M, while the CA community scientifically developed CALM. The paper shares the designs, impacts, and challenges of both programs.

Result: SIGPLAN-M has reached 328 mentees and 234 mentors across 41 countries, with mentees reporting it as "life changing" and "a career saver." CALM, in its pilot phase, has 13 mentors and 21 mentees across 7 countries and has received very positive feedback.

Conclusion: The authors hope that sharing their experiences will encourage the development of similar long-term mentoring programs across all of computer science.
...
TL;DR: This paper introduces a spreadsheet-based scheduling tool for the City of Toronto Long Term Care Homes & Services (LTCH&S) division, automating schedule generation and incorporating nurses' preferences.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenging task of scheduling nursing staff and reduce the high absenteeism rate among part-time nurses.

Method: A hierarchical optimization model is used to generate feasible schedules that maximize preference scores while meeting demand and seniority requirements.

Result: The scheduling tool allows nursing managers to generate feasible schedules in a fraction of an hour, and schedules successfully accounted for preferences with on average above 94% of the allocated shifts ranked as most preferred.

Conclusion: The implemented scheduling tool significantly reduces scheduling time and effectively incorporates nurse preferences.
...
TL;DR: ICAEW developed three papers on spreadsheet best practices over five years.


<details>
  <summary>Details</summary>
Motivation: To promote good spreadsheet use and work environments.

Method: Reviewing the history and key lessons of the three papers.

Result: ICAEW enhanced its position in the field through the paper creation process.

Conclusion: The paper discusses the development, lessons, and impact of ICAEW's thought leadership papers on spreadsheet practices.
...
TL;DR: This paper studies spreadsheet use in a UK higher education institution, finding widespread use and a need for clearer guidelines.


<details>
  <summary>Details</summary>
Motivation: To understand how spreadsheets are used in a higher education institution.

Method: A case study of spreadsheet use in academic registry and finance units.

Result: Large volume of spreadsheets, typical developer profiles, and need for clear guidelines.

Conclusion: The institution needs clear principles and guidelines for spreadsheet development to ensure data integrity, reduce duplication, and optimize use.
...
TL;DR: This article discusses the development of computer simulation training methods for neural networks in spreadsheet environments, and analyzes the history of computational neuroscience.


<details>
  <summary>Details</summary>
Motivation: To explore the necessity of developing training methods for computer simulation of neural networks in spreadsheet environments.

Method: Systematic review of applications, analysis of historical writings, and identification of key models.

Result: Identified basic approaches to network simulation training in spreadsheets, the role of the journal "Bulletin of Mathematical Biophysics" and key figures, and promising models for developing training methods.

Conclusion: Mastering models based on the historical and genetic approach is crucial for acquiring neural simulation competence in spreadsheet environments, highlighting continuous, discrete, and discrete-continuous models.
...
TL;DR: This paper introduces the Edu-Edition of the Spreadsheet Competency Framework (E2SCF) for finance professionals.


<details>
  <summary>Details</summary>
Motivation: Building spreadsheet competences should start early in education with expert teacher support for effective learning.

Method: The E2SCF features high mathability computer-supported real-world problem solving, two-directional knowledge transfer, data/error analysis, and spreadsheet programming.

Result: E2SCF is designed for basic and general users to build firm spreadsheet knowledge and transferable problem-solving skills.

Conclusion: The E2SCF framework is set up for basic and general users to build up firm spreadsheet knowledge and to develop transferable problem solving skills and competences.
...
TL;DR: This paper discusses the future of spreadsheet technology, considering the impact of Big Data, increased end-user computing, and mobile computing.


<details>
  <summary>Details</summary>
Motivation: To explore the future of spreadsheet technology and its consequences for users.

Method: A workshop was convened with academia and industry to discuss the success, trends, and future directions of spreadsheets.

Result: The paper records the views of participants on the reasons for the success of the spreadsheet, the trends driving change, and the likely directions of change for the spreadsheet. It also sets out key directions for further research in the evolution and use of spreadsheets.

Conclusion: The paper concludes by looking at the implications of these trends for the end users.
...
TL;DR: A survey of Clinical Decision Support System (CDSS) users from the Medical Algorithms Company website is analyzed to understand the impact of CDSS on clinical practice.


<details>
  <summary>Details</summary>
Motivation: To understand how CDSS impact clinical practice.

Method: A survey of CDSS users from the Medical Algorithms Company website.

Result: CDSS are used in various settings as an operational, research, and reference tool. The initial logic is worked out on a spreadsheet.

Conclusion: CDSS provided by Medal are being used by clinical professionals in a variety of settings.
...
TL;DR: This paper evaluates the performance of Large Language Models (LLMs) on spreadsheet tasks, finding limitations in complex operations.


<details>
  <summary>Details</summary>
Motivation: To assess LLMs' capabilities in spreadsheet-related tasks, which are underexplored.

Method: A comprehensive benchmark framework is introduced to evaluate LLMs in executing spreadsheet functions, formula generation, and data manipulation tasks.

Result: LLMs perform well on simple tasks but struggle with complex, multi-step operations, often producing incorrect outputs.

Conclusion: Current LLMs have limitations in handling spreadsheet tasks requiring precise logical reasoning, suggesting a need for integrating symbolic reasoning capabilities.
...
TL;DR: This paper introduces a text-driven approach to analyze diagrams in business documents by extracting textual metadata from source files (e.g., xlsx, pptx, docx) and feeding it into Large Language Models (LLMs), bypassing the limitations of Vision-Language Models (VLMs).


<details>
  <summary>Details</summary>
Motivation: Accurately identifying and extracting structures and relationships in diagrams using VLMs is challenging.

Method: The study extracts diagram information from xlsx files and transforms shape data into textual input for LLMs to analyze relationships and answer business-oriented questions.

Result: The proposed text-driven framework outperformed a VLM-based method in answering questions requiring detailed comprehension of diagram structures.

Conclusion: Direct textual extraction from original source files enables robust diagram understanding through LLMs, enhancing workflow efficiency and information analysis in business scenarios. The method can be extended to other document formats like pptx and docx.
...
TL;DR: This paper analyzes FM leaderboards to identify issues and improve transparency.


<details>
  <summary>Details</summary>
Motivation: Lack of standardized guidelines for FM evaluation hinders effective FM selection.

Method: Collect and analyze 1,045 FM leaderboards, identify workflow patterns, and develop a domain model.

Result: Identifies five workflow patterns and eight types of leaderboard smells.

Conclusion: Mitigating these smells improves transparency and accountability in FM comparison and selection.
...
TL;DR: This paper investigates whether dynamic arrays in Excel can replace traditional spreadsheet techniques for more professional development and improved solution integrity.


<details>
  <summary>Details</summary>
Motivation: Spreadsheets are popular but error-prone, raising questions about their suitability for serious analysis. The paper aims to address this by exploring the use of dynamic arrays.

Method: The paper examines the use of dynamic arrays within a professional development environment, comparing them to traditional techniques.

Result: The paper anticipates that fully dynamic models will require less manual intervention, reducing errors and risk.

Conclusion: The paper concludes by suggesting the adoption of dynamic arrays could improve solution integrity in spreadsheets.
...
TL;DR: This paper explores using unconventional Excel techniques to create structured solutions.


<details>
  <summary>Details</summary>
Motivation: Spreadsheets are widely used but lack the structural elements of conventional code, leading to scaling issues.

Method: The paper investigates controversial or lesser-used Excel techniques.

Result: The paper aims to create coherent solutions by sequencing formulas like a programming language.

Conclusion: The paper seeks to overcome the limitations of standard Excel practices by introducing structured programming approaches.
...
TL;DR: This paper introduces a method for model-checking real-time systems by exploiting variability in formal models and requirements, using MS Excel spreadsheets for configuration.


<details>
  <summary>Details</summary>
Motivation: To address the complexity of model checking real-time systems and the trade-off between detail and state explosion.

Method: Using MS Excel spreadsheets to describe the configuration of variability in formal specifications, automatically processed by a prototype tool to generate instances and run the model checker.

Result: Extending previous work by analyzing valid feature combinations while maintaining a simple spreadsheet interface.

Conclusion: The paper proposes an extension to their previous work, focusing on analyzing valid feature combinations while preserving the simplicity of a spreadsheet-based interface.
...
TL;DR: This paper introduces SheetCopilot, an LLM-powered agent for automating spreadsheet tasks using natural language. It defines atomic actions, uses a state machine for task planning, and provides a dataset and evaluation pipeline.


<details>
  <summary>Details</summary>
Motivation: End users spend significant time on repetitive and error-prone spreadsheet tasks but lack automation skills.

Method: The paper proposes SheetCopilot, an agent that uses natural language to control spreadsheets. It uses atomic actions and a state machine-based task planning framework.

Result: SheetCopilot achieves a 44.3% task completion rate, outperforming code generation baselines.

Conclusion: The paper demonstrates the potential of LLMs for software control, specifically in spreadsheet automation.
...
TL;DR: Excel's calculation engine upgrade in 2018, introducing Dynamic Arrays, has led to significant changes in spreadsheet solution development, moving towards more formal programming approaches.


<details>
  <summary>Details</summary>
Motivation: To demonstrate how the introduction of Dynamic Arrays in Excel has enabled a shift from ad-hoc practices to more structured, programming-like methods in spreadsheet solutions.

Method: Discussing emerging trends and pioneering work within the Excel community following the 2018 upgrade.

Result: Identifying trends in how the new Excel functionality is being used and its potential impact.

Conclusion: The paper concludes by highlighting the ongoing changes in spreadsheet practices due to Dynamic Arrays and their potential implications for risk management in business and engineering.
...
TL;DR: This paper introduces a new taxonomy for analytical spreadsheet models, expanding on a previous one to include nine types based on use case and engineering resources.


<details>
  <summary>Details</summary>
Motivation: To categorize and understand the different types of analytical spreadsheet models used in practice and in research.

Method: The authors extend a previous three-type taxonomy and connect it to disparate research literature.

Result: The paper identifies nine types of spreadsheet models and distinguishes between analytical solutions and industrial-quality models.

Conclusion: The taxonomy helps identify where spreadsheet development guidelines are most useful, provides a perspective on spreadsheet errors and risks, and offers a structure for understanding how spreadsheets evolve. It also suggests future research directions.
...
TL;DR: This paper explores ChatGPT's ability to generate spreadsheet formulas, highlighting both successes and limitations in reasoning and problem-solving.


<details>
  <summary>Details</summary>
Motivation: To assess the potential of LLMs like ChatGPT in democratizing computer programming through spreadsheet formula generation.

Method: Experimentally testing ChatGPT's performance in generating valid spreadsheet formulas under varying conditions of information availability and problem complexity.

Result: ChatGPT can generate correct spreadsheet formulas with correct reasoning in certain circumstances, but its accuracy diminishes with limited information, uncertainty, or complex problems, leading to errors and hallucinations.

Conclusion: ChatGPT demonstrates potential in spreadsheet formula generation but requires careful consideration of its limitations related to reasoning, inference, and deduction, especially in complex or uncertain scenarios.
...
TL;DR: CORNET is a system that automatically learns conditional formatting rules from user examples in spreadsheet software.


<details>
  <summary>Details</summary>
Motivation: Users need to manually write conditional formatting rules in spreadsheet software.

Method: CORNET combines symbolic rule enumeration, based on semi-supervised clustering and iterative decision tree learning, with a neural ranker.

Result: CORNET generates formatting rule suggestions for the user to apply to the spreadsheet after the user provides one or two formatted cells as examples.

Conclusion: CORNET is a simple add-in to Microsoft Excel that helps users automatically generate conditional formatting rules.
...
TL;DR: This paper introduces a tool that creates an abstract intermediate representation (AIR) of a spreadsheet to facilitate the transfer from spreadsheets into scientific programming languages while preserving inter-dependency information about data.


<details>
  <summary>Details</summary>
Motivation: Data scientists are moving towards scientific programming languages like Python for data analysis, but information like formulas and dependencies are lost when converting spreadsheets.

Method: The authors propose a tool that creates an abstract intermediate representation (AIR) of a spreadsheet.

Result: A python library is built on top of the tool to perform some data analysis in python.

Conclusion: The tool facilitates the transfer from spreadsheets into scientific programming languages while preserving inter-dependency information about data.
...
### [101] [Exploring Spreadsheet Use and Practices in a Technologically Constrained Setting](https://arxiv.org/abs/2201.07696)
*Khwima Mckinley Mkamanga,Simon Thorne*

Main category: cs.SE

TL;DR: 该论文探讨了电子表格在马拉维一家水务公司的业务运营中的影响，这家公司位于撒哈拉以南非洲，是一家典型的半政府机构，运营于技术欠发达国家。


<details>
  <summary>Details</summary>
Motivation: 研究重点关注电子表格的使用范围、生命周期以及组织政策和治理。结果将有助于通过影响管理组织中与电子表格相关的潜在风险的新方法来定义未来的电子表格使用。

Method: 研究方法未在摘要中明确说明。

Result: 调查结果表明，电子表格在该组织中的普及为业务自动化提供了有利环境。该论文还强调了导致普遍使用电子表格相关高风险的管理、技术和人为因素问题。

Conclusion: 研究得出的结论证实，在许多领域有很大的改进空间，例如实施管理电子表格开发过程和采用的综合政策和法规。
...
### [102] [Methodology for Assessing the State of the Practice for Domain X](https://arxiv.org/abs/2110.11575)
*Spencer Smith,Jacques Carette,Peter Michalski,Ao Dong,Olu Owojaiye*

Main category: cs.SE

TL;DR: The paper presents a methodology for assessing software development practices in research software domains.


<details>
  <summary>Details</summary>
Motivation: Understanding the current state of software development practices for research software.

Method: A methodology involving identifying software packages, gathering data, using a measurement template, interviewing developers, and ranking software using the Analytic Hierarchy Process (AHP).

Result: Estimation of 173 person hours to complete an assessment for a given domain.

Conclusion: The methodology, spreadsheet templates, and AHP tool can be used to assess software development practices.
...
### [103] [Agile Elicitation of Scalability Requirements for Open Systems: A Case Study](https://arxiv.org/abs/2108.00567)
*Gunnar Brataas,Antonio Martini,Geir Kjetil Hanssen,Georg Ræder*

Main category: cs.SE

TL;DR: This paper introduces the ScrumScale model, a lightweight artifact for eliciting scalability requirements in agile software development, and presents a case study where it was used in an open banking project.


<details>
  <summary>Details</summary>
Motivation: Eliciting scalability requirements during agile software development is complicated and poorly described in previous research.

Method: The ScrumScale model, a simple spreadsheet, is introduced and clarified using design science research and coordination theory. An open banking case study is presented to demonstrate its application.

Result: TietoEVRY spent 55 hours eliciting scalability requirements using the ScrumScale model. TietoEVRY found the model provided a systematic way of producing scalability requirements and improved communication with stakeholders.

Conclusion: The ScrumScale model offers a systematic approach to eliciting scalability requirements in agile development and facilitates communication among stakeholders.
...
### [104] [SpreadsheetCoder: Formula Prediction from Semi-structured Context](https://arxiv.org/abs/2106.15339)
*Xinyun Chen,Petros Maniatis,Rishabh Singh,Charles Sutton,Hanjun Dai,Max Lin,Denny Zhou*

Main category: cs.SE

TL;DR: This paper introduces SpreadsheetCoder, a BERT-based model for synthesizing spreadsheet formulas from tabular context, including headers and semi-structured data.


<details>
  <summary>Details</summary>
Motivation: Previous spreadsheet formula synthesis methods don't fully capture the rich context in real-world spreadsheets, especially the relationships between rows/columns and the information provided by headers.

Method: The paper proposes SpreadsheetCoder, a BERT-based model architecture that represents tabular context in both row-based and column-based formats. The model is trained on a large dataset of spreadsheets.

Result: SpreadsheetCoder achieves a top-1 prediction accuracy of 42.51%, a considerable improvement over baselines. It also assists 82% more users in composing formulas on Google Sheets compared to a rule-based system.

Conclusion: SpreadsheetCoder effectively leverages tabular context for spreadsheet formula synthesis, demonstrating significant improvements over existing methods.
...
### [105] [From webtables to datatables](https://arxiv.org/abs/2006.14694)
*Mária Csernoch*

Main category: cs.SE

TL;DR: 该论文探讨了如何利用网页表格（Webtables）在商业和专业组织中教授电子表格技能，通过知识转移、问题解决、讨论和计算思维的培养。以英雄联盟（LOL）排行榜的转换为例，提供了文字处理器和电子表格两种解决方案，鼓励讨论和创新。


<details>
  <summary>Details</summary>
Motivation: 利用网页表格在商业和专业组织中教授电子表格技能，培养计算思维。

Method: 以英雄联盟排行榜的转换为例，详细描述转换算法，并提供文字处理器和电子表格两种解决方案。

Result: 提供了两种解决方案，一种使用文字处理器，另一种纯粹使用电子表格应用程序。

Conclusion: 该论文为利用网页表格教授电子表格技能提供了一个实际案例和多种解决方案，鼓励进一步的讨论和创新。
...
### [106] [Implementation Strategies for Multidimensional Spreadsheets](https://arxiv.org/abs/2006.05814)
*Paul Mireault*

Main category: cs.SE

TL;DR: This paper analyzes how experienced Excel developers handle multi-dimensional variables in spreadsheets.


<details>
  <summary>Details</summary>
Motivation: To understand the different strategies employed when implementing spreadsheets with multi-dimensional variables in Excel.

Method: Analyzing spreadsheets created by experienced Excel developers in a challenge setting.

Result: Identified two main strategies: projection onto a 2D plane and a database approach using dataset tables.
...
### [107] [Abstracting spreadsheet data flow through hypergraph redrawing](https://arxiv.org/abs/2006.04794)
*David Birch,Nicolai Stawinoga,Jack Binks,Bruno Nicoletti,Paul Kelly*

Main category: cs.SE

TL;DR: This paper introduces a novel approach to enhance spreadsheets by raising their level of abstraction. It addresses the limitations of traditional spreadsheets, such as error-proneness due to low-level cells and hidden linkages.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of traditional spreadsheets, which suffer from low-level abstraction, hidden linkages, and limited cell content, leading to error-prone data models.

Method: The method involves transforming spreadsheets into fine-grained graphs, representing cells as hypergraph edges, and redrawing cell boundaries to create higher-level cells. Common sub-expression identification and sub-tree isomorphisms are used for vector operation detection.

Result: The approach is illustrated via common sub-expression identification and the application of sub-tree isomorphisms for the detection of vector (array) operations.

Conclusion: The paper concludes by proposing that researchers should explore techniques to redraw cell boundaries to create higher-level cells that more faithfully represent the end-user's real-world/mental model.
...
### [108] [Comprehensive review for common types of errors using spreadsheets](https://arxiv.org/abs/1912.09209)
*Ali Aburas*

Main category: cs.SE

TL;DR: This paper reviews existing research on error detection and correction in spreadsheets.


<details>
  <summary>Details</summary>
Motivation: Spreadsheets are widely used but prone to errors, impacting business organizations.

Method: Comprehensive review and classification of existing approaches.

Result: Discussion of up-to-date research work approaches in terms of definition, how they work, and kinds of errors they can find in spreadsheets. Identification of common errors made by end users.

Conclusion: Summarizes current approaches for finding and fixing errors in spreadsheets and identifies common user errors.
...
### [109] [A Coding-free Software Framework of Developing Web Data Management Systems](https://arxiv.org/abs/1910.05685)
*Can Yang,Shiying Pan,Runmin Li,Yu Liu,Lizhang Peng*

Main category: cs.SE

TL;DR: This paper introduces a coding-free method for constructing data management systems on the cloud using a SaaS architecture, allowing non-programmers to develop such systems.


<details>
  <summary>Details</summary>
Motivation: The increasing demand for cloud-based data management systems and the difficulty for non-programmers to develop them.

Method: A universal web platform is designed to generate customized system instances from a requirements table in spreadsheet.

Result: The empirical result demonstrates the feasibility and availability of the coding-free method in developing web data management systems.

Conclusion: The proposed framework and its web deployment show the potential of coding-free methods for developing web data management systems.
...
### [110] [Structured Spreadsheet Modelling and Implementation with Multiple Dimensions -- Part 2: Implementation](https://arxiv.org/abs/1909.00891)
*Paul Mireault*

Main category: cs.SE

TL;DR: This paper provides steps to implement a multi-dimensional problem into an easily maintainable spreadsheet.


<details>
  <summary>Details</summary>
Motivation: Building on a conceptual model from Part 1 involving multi-dimensional variables (Products, Regions, Sectors, Months), this paper addresses the need for a practical implementation method.

Method: Presents precise steps to translate the conceptual model into a spreadsheet.

Result: A spreadsheet that is easy to maintain.

Conclusion: Provides a method for implementing multi-dimensional problems in a practical and maintainable spreadsheet format, based on a previously developed conceptual model.
...
### [111] [On the Refinement of Spreadsheet Smells by means of Structure Information](https://arxiv.org/abs/1810.04542)
*Patrick Koch,Birgit Hofer,Franz Wotawa*

Main category: cs.SE

TL;DR: This paper introduces a new approach to detect smells in spreadsheets by exploiting inferred structural information, which reduces incorrect/redundant reports and reveals novel deficits.


<details>
  <summary>Details</summary>
Motivation: Current spreadsheet smell detection techniques have drawbacks that lead to incorrect or redundant smell reports.

Method: The paper proposes a static analysis approach to infer clusters and blocks of related cells, and then refines existing smells and proposes novel smell detection techniques using the inferred structures.

Result: The refinements reduce the number of incorrectly and redundantly reported smells, and novel deficits are revealed by the newly introduced smells.

Conclusion: The proposed techniques improve spreadsheet smell detection by incorporating structural information.
...
### [112] [Now You're Thinking With Structures: A Concept for Structure-based Interactions with Spreadsheets](https://arxiv.org/abs/1809.03435)
*Patrick Koch*

Main category: cs.SE

TL;DR: This paper introduces a new concept for structure-aware understanding of and interaction with spreadsheets.


<details>
  <summary>Details</summary>
Motivation: Spreadsheets are hard to comprehend and adapt after reaching a certain complexity. Cognition of complex systems is facilitated by having a higher order mental model.

Method: Structural information is used to enrich visualizations, reactively enhance traditional user actions, and provide tools to proactively alter the overall spreadsheet makeup instead of individual cells.

Result: Implemented a tool for structure inference and visualization along the common spreadsheet layout.

Conclusion: Providing the tools for thinking about and interacting with spreadsheets in this manner will benefit users both in terms of productivity and overall spreadsheet quality.
...
### [113] [Typed Table Transformations](https://arxiv.org/abs/1809.02746)
*Martin Erwig*

Main category: cs.SE

TL;DR: This paper introduces a new approach to transforming spreadsheet tables based on row and column type transformations.


<details>
  <summary>Details</summary>
Motivation: The motivation is that spreadsheet table labels represent types for the data, and the placement of values is controlled by these types.

Method: The method involves transformations of row and column types.

Result: The paper illustrates type-based table construction and transformation.

Conclusion: The paper lays out research questions for future work.
...
### [114] [Implementing WHERE and ORDER BY as spreadsheet formulas](https://arxiv.org/abs/1809.00025)
*Paul Mireault*

Main category: cs.SE

TL;DR: This paper introduces spreadsheet formulas that mimic SQL's WHERE and ORDER BY clauses, enabling dynamic filtering and sorting in spreadsheets.


<details>
  <summary>Details</summary>
Motivation: Existing spreadsheet tools for filtering and sorting data don't automatically update with changes in calculated values.

Method: Development of spreadsheet formulas to replicate the functionality of SQL's WHERE and ORDER BY clauses.

Result: The paper presents formulas that achieve SQL-like filtering and sorting within a spreadsheet.

Conclusion: The developed formulas provide a dynamic way to filter and sort data in spreadsheets, overcoming the limitations of built-in tools.
...
### [115] [The use of Charts, Pivot Tables, and Array Formulas in two Popular Spreadsheet Corpora](https://arxiv.org/abs/1808.10642)
*Bas Jansen,Felienne Hermans*

Main category: cs.SE

TL;DR: This paper analyzes the use of charts, pivot tables, and array formulas in spreadsheets using the Enron and EUSES corpora to improve spreadsheet quality.


<details>
  <summary>Details</summary>
Motivation: Spreadsheets are widely used in industry for decision-making, but they are error-prone, leading to potential financial losses. Existing research focuses on formulas, neglecting other constructions like charts, pivot tables, and array formulas.

Method: The paper analyzes the Enron and EUSES spreadsheet corpora to study the usage of charts, pivot tables, and array formulas.

Result: The paper analyzes two popular spreadsheet corpora: Enron and EUSES on the use of the aforementioned constructions.

Conclusion: To improve spreadsheet quality it is important to understand how spreadsheets are used and to obtain a complete understanding, the use of charts, pivot tables, and array formulas should be included in research.
...
### [116] [Asheetoxy: A Taxonomy for Classifying Negative Spreadsheet-related Phenomena](https://arxiv.org/abs/1808.10231)
*Daniel Kulesz,Stefan Wagner*

Main category: cs.SE

TL;DR: This paper introduces Asheetoxy, a new taxonomy for classifying spreadsheet phenomena that avoids the ambiguous term "error".


<details>
  <summary>Details</summary>
Motivation: Existing spreadsheet error taxonomies are ambiguous, require detailed knowledge, and cannot classify phenomena in available corpora.

Method: The authors propose a simple, phenomenon-oriented taxonomy called Asheetoxy and test it with 7 participants.

Result: A study with 7 participants indicates that even non-spreadsheet researchers similarly classify real-world spreadsheet phenomena using Asheetoxy.

Conclusion: Asheetoxy is a promising taxonomy for classifying spreadsheet phenomena.
...
### [117] [Proceedings of the 5th International Workshop on Software Engineering Methods in Spreadsheets (SEMS'18)](https://arxiv.org/abs/1808.09174)
...
<details>
  <summary>Details</summary>
Motivation: 介绍第五届软件工程方法在电子表格中的国际研讨会(SEMS'18)
...
### [118] [Combining Spreadsheet Smells for Improved Fault Prediction](https://arxiv.org/abs/1805.10493)
*Patrick Koch,Konstantin Schekotihin,Dietmar Jannach,Birgit Hofer,Franz Wotawa*

Main category: cs.SE

TL;DR: This paper introduces a machine learning approach using AdaBoost to combine spreadsheet smells for improved fault prediction accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing spreadsheet smells have limited predictive power for faults.

Method: Using an AdaBoost ensemble classifier to combine predictions of individual smells.

Result: Significant improvements in fault prediction accuracy on two real-world datasets.

Conclusion: The machine learning approach enhances fault prediction accuracy in spreadsheets by combining individual smell predictions.
...
### [119] [An Easy & Collaborative RDF Data Entry Method using the Spreadsheet Metaphor](https://arxiv.org/abs/1804.04175)
*Markus Schröder,Christian Jilek,Jörn Hees,Sven Hertling,Andreas Dengel*

Main category: cs.SE

TL;DR: A web-based spreadsheet editor that converts entries into RDF statements.


<details>
  <summary>Details</summary>
Motivation: To enable various users to easily create semantic data whether they are RDF experts or novices.

Method: A zero-configuration, web-based spreadsheet editor.

Result: Participants created more statements in less time with similar or better quality compared to other approaches.

Conclusion: The proposed editor allows users to easily create semantic data.
...
### [120] [Structured Spreadsheet Modelling and Implementation with Multiple Dimensions - Part 1: Modelling](https://arxiv.org/abs/1801.09777)
*Paul Mireault*

Main category: cs.SE

TL;DR: 该论文讨论了在电子表格软件中处理多维数据的方法，特别是时间维度和其他维度。


<details>
  <summary>Details</summary>
Motivation: 指出在电子表格中处理多维数据的常见方法（如重复公式块或创建多个工作表）效率低下。

Method: 论文中没有明确说明具体方法，但暗示了要提出一种更有效的方法来表示和操作多维数据。

Result: 论文中没有明确说明具体结果。

Conclusion: 论文中没有明确说明具体结论。
...
### [121] [Mitigating Spreadsheet Risk in Complex Multi-Dimensional Models in Excel](https://arxiv.org/abs/1802.01640)
*Steve Litt*

Main category: cs.SE

TL;DR: This paper introduces PivotModel, a solution designed to mitigate spreadsheet risk in complex multi-dimensional models within Microsoft Excel.


<details>
  <summary>Details</summary>
Motivation: Companies rely on Excel but face risks due to manual processes and errors in complex spreadsheets.

Method: The paper defines PivotModel, which works like a PivotTable but utilizes Excel's capabilities to address complex multi-dimensional models.

Result: The solution aims to mitigate spreadsheet risk.

Conclusion: PivotModel is presented as a solution to control spreadsheet risk for complex multi-dimensional models in Microsoft Excel.
...
### [122] [Proposed Spreadsheet Transparency Definition and Measures](https://arxiv.org/abs/1802.01628)
*Craig Hatmaker*

Main category: cs.SE

TL;DR: This paper defines spreadsheet modeling transparency for auditors and modelers.


<details>
  <summary>Details</summary>
Motivation: Lack of clear definition and measures for financial model transparency.

Method: Proposes a specific definition for spreadsheet modeling transparency.

Result: Enables creation of measures and tools for auditors and objective comparison of modeling methods.

Conclusion: Provides a basis for determining transparency requirements and selecting optimal modeling methods.
...
### [123] [Alternative Spreadsheet Model Designs for an Operations Management Model Embedded in a Periodic Business Process](https://arxiv.org/abs/1802.00484)
*Thomas A. Grossman,Vijay Mehrotra,Mouwafac Sidaoui*

Main category: cs.SE

TL;DR: This paper analyzes three spreadsheet implementations of an operations management model for supply and distribution planning.


<details>
  <summary>Details</summary>
Motivation: To evaluate the suitability of different spreadsheet designs (data-driven, canonical, and table-driven) for accuracy, modification, analysis, and transfer in the context of a periodic business process.

Method: The paper compares three spreadsheet implementations of a supply chain model: a data-driven design, a canonical design, and a novel table-driven design. It evaluates each design based on accuracy, ease of modification, analysis capabilities, and transferability.

Result: The technical (table-driven) design allows modification for new data and structural elements without manually editing cell formulas, which speeds up modification and reduces errors. The data-driven design highlights poor spreadsheet practices by inexperienced modelers.

Conclusion: The table-driven design is promising for other model classes and identifies areas for future research.
...
### [124] [Mitigating Spreadsheet Model Risk with Python Open Source Infrastructure](https://arxiv.org/abs/1801.09771)
*Oliver Beavers*

Main category: cs.SE

TL;DR: This paper introduces open-source Python tools for auditing spreadsheets, addressing the issue of errors due to the lack of software engineering practices in spreadsheet development.


<details>
  <summary>Details</summary>
Motivation: Spreadsheet models often suffer from errors due to the absence of standard software engineering tools and practices.

Method: The paper proposes using open-source Python packages to create reproducible audit tools for spreadsheets.

Result: The paper enables stakeholders to develop clearly defined model oracles for testing and auditing spreadsheet calculations.

Conclusion: The research provides a foundation for spreadsheet modeling professionals to enhance model reliability through Python-based audit tools.
...
### [125] [Structuring Spreadsheets with the "Lish" Data Model](https://arxiv.org/abs/1801.08603)
*Alan Hall,Michel Wermelinger,Tony Hirst,Santi Phithakkitnukoon*

Main category: cs.SE

TL;DR: This paper introduces a novel data model named "lish" for spreadsheet-like environments, which uses nested lists and templates to capture higher-level structures.


<details>
  <summary>Details</summary>
Motivation: Traditional spreadsheets lack awareness of larger structures, hindering comprehension and increasing errors due to formula replication.

Method: The paper proposes a "lish" data model based on cells organized into nested lists, where users can employ templates to prototype repeating structures.

Result: A small demonstration application showcases the "lish" in operation.

Conclusion: The "lish" data model aims to preserve the simplicity of spreadsheets while capturing higher structures using nested lists and templates.
...
### [126] [Automated Refactoring of Nested-IF Formulae in Spreadsheets](https://arxiv.org/abs/1712.09797)
*Jie Zhang,Shi Han,Dan Hao,Lu Zhang,Dongmei Zhang*

Main category: cs.SE

TL;DR: This paper introduces an AST-based automated approach to refactor nest-IF formulae in spreadsheets, improving readability and reducing errors.


<details>
  <summary>Details</summary>
Motivation: Nest-IF expressions in spreadsheets are hard to read, understand, and maintain, but users lack the skills to fix them.

Method: The approach detects and removes logic redundancy on the AST, identifies fragmented higher-level semantics, and reassembles the syntax using concise built-in functions.

Result: The approach relieved the smell of over 99% of nest-IF formulae and reduced nesting levels by more than half in over 50% of the refactorings. A user study showed that participants preferred the refactored formulae.

Conclusion: The automated refactoring approach is effective, necessary, and helpful for improving spreadsheet formulae.
...
### [127] [A CNN toolbox for skin cancer classification](https://arxiv.org/abs/1908.08187)
*Fabrizio Nunnari,Daniel Sonntag*

Main category: eess.IV

TL;DR: A software toolbox is developed for configuring CNNs for skin cancer classification, enabling both developers and non-technical users to experiment with different architectures and hyper-parameters.


<details>
  <summary>Details</summary>
Motivation: The motivation is to provide a flexible tool for configuring and experimenting with CNNs in the context of skin cancer classification, addressing the needs of both technical and non-technical users.

Method: The method involves a software architecture that allows quick setup of CNN architectures and hyper-parameter configurations, along with a user interface manageable as a spreadsheet.

Result: Preliminary results quantify the impact of image augmentation, image resolution, and rescaling filter on detection performance and training time using two CNNs for melanoma detection on dermoscopic images.

Conclusion: The toolbox facilitates experimentation and optimization of CNNs for skin cancer classification, with potential for future extensions like meta-learning and AutoML.
...
### [128] [Fortune: Formula-Driven Reinforcement Learning for Symbolic Table Reasoning in Language Models](https://arxiv.org/abs/2505.23667)
*Lang Cao,Jingxian Xu,Hanbing Liu,Jinyu Wang,Mengyu Zhou,Haoyu Dong,Shi Han,Dongmei Zhang*

Main category: cs.AI

TL;DR: This paper introduces Formula Tuning (Fortune), a reinforcement learning framework that trains LMs to generate executable spreadsheet formulas for question answering over tabular data.


<details>
  <summary>Details</summary>
Motivation: Large language models struggle with numerical and symbolic reasoning over tables, especially in complex scenarios.

Method: A reinforcement learning framework is used to train LMs to generate executable spreadsheet formulas, using answer correctness as a reward signal.

Result: Formula Tuning substantially enhances LM performance, particularly on multi-step numerical and symbolic reasoning tasks.

Conclusion: Formula-driven RL has the potential to advance symbolic table reasoning in LMs.
...
### [129] [The AI Risk Repository: A Comprehensive Meta-Review, Database, and Taxonomy of Risks From Artificial Intelligence](https://arxiv.org/abs/2408.12622)
...
TL;DR: The paper introduces an AI Risk Repository, a comprehensive database of AI risks extracted from various taxonomies, aiming to provide a common frame of reference for discussing, researching, and addressing AI risks.


<details>
  <summary>Details</summary>
Motivation: The lack of shared understanding of AI risks hinders comprehensive discussion, research, and response to them.

Method: The authors systematically reviewed AI risk taxonomies, consulted with experts, and developed two overarching taxonomies: a Causal Taxonomy based on entity, intentionality, and timing, and a Domain Taxonomy classifying risks into seven domains and 23 subdomains. They then curated and categorized 777 risks into a publicly accessible repository.

Result: The AI Risk Repository is the first attempt to rigorously curate, analyze, and extract AI risk frameworks into a comprehensive, extensible, and categorized risk database.

Conclusion: The AI Risk Repository creates a foundation for a more coordinated, coherent, and complete approach to defining, auditing, and managing the risks posed by AI systems.
...
### [130] [SpreadsheetLLM: Encoding Spreadsheets for Large Language Models](https://arxiv.org/abs/2407.09025)
...
TL;DR: SpreadsheetLLM introduces an efficient encoding method, SheetCompressor, to enhance LLMs' performance on spreadsheets by addressing token constraints and improving compression.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with spreadsheets' 2D grids and formatting. This paper aims to improve LLMs' understanding and reasoning on spreadsheets.

Method: Developed SheetCompressor, an encoding framework with structural-anchor-based compression, inverse index translation, and data-format-aware aggregation. Also, proposed Chain of Spreadsheet for downstream tasks.

Result: SheetCompressor improves performance by 25.6% in GPT4's in-context learning and achieves a state-of-the-art 78.9% F1 score after fine-tuning, with a 25x compression ratio.

Conclusion: SpreadsheetLLM is highly effective across various spreadsheet tasks by leveraging the layout and structure of spreadsheets.
...
### [131] [SheetAgent: Towards A Generalist Agent for Spreadsheet Reasoning and Manipulation via Large Language Models](https://arxiv.org/abs/2403.03636)
*Yibin Chen,Yifu Yuan,Zeyu Zhang,Yan Zheng,Jinyi Liu,Fei Ni,Jianye Hao,Hangyu Mao,Fuzheng Zhang*

Main category: cs.AI

TL;DR: This paper introduces SheetRM, a new benchmark for evaluating LLMs in complex spreadsheet manipulation tasks, and proposes SheetAgent, a novel autonomous agent that outperforms existing methods by using planning, information gathering, and retrieval modules for improved reasoning and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs struggle with complex, real-world spreadsheet tasks requiring reasoning. This paper aims to address this gap by introducing a more realistic benchmark and a novel agent designed for such tasks.

Method: The paper introduces SheetRM, a benchmark for long-horizon, multi-category spreadsheet tasks. It also proposes SheetAgent, an autonomous agent with Planner, Informer, and Retriever modules for iterative task reasoning and reflection.

Result: SheetAgent achieves 20-40% improvement in pass rates on multiple benchmarks compared to baselines, demonstrating enhanced precision and table reasoning abilities.

Conclusion: The paper concludes that SheetAgent effectively addresses the challenges of complex spreadsheet manipulation by leveraging LLMs and a modular design, achieving superior performance compared to existing methods.
...
### [132] [Large Language Model for Table Processing: A Survey](https://arxiv.org/abs/2402.05121)
*Weizheng Lu,Jing Zhang,Ju Fan,Zihao Fu,Yueguo Chen,Xiaoyong Du*

Main category: cs.AI

TL;DR: This survey paper comprehensively reviews table-related tasks and the use of Large Language Models (LLMs) and Visual Language Models (VLMs) in automating these tasks.


<details>
  <summary>Details</summary>
Motivation: Automating table-centric tasks with LLMs/VLMs is beneficial and attracts interest.

Method: The survey covers user scenarios, technical aspects, training techniques, and prompt engineering for LLMs/VLMs in table processing.

Result: Summarizes training techniques and discusses prompt engineering for various table-related tasks.

Conclusion: Highlights challenges like diverse user input and slow thinking using chain-of-thought.
...
### [133] [FormaT5: Abstention and Examples for Conditional Table Formatting with Natural Language](https://arxiv.org/abs/2310.17306)
*Mukul Singh,José Cambronero,Sumit Gulwani,Vu Le,Carina Negreanu,Elnaz Nouri,Mohammad Raza,Gust Verbruggen*

Main category: cs.AI

TL;DR: FormaT5是一个基于Transformer的模型，用于根据目标表格和自然语言描述生成条件格式(CF)规则。


<details>
  <summary>Details</summary>
Motivation: 用户在电子表格软件中编写数据相关的条件格式(CF)规则时面临挑战，因为这需要他们理解和实现底层逻辑。

Method: FormaT5通过预测占位符来学习，并通过一个弃权目标来解决欠规范问题，并通过编程示例系统填充这些占位符。

Result: FormaT5在包含来自四个不同来源的真实世界描述的1053个CF任务的基准测试中，优于8种不同的神经方法。

Conclusion: 弃权和填充允许FormaT5在基准测试中胜过8种不同的神经方法，无论有没有示例。
...
### [134] [Don't Treat the Symptom, Find the Cause! Efficient Artificial-Intelligence Methods for (Interactive) Debugging](https://arxiv.org/abs/2306.12850)
*Patrick Rodler*

Main category: cs.AI

TL;DR: This paper introduces model-based diagnosis, a domain-independent approach for troubleshooting complex systems to minimize downtime and repair costs.


<details>
  <summary>Details</summary>
Motivation: The increasing complexity and dependency on sophisticated systems in the modern world leads to potential system failures with significant negative effects, necessitating methods to minimize harm from failures.

Method: The thesis introduces model-based diagnosis and discusses research approaches addressing major challenges in the field, using techniques from knowledge representation, automated reasoning, and machine learning.

Result: The paper does not present specific results but rather provides an overview and discussion of approaches.

Conclusion: The thesis serves as an introduction to model-based diagnosis, highlighting challenges and discussing research directions.
...
### [135] [CORNET: Learning Table Formatting Rules By Example](https://arxiv.org/abs/2208.06032)
*Mukul Singh,José Cambronero,Sumit Gulwani,Vu Le,Carina Negreanu,Mohammad Raza,Gust Verbruggen*

Main category: cs.AI

TL;DR: CORNET是一个自动从用户提供的格式化单元格示例中学习条件格式规则的系统。


<details>
  <summary>Details</summary>
Motivation: 电子表格中的样式格式对于演示和分析都很重要，但编写格式规则对用户来说具有挑战性，因为它需要了解底层的规则语言和数据逻辑。

Method: CORNET结合了符号规则枚举和神经排序器来学习条件格式规则。

Result: CORNET能够准确地学习各种评估设置中的规则，并且能够找到比用户编写的更短的规则，以及发现用户手动格式化的电子表格中的规则。

Conclusion: CORNET系统能够有效地从用户提供的示例中学习和应用电子表格的条件格式规则。
...
### [136] [Named Entity Recognition in Industrial Tables using Tabular Language Models](https://arxiv.org/abs/2209.14812)
*Aneta Koleva,Martin Ringsquandl,Mark Buckley,Rakebul Hasan,Volker Tresp*

Main category: cs.AI

TL;DR: 本研究探讨了如何将基于Transformer的表格数据编码模型应用于工业命名实体识别(NER)问题，特别是在表格结构电子表格中识别实体。面临的挑战包括电子表格的技术性以及缺乏标记数据。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于将表格Transformer应用于工业界常见的表格数据，解决工业界命名实体识别问题，特别是电子表格中的实体识别。

Method: 研究方法包括：1) 基于领域知识图谱开发专门的表格数据增强策略；2) 比较表格结构作为归纳偏置与线性化序列的效果。

Result: 实验结果表明，表格Transformer优于其他基线模型，并且其表格归纳偏置对于Transformer模型的收敛至关重要。

Conclusion: 研究结论是，表格Transformer在低资源场景下表现出色，表格结构作为归纳偏置对于模型性能至关重要。
...
### [137] [Spreadsheet computing with Finite Domain Constraint Enhancements](https://arxiv.org/abs/2203.10944)
*Ezana N. Beyenne*

Main category: cs.AI

TL;DR: This thesis introduces a framework extending spreadsheet computing to solve constraint satisfaction problems by integrating a finite constraint solver. It enhances usability with spreadsheet-specific constraints and demonstrates the extended paradigm's utility.


<details>
  <summary>Details</summary>
Motivation: Spreadsheet applications are widely used but limited by single-direction data flow, restricting them to bookkeeping tasks.

Method: A framework is presented that integrates a finite constraint solver with the spreadsheet paradigm, allowing cells to be attached to finite domains or constraints. Spreadsheet-specific constraints are introduced to improve scalability.

Result: The framework enhances the spreadsheet computing paradigm, enabling it to solve constraint satisfaction problems.

Conclusion: The extended spreadsheet paradigm's usability and usefulness are demonstrated through examples, showcasing its ability to overcome limitations of traditional spreadsheet applications.
...
### [138] [Human-Machine Collaboration for Democratizing Data Science](https://arxiv.org/abs/2004.11113)
*Clément Gautrais,Yann Dauxais,Stefano Teso,Samuel Kolb,Gust Verbruggen,Luc De Raedt*

Main category: cs.AI

TL;DR: VisualSynth is a system for human-machine collaboration in data science that allows users to interact with spreadsheet software to perform and automate various data analysis tasks.


<details>
  <summary>Details</summary>
Motivation: Democratize data science by allowing users to interact with standard spreadsheet software to perform data analysis tasks.

Method: The user provides colored sketches to partially specify data science tasks, which are then determined and executed using AI techniques.

Result: The system can perform and automate various data analysis tasks ranging from data wrangling, data selection, clustering, constraint learning, predictive modeling and auto-completion.

Conclusion: The paper introduces VisualSynth, a novel framework and system for human-machine collaboration in data science.
...
### [139] [Automated Discovery of Data Transformations for Robotic Process Automation](https://arxiv.org/abs/2001.01007)
*Volodymyr Leno,Marlon Dumas,Marcello La Rosa,Fabrizio Maria Maggi,Artem Polyvyanyy*

Main category: cs.AI

TL;DR: This paper introduces a method to discover data transformation routines in UI logs for RPA, focusing on data transfers between spreadsheets or web forms.


<details>
  <summary>Details</summary>
Motivation: Companies need to identify and automate repetitive routines to fully utilize RPA, but discovering automatable routines is challenging.

Method: The paper maps the problem to data transformation discovery by example and proposes optimizations to improve efficiency, considering UI log information and the separation of alphabetic and numeric tokens.

Result: The proposed approach and optimizations are evaluated using UI logs that replicate a real-life data transfer routine.

Conclusion: The paper presents a solution for discovering data transformation routines in UI logs to enhance RPA implementation, with optimizations for efficiency.
...
### [140] [Neural-Guided Deductive Search for Real-Time Program Synthesis from Examples](https://arxiv.org/abs/1804.01186)
*Ashwin Kalyan,Abhishek Mohta,Oleksandr Polozov,Dhruv Batra,Prateek Jain,Sumit Gulwani*

Main category: cs.AI

TL;DR: This paper introduces Neural Guided Deductive Search (NGDS), a hybrid approach combining symbolic logic and statistical models for program synthesis from input-output examples.


<details>
  <summary>Details</summary>
Motivation: Existing synthesis systems struggle with real-time synthesis on challenging benchmarks, relying heavily on either hand-engineered deductive logic or data-intensive statistical models.

Method: NGDS combines deductive search with neural networks, using the former to simplify the learning problem for the latter, enabling training on limited real-world data.

Result: NGDS achieves up to 12x speed-up compared to state-of-the-art systems while synthesizing accurate programs in real-world customer scenarios.

Conclusion: NGDS effectively leverages both symbolic logic and statistical models for program synthesis, achieving improved speed and accuracy on real-world benchmarks.
...
### [141] [Mathematics of Digital Hyperspace](https://arxiv.org/abs/2103.15203)
*Jeremy Kepner,Timothy Davis,Vijay Gadepally,Hayden Jananthan,Lauren Milechin*

Main category: cs.MS

TL;DR: This paper introduces the semilink concept for enhanced data manipulation in digital hyperspace, leveraging GraphBLAS for graph analytics, database operations, and machine learning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of representing, traversing, and transforming unstructured data in digital hyperspace.

Method: The paper explores the semilink concept, combining semirings, and utilizes the GraphBLAS standard.

Result: The paper suggests that with semilinks and key-based indices, GraphBLAS can become a richer associative array algebra, potentially replacing spreadsheets, database tables, and data-centric operating systems.

Conclusion: The conclusion is that semilinks, combined with GraphBLAS, can significantly improve the navigation and manipulation of unstructured data in digital hyperspace.
...
TL;DR: This paper introduces dynamic kernel matching (DKM), a method for adapting statistical classifiers to non-conforming data, and demonstrates its application to T-cell receptor sequence data for disease diagnosis.


<details>
  <summary>Details</summary>
Motivation: The motivation is to uncover patterns in non-conforming data where standard statistical classifiers are not directly applicable, specifically in datasets of T-cell receptor sequences related to disease antigen and CMV serostatus.

Method: The method involves modifying established statistical classifiers using dynamic kernel matching (DKM) to handle non-conforming data.

Result: The study successfully fits statistical classifiers augmented with DKM to both datasets and reports performance on holdout data using standard metrics.

Conclusion: The patterns identified by the classifiers agree with observations from experimental studies, suggesting the validity and potential utility of the DKM approach.
...
TL;DR: This paper introduces a system for spreadsheet-based metadata entry that ensures adherence to community standards and provides quality control.


<details>
  <summary>Details</summary>
Motivation: Researchers prefer spreadsheets for metadata entry despite their limitations in consistency and compliance.

Method: The system uses customizable templates, controlled terminologies/ontologies, and a web-based tool for error identification and correction.

Result: The approach is deployed in the HuBMAP consortium for metadata collection across various biological assays.

Conclusion: The system supports spreadsheet-based metadata entry while ensuring rigorous adherence to community-based metadata standards and providing quality control.
...
TL;DR: This paper analyzes transformative agreements between academic publishers and research institutions using the ESAC Initiative database.


<details>
  <summary>Details</summary>
Motivation: Research institutions are 'trapped' in transformative agreements, hindering the transition to fully Open Access.

Method: Web-scraping data from the ESAC Initiative database and combining qualitative and quantitative methods.

Result: Academia is stuck in the hybrid system, endowing legacy publishers with market power, raising entry barriers, lowering competition, and increasing costs.

Conclusion: Transformative agreements may not be an effective bridge to a fully Open Access world.
...
TL;DR: This paper introduces a novel approach for spreadsheet-based metadata entry with enhanced compliance and quality control.


<details>
  <summary>Details</summary>
Motivation: Spreadsheets are widely used for metadata entry but lack compliance and quality control. Existing tools have limitations like complexity and customization issues.

Method: The approach involves customizable templates, controlled terminologies, and a web-based tool for error identification and correction.

Result: The approach is demonstrated in a biomedical consortium for metadata collection about scientific experiments.

Conclusion: The paper presents an end-to-end solution for improving metadata quality and compliance in spreadsheet-based workflows.
...
TL;DR: This paper introduces the Synthesis documentation system to address challenges in historical science research, such as collaborative documentation, detail representation, data structure extension, and data reuse.


<details>
  <summary>Details</summary>
Motivation: Current methods using spreadsheets or simple relational databases have limitations in collaborative documentation, representing inferred relations, extending data structures, integrating diverse sources, and reusing data.

Method: The paper presents the Synthesis documentation system, a Web-based and collaborative tool that utilizes CIDOC-CRM and RDF standards for semantic interoperability and long-term data validity.

Result: The Synthesis system is used by a large number of historians in an ongoing research project in the field of History of Art.

Conclusion: The system focuses on semantic interoperability and the production of data of high value and long-term validity.
...
TL;DR: This paper introduces FAST CAT, a collaborative system for data entry and curation in Digital Humanities, addressing limitations of current data management practices in empirical research.


<details>
  <summary>Details</summary>
Motivation: Current data management practices in empirical sciences have limitations such as high dependency on initial research hypothesis, lack of representation of details, and difficulty in revisiting original data sources.

Method: The paper presents FAST CAT, a collaborative system for assistive data entry and curation. It describes the related challenges and the methodology for supporting semantic interoperability.

Result: The paper discusses the use of FAST CAT in the context of the SeaLiT project, which examines the impacts of steamboats in the Mediterranean area.

Conclusion: FAST CAT is presented as a solution to improve data management and collaboration in Digital Humanities and similar empirical research fields.
...
TL;DR: This paper uses online incremental machine learning to detect ransomware encryption activities, even with evolving obfuscation techniques.


<details>
  <summary>Details</summary>
Motivation: The increasing sophistication of ransomware encryption methods (like Base64 encoding and partial encryption) necessitates advanced countermeasures.

Method: Online incremental machine learning algorithms, specifically Hoeffding Tree and Random Forest, are applied to predict file encryption. A dataset of 32.6 GB with 11,928 files encrypted by 75 ransomware families is used.

Result: Hoeffding Tree excels at detecting traditional and AES-Base64 encryption. Random Forest with warm-start is better at identifying intermittent encryption.

Conclusion: Tailored machine learning solutions are necessary to counter sophisticated ransomware strategies.
...
TL;DR: JUBILEE is a secure, incentive-compatible mechanism for debt relief using secure computation and blockchain, improving upon previous methods by ensuring debtor consent and higher settlement efficiency.


<details>
  <summary>Details</summary>
Motivation: To create a debt relief mechanism that is secure, fair, and efficient, addressing limitations of existing methods that rely on trusted third parties.

Method: Uses secure computation techniques and smart contracts on a blockchain to enable private information revelation and debt settlement.

Result: Achieves individual rationality, incentive compatibility, truthfulness, ex-post efficiency, and optimality in debt relief, with higher expected profits and success probability.

Conclusion: JUBILEE offers a practical and improved solution for debt relief by leveraging secure computation and blockchain technology, ensuring debtor consent and more harmonious debt settlements.
...
### [150] [LEI2JSON: Schema-based Validation and Conversion of Livestock Event Information](https://arxiv.org/abs/2310.17414)
*Mahir Habib,Muhammad Ashad Kabir,Lihong Zheng*

Main category: eess.SY

TL;DR: LEI2JSON is a Google Sheets add-on that helps livestock producers standardize their data by converting it to JSON format and validating it against a schema.


<details>
  <summary>Details</summary>
Motivation: Livestock producers need help standardizing livestock event data.

Method: A Google Sheets add-on, LEI2JSON, is used to build spreadsheet templates, convert data to JSON, and validate against a schema.

Result: LEI2JSON enables efficient data standardization, saving time and resources. Experimental evaluation assesses the tool's effectiveness.

Conclusion: LEI2JSON facilitates seamless storage of livestock event information locally or on Google Drive in JSON.
...
TL;DR: This paper introduces a catenary trajectory design to minimize wellbore friction in drilling, potentially reducing costs.


<details>
  <summary>Details</summary>
Motivation: Wellbore friction is a significant concern in drilling due to its impact on total cost. The catenary concept aims to address this, but requires further analysis.

Method: The paper uses a case study to compare Catenary Trajectory Design with traditional 2D Arc design.  A calculation procedure is implemented in an MS Excel spreadsheet.

Result: The case study examines the outcome of using a catenary trajectory versus a 2D arc design.

Conclusion: The MS Excel spreadsheet provides a user-friendly and reliable tool for designing catenary well trajectories for extended-reach wells, potentially minimizing torque and drag.
...
TL;DR: This paper introduces a two-layer model-based hazard analysis method that combines formal methods and simulation to identify unsafe behaviors in safety-critical systems, demonstrated on a human-robot collaboration system.


<details>
  <summary>Details</summary>
Motivation: Current hazard analysis methods are insufficient for increasingly complex systems, and testing-based methods can be costly or dangerous.

Method: The proposed approach synthesizes unsafe behaviors from a formal model using Supervisory Control Theory and then inputs the results into a simulation for detailed analysis using domain-specific risk metrics.

Result: The benefits of the approach are demonstrated on an industrial human-robot collaboration system.

Conclusion: The two-layer approach combines the benefits of exhaustive analysis using formal methods with detailed analysis using simulation.
...
### [153] [Is spreadsheet syntax better than numeric indexing for cell selection?](https://arxiv.org/abs/2505.23296)
*Philip Heltweg,Dirk Riehle,Georg-Daniel Schwarz*

Main category: cs.PL

TL;DR: This paper compares spreadsheet-style syntax and numeric indexing for cell selection in data engineering tasks.


<details>
  <summary>Details</summary>
Motivation: To improve the speed and correctness of reading and writing code for data practitioners when selecting a subset of cells.

Method: A controlled experiment with student participants was conducted to compare the two syntax options.

Result: Spreadsheet-style syntax resulted in fewer mistakes and faster code writing compared to numeric syntax. Participants also made fewer mistakes when reading code with spreadsheet-style syntax.

Conclusion: Spreadsheet syntax is a promising alternative to numeric syntax in data engineering tools for practitioners without a software engineering background.
...
### [154] [FLAME: A small language model for spreadsheet formulas](https://arxiv.org/abs/2301.13779)
*Harshit Joshi,Abishai Ebenezer,José Cambronero,Sumit Gulwani,Aditya Kanade,Vu Le,Ivan Radiček,Gust Verbruggen*

Main category: cs.PL

TL;DR: FLAME is a small transformer model (60M parameters) trained on Excel formulas that outperforms larger models in formula repair, completion, and retrieval.


<details>
  <summary>Details</summary>
Motivation: Large language models for formula authoring are expensive and difficult to deploy.

Method: Transformer model trained on Excel formulas with sketch deduplication, Excel-specific tokenizer, and domain-specific pre-training objectives (masked span prediction and noisy auto-encoding).

Result: FLAME outperforms Davinci (175B), Cushman (12B), and CodeT5 (220M) in 10 of 14 evaluation settings for repair and completion. It also outperforms CodeT5, CodeBERT, and GraphCodeBERT in formula retrieval.

Conclusion: FLAME leverages domain insights to achieve competitive performance with a substantially smaller size and less training data.
...
### [155] [Enhanced Spreadsheet Computing with Finite-Domain Constraint Satisfaction](https://arxiv.org/abs/2203.16346)
*Ezana N. Beyenne,Hai-Feng Guo*

Main category: cs.PL

TL;DR: This paper introduces an enhanced spreadsheet system that supports finite-domain constraint solving in a visual environment, breaking the mono-directional data flow limitation of traditional spreadsheets.


<details>
  <summary>Details</summary>
Motivation: Spreadsheets are widely used but limited to bookkeeping-like applications due to their mono-directional data flow.

Method: The paper extends the spreadsheet computing paradigm and constructs a spreadsheet-specific constraint language for users to specify constraints among data cells.

Result: The new system simplifies the development of constraint-based applications using a visual tabular interface.

Conclusion: The extended spreadsheet paradigm is more usable and useful, as demonstrated by examples.
...
### [156] [Typed Image-based Programming with Structure Editing](https://arxiv.org/abs/2110.08993)
*Jonathan Edwards,Tomas Petricek*

Main category: cs.PL

TL;DR: This paper introduces a new approach to enable collaboration in image-based programming through types and structure editing, focusing on schema change on persistent data.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of image-based programming in collaboration and deployment compared to conventional file-based programming, which has hindered its commercial success.

Method: The method involves using static types and structure editing to capture changes in type definitions and automatically adapt the data to suit. The paper presents a theory for version control of structure editing.

Result: The paper presents a theory realizing the idea of version control for structure editing.

Conclusion: The paper concludes that the proposed approach could offer a new way to collaborate in image-based programming if extended to cover the entire programming experience.
...
### [157] [ExceLint: Automatically Finding Spreadsheet Formula Errors](https://arxiv.org/abs/1901.11100)
*Daniel W. Barowy,Emery D. Berger,Benjamin Zorn*

Main category: cs.PL

TL;DR: This paper introduces ExceLint, a static analysis tool for detecting errors in spreadsheet formulas by identifying surprising disruptions in rectangular regions, outperforming existing methods in speed and effectiveness.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the high risk of errors in spreadsheets, especially in finance, by providing a tool to detect formula errors.

Method: The method involves a static analysis that leverages the rectangular structure of spreadsheets and uses an information-theoretic approach to identify surprising disruptions in nearby rectangular regions.

Result: The result is ExceLint, an implementation of the static analysis, which is shown to be fast (median of 5 seconds per spreadsheet) and more effective than state-of-the-art analysis across a corpus of 70 spreadsheets.

Conclusion: ExceLint is a fast and effective tool for detecting errors in spreadsheet formulas.
...
TL;DR: FORTAP通过利用大量的电子表格公式进行数值推理感知表预训练，在单元格类型分类和公式预测两个下游任务上取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 表格存储了丰富的数值数据，但基于表格的数值推理仍然是一个挑战。电子表格公式可以对表格中的数值进行计算，可以作为数值推理的强监督。

Method: 设计了两个公式预训练任务，以明确地指导FORTAP学习半结构化表格中的数值参考和计算。

Result: FORTAP在单元格类型分类和公式预测两个代表性的下游任务上取得了最先进的结果。
...
TL;DR: This paper introduces Mondrian, a novel approach for automatically identifying layout templates and extracting regions in multiregion spreadsheet files.


<details>
  <summary>Details</summary>
Motivation: The widespread use of spreadsheets leads to large data collections, but their flexible structure hinders automated analysis, especially with multiregion files.

Method: Mondrian renders spreadsheets as images, identifies potential region elements, clusters them into regions, and then compares file layouts as graphs to find templates.

Result: Mondrian outperforms state-of-the-art table recognition algorithms in detecting region boundaries and identifying recurring layouts on real-world enterprise spreadsheets.

Conclusion: Mondrian effectively identifies layout templates and extracts regions in multiregion spreadsheets, improving automated analysis.
...
TL;DR: TUTA is a unified pre-training architecture for understanding generally structured tables.


<details>
  <summary>Details</summary>
Motivation: Understanding a table requires spatial, hierarchical, and semantic information.

Method: Enhance transformers with three novel structure-aware mechanisms: a bi-dimensional coordinate tree, tree-based attention and position embedding, and three progressive pre-training objectives.

Result: Achieving state-of-the-art on five widely-studied datasets.

Conclusion: TUTA is highly effective for table structure understanding.
...
TL;DR: TableSense is a novel end-to-end framework for spreadsheet table detection, using CNN and active learning.


<details>
  <summary>Details</summary>
Motivation: Automatic table detection is a key enabling technique in spreadsheet data intelligence, but is challenged by the diversity of table structures and layouts.

Method: Develops a cell featurization scheme, an enhanced CNN model, and an active learning based smart sampling algorithm.

Result: Achieves 91.3% recall and 86.5% precision in EoB-2 metric, outperforming existing methods.

Conclusion: TableSense is highly effective for spreadsheet table detection.
...
TL;DR: This paper introduces a method for recommending related tables by computing the similarity between tables using multiple semantic spaces and a discriminative learning model.


<details>
  <summary>Details</summary>
Motivation: To provide users of spreadsheet programs with recommendations for related structured content on the Web.

Method: Representing table elements in multiple semantic spaces, and then combining element-level similarities using a discriminative learning model.

Result: The proposed approach delivers state-of-the-art performance on a purpose-built test collection from Wikipedia tables.

Conclusion: The paper demonstrates a theoretically sound framework for performing table matching and achieves state-of-the-art performance.
...
TL;DR: SmartTable is an online spreadsheet with intelligent assistance for relational tables, helping users populate rows and extend columns.


<details>
  <summary>Details</summary>
Motivation: To provide intelligent assistance for users working with relational tables in online spreadsheets.

Method: An online spreadsheet application with features for populating rows and extending columns.

Result: The application is implemented and released as open source.

Conclusion: SmartTable offers intelligent assistance for relational tables by helping users populate rows and extend columns.
...
### [164] [Wordle: A Microcosm of Life. Luck, Skill, Cheating, Loyalty, and Influence!](https://arxiv.org/abs/2309.02110)
*James P. Dilger*

Main category: math.HO

TL;DR: The study analyzes Wordle player behavior using publicly available data from the New York Times.


<details>
  <summary>Details</summary>
Motivation: To understand cheating, player preferences, and influence in Wordle.

Method: Data compilation and analysis of Wordle players' first guesses from May-August 2023.

Result: Identified cheating, starting word preferences, and the impact of external influences on player behavior.

Conclusion: Provides quantitative evidence of cheating and player behavior in Wordle, going beyond social media analysis.
...
### [165] [GeoGebra e situações que envolvem modelação numa abordagem STEAM](https://arxiv.org/abs/1907.02099)
*J. M. D. S. Dos Santos,A. P. Silveira,A. E. S. Trocado*

Main category: math.HO

TL;DR: This paper introduces STEAM approach materials using GeoGebra for math classes in Portuguese-speaking countries, initially for teachers and later for students. The materials focus on modeling geometric problems and using GeoGebra's various windows to analyze them. The tasks are designed for users of all knowledge levels and connect math with other sciences and arts.


<details>
  <summary>Details</summary>
Motivation: To implement a STEAM approach in mathematics classes using GeoGebra in the lusophone space.

Method: Modeling situations in 2D and 3D geometric problems and using GeoGebra software for analysis, with tasks designed for users of all knowledge levels and supported by scripts.

Result: Tasks developed for training courses have an impact on the STEAM approach and are being adapted for Spanish and English speakers.

Conclusion: The tasks allow connections with other Sciences and the Arts, and allow the development of projects using and consolidating relevant mathematical contents. The materials will be adapted for Spanish and English speakers due to their interest in the Iberian space and global dissemination.
